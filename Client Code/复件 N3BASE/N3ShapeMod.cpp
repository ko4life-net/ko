...
CN3Shape::CPart* CN3ShapeMod::GetPartByPMeshName(LPCTSTR pszName)
{
	void* pPos = m_Parts.Position(0);
	for(; pPos; m_Parts.Next(pPos))
	{
		CPart* pPart = m_Parts.Get(pPos);
		if (pPart == NULL) continue;
		CN3PMesh* pPMesh = pPart->Mesh();
		if (pPMesh == NULL) continue;
		if (lstrcmpi(pPMesh->FileName(), pszName) == 0) return pPart;
	}
	return NULL;
}
...
{
	__ASSERT(m_Parts.Count()>0, "먼저 shape를 load해야 한다.");
...
//	}
	...
	for (i=0; i<m_iModPartCount; ++i)
	{
		...
		m_pModParts[i].pPart = GetPartByPMeshName(szPMeshName);
	...
	// m_pMatchPart2ModPart CPart갯수만큼 생성 및 초기화(매칭시키기)
	...
	{
		void* pPos = m_Parts.Position(0);
		for(j=0; pPos; m_Parts.Next(pPos), ++j)
		{
			CPart* pPart = m_Parts.Get(pPos);
			if (pPart == m_pModParts[i].pPart)
			{
...
void CN3ShapeMod::ReCalcMatrix()
{
	...
	void* pPos = m_Parts.Position(0);
	int count;
	for(count = 0; pPos; m_Parts.Next(pPos), ++count)
	{
		pPart = m_Parts.Get(pPos);
		if (pPart == NULL) continue;
		if (m_pMatchPart2ModPart[count])
		{
			__ModPart* pModPart = m_pMatchPart2ModPart[count];
			// Scale
			pPart->m_Matrix.Scale(pModPart->CurStateInfo.vScale);
			// 회전
			static __Matrix44 mtxRot;
			D3DXMatrixRotationQuaternion(&mtxRot, &(pModPart->CurStateInfo.qRot));
			pPart->m_Matrix *= mtxRot;

			// 위치
			pPart->m_Matrix.PosSet(pPart->m_vPivot + pModPart->CurStateInfo.vPos);
...
void CN3ShapeMod::Tick(float fFrm)
{
	...
	{
		m_fTimeChanging += (1.0f/s_fFrmPerSec);
