...
void CN3GESnow::Tick()
{
	...
	float		fAddRadian = D3DX_PI/(10.f*s_fFrmPerSec);
...
//		pV1->x += vAdd.x;	pV1->y += vAdd.y;	pV1->z += vAdd.z;
		...
		if (fDiff < 0)	// 높이 범위를 벗어났을 경우
		{
			...
			pParticle->vOffset2.Set(__Cosine(fRadian)*m_fSnowSize/2, -sqrt3*m_fSnowSize/6.f, __Sine(fRadian)*m_fSnowSize/2);
			pParticle->vOffset3.Set(-__Cosine(fRadian)*m_fSnowSize/2, -sqrt3*m_fSnowSize/6.f, -__Sine(fRadian)*m_fSnowSize/2);
		}
		else
		{
...
void CN3GESnow::Create(float fDensity, float fWidth, float fHeight, float fSnowSize, __Vector3& vVelocity)
{
	Release();
