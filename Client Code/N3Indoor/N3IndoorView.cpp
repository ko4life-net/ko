...
void CN3IndoorView::TickRender()
{
	...
	if(pFrm)	pFrm->SetFPSString((LPCTSTR)szFPS);
}
...
LRESULT CN3IndoorView::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if (!pFrm) return CView::WindowProc(message, wParam, lParam);
...
BOOL CN3IndoorView::ExecuteModeMsgFilter(MSG* pMsg)
{
	...
	COrganizeView* pView = NULL;
	pFrm = (CMainFrame*)AfxGetMainWnd();
	if(pFrm)	pView = pFrm->GetOrganizeView();	else return FALSE;
				...
				switch(pMsg->wParam)
				{
					case 0x41:					// 왼쪽..
						...
						}
						{
							...
							__Vector3 vec = vAt - vPos;	vec.Normalize();
							__Matrix44 mtx; mtx.RotationY(-fRY*10.0f);	vec *= mtx;
					...
					case 0x57:					// 전진..
						...
						}
						{
							__Vector3 vPos = pFrm->m_Camera.EyePos();
							__Vector3 vAt = pFrm->m_Camera.AtPos();
							__Vector3 vec = vAt - vPos;	vec.Normalize();	vec *= 0.6f;
					...
					case 0x58:					// 밑으로..
						...
						}
						{
							__Vector3 vPos = pFrm->m_Camera.EyePos();
							__Vector3 vAt = pFrm->m_Camera.AtPos();
							__Vector3 vec; vec.Set(0.0f, -0.1f, 0.0f);
...
void CN3IndoorView::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	COrganizeView* pView = NULL;
	pFrm = (CMainFrame*)AfxGetMainWnd();
	if(pFrm)	pView = pFrm->GetOrganizeView();
