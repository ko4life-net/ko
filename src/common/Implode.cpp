...
const static unsigned char DistBits[] = {0x02, 0x04, 0x04, 0x05, 0x05, 0x05, 0x05, 0x06, 0x06, 0x06, 0x06, 0x06, 0x06,
...
const static unsigned char DistCode[] = {0x03, 0x0D, 0x05, 0x19, 0x09, 0x11, 0x01, 0x3E, 0x1E, 0x2E, 0x0E, 0x36, 0x16,
...
const static unsigned char ExLenBits[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};

const static unsigned short LenBase[] = {0x0000, 0x0001, 0x0002, 0x0003, 0x0004, 0x0005, 0x0006, 0x0007,
                                         0x0008, 0x000A, 0x000E, 0x0016, 0x0026, 0x0046, 0x0086, 0x0106};

const static unsigned char LenBits[] = {0x03, 0x02, 0x03, 0x03, 0x04, 0x04, 0x04, 0x05,
                                        0x05, 0x05, 0x05, 0x06, 0x06, 0x06, 0x07, 0x07};

const static unsigned char LenCode[] = {0x05, 0x03, 0x01, 0x06, 0x0A, 0x02, 0x0C, 0x14,
...
void WriteCmpData(TCmpStruct * pWork) {
    ...
    do {
        ...
        while (1) {
            ...
            loaded =
                pWork->read_buf((char *)&pWork->work_buff[UNCMP_OFFSET + total_loaded], &bytes_required_, pWork->param);
        ...
        while (uncmp_end > uncmp_begin) {
            ...
            if (nreps1 != 0) {
                while (1) {
                    ...
                    if (nreps >= nreps1 || (nreps + 1 >= nreps1 && save_offs0000 <= 0x80)) {
                        nreps1 = nreps;
                    ...
                    LABEL_41:
                        ...
                        if (nreps1 == 2) {
                            ...
                            OutputBits(
                                pWork, pWork->dist_bits[pWork->offs0000 >> ((unsigned char)(pWork->dsize_bits & 0xff))],
                ...
                if ((uncmp_end - uncmp_begin) >= 2 && (uncmp_end - uncmp_begin != 2 || pWork->offs0000 < 0x100)) {
                    goto LABEL_41;
                }
            }
        LABEL_29:
...
unsigned int PKEXPORT implode(unsigned int (*read_buf)(char * buf, unsigned int * size, void * param),
                              void (*write_buf)(char * buf, unsigned int * size, void * param), char * work_buf,
                              void * param, unsigned int * type, unsigned int * dsize) {
