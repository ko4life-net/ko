        ...
        for (int j = 0; j < iDataCount; j++) {
            int iSuccess =
                this->ParseLine(szLine, iOffset, iVal, dwVal, dfVal,
                                szText); // 성공하면 0, 추가해야하고 끝이면 -1 추가할 필요 없고 끝이면.. -2 리턴
...
bool CTableGenerator::Generate(int iIndex, const std::string & szEnumFileName, const std::string & szTxtFileName) {
    ...
    std::string szGrade[10] = {"",        "브론즈 ", "실버 ", "골든 ",     "플레티넘 ",
                               "크림슨 ", "루나 ",   "솔라 ", "에인션트 ", "미스틱 "};
    ...
    std::vector<std::string> * pLineArrays = new std::vector<
        std::string>[iCountWhole + 1]; // 라인 버퍼에 쓸 문자열 배열 준비.. +1을 하는 이유는 첫줄에 설명 넣을려고 한다.
    ...
    for (int i = 0; i < iCountWhole; i++) {
        ...
        if (iType == iExt) {
            ...
            for (int j = 0; j < iDTCountBasic; j++) {
                ...
                case IG1_REQ_CHA: {
                    bool bApplyAbility =
                        false; // 요구 능력치가 0 일때 무시하고... 계산한 값이 0 보다 작을때 0으로 표시할것..
                    ...
                    if (DT_STRING != dt) {
                        ...
                        if (bApplyAbility) // 요구 능력치로 계산..
                        {
                            if (0 == m_Datas[j].m_iValues[iIndexCur1]) {
                                iValue = 0; // 요구 능력치가 0 이면 계산하지 않는다.
                            }
                            if (iValue < 0) {
                                iValue = 0;
                            }
    ...
    for (int i = 0; i <= iCountWhole; i++) // 설명까지 쓴다.
    {
        ...
        for (int j = 0; j < iColCount; j++) {
            ...
            if (j < iColCount - 1) {
                WriteFile(hFile, "\t", 1, &dwRWC, NULL); // 탭 구분..
            } else {
                WriteFile(hFile, "\r\n", 2, &dwRWC, NULL); // 줄 넘기기.
            }
        }
...
bool CTableGenerator::Convert2Bin(const std::string & szFN) {
    ...
    while (fgets(line, iMaxStrLen, stream)) {
        ...
        if (!bQuotationActived) {
            ++iCount; // 따옴표 쳐져있는데 줄 바뀌면 카운트를 세지 않는다.
        }
    }
    ...
    while (iRowCount < iCount) {
        ...
        while (iColCount < iDataCount) {
            if (bQuotationActived) { // 문자열이면
                if (NULL == token) {
                    ...
                    if (iLast >= 0 && '\"' == token[iLast]) { // 따옴표가 있으면 문자열 끝이다.
                        bQuotationActived = false;
            ...
            if (FALSE == (token || (NULL == token && DT_STRING == m_DataTypes[iColCount])) &&
                ...
                int iSelect = MessageBox(hWnd,
                                         "테이블에 문자열 외에 빈 칸이 있습니다.\nYes : 멈춤\nNo : 이번만 무시하고 "
                                         "계속\n Cancel : 모두 무시하고 계속",
                                         "Convert Error", MB_YESNOCANCEL);
                if (IDYES == iSelect) {
                    MessageBox(hWnd, "데이타 기록이 중단되었습니다. 이 파일은 제대로 작동 되지 않을 것입니다.",
                               "Convert Error", MB_OK);
                    CloseHandle(hFile);
                    fclose(stream);
                    ASSERT(0);
        ...
        if (bQuotationActived) {
            continue; // 따옴표가 열려 있으면 계속 문자열이므로 다시 위로..
        }
    ...
    if (iCount == iRowCount) {
        ...
        wsprintf(
            szErr,
            "전체 %d개의 데이타중 %d개의 데이타만 기록이 완료되었습니다.\n이 테이블은 제대로 작동되지 않을것입니다.",
            iCount, iRowCount);
