...
BOOL CRscTablesDlg::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	if((void*)wParam == &m_ListBoxDataType)
	{
		...
		if (iIndex < 0 || iIndex >= m_Generator.DataTypeCount()) return TRUE;
		...
		if (lstrcmpi(pItem->m_curValue, "char") == 0) DataType = DT_CHAR;
		else if (lstrcmpi(pItem->m_curValue, "BYTE") == 0) DataType = DT_BYTE;
		else if (lstrcmpi(pItem->m_curValue, "short") == 0) DataType = DT_SHORT;
		else if (lstrcmpi(pItem->m_curValue, "WORD") == 0) DataType = DT_WORD;
		else if (lstrcmpi(pItem->m_curValue, "int") == 0) DataType = DT_INT;
		else if (lstrcmpi(pItem->m_curValue, "DWORD") == 0) DataType = DT_DWORD;
		else if (lstrcmpi(pItem->m_curValue, "string") == 0) DataType = DT_STRING;
		else if (lstrcmpi(pItem->m_curValue, "float") == 0) DataType = DT_FLOAT;
		else if (lstrcmpi(pItem->m_curValue, "double") == 0) DataType = DT_DOUBLE;
...
void CRscTablesDlg::UpdateInfo()
{
	...
	for (i=0; i<iDataCount; ++i)
	{
		...
		{
		case DT_CHAR: iSel = 0; break;
		case DT_BYTE: iSel = 1; break;
		case DT_SHORT: iSel = 2; break;
		case DT_WORD: iSel = 3; break;
		case DT_INT: iSel = 4; break;
		case DT_DWORD: iSel = 5; break;
		case DT_STRING: iSel = 6; break;
		case DT_FLOAT: iSel = 7; break;
		case DT_DOUBLE: iSel = 8; break;
...
{
	if(pszFN_Enm) *pszFN_Enm = "";
	if(pszFN_Txt) *pszFN_Txt = "";
	...
	if(bOpen)
	{
		if(iIndex == -2)
		{
			if(pszFN_Enm)
			{
				...
				std::string szFN = szFName; szFN += szExt;
...
void CRscTablesDlg::OnBOpenTableBasic() 
{
	...
	if(false == this->BrowseDataEnumAndTxt(-2, TRUE, &szFN_Enm, &szFN_Txt)) return;
...
{
	std::string szFN_Enm;
	if(false == this->BrowseDataEnumAndTxt(-1, TRUE, &szFN_Enm, NULL)) return;
...
void CRscTablesDlg::OnDblclkListTableExt() 
{
	...
	if(iSel < 0 || iSel >= MAX_ITEM_EXTENSION) return;
	
	std::string szFN_Txt;
	if(false == this->BrowseDataEnumAndTxt(iSel, TRUE, NULL, &szFN_Txt)) return;
...
void CRscTablesDlg::OnBGenerateSelected() 
{
	...
	std::string szFN_Enm, szFN_Txt;
	if(false == this->BrowseDataEnumAndTxt(-3, FALSE, &szFN_Enm, &szFN_Txt)) return;
...
void CRscTablesDlg::OnItemFileListLoad() 
{
	...
	for(int i = 0; i < MAX_ITEM_EXTENSION + 3; i++)
	{
		...
		if(iLen > 0) szBuffs[i][iLen-1] = NULL; // '\r'
		else szBuffs[i][0] = NULL;
	}
