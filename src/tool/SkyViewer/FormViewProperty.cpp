...
void CFormViewProperty::OnInitialUpdate() {
    ...
    m_LPProperty.AddPropItem("종류", "", PIT_COMBO,
                             "하늘색|안개색|별수|달모양|해의 색|해의 Glow 색|해의 Flare 색|구름층1의 색|구름층2의 "
                             "색|구름텍스처 바꾸기|라이트0|라이트1|라이트2");
...
BOOL CFormViewProperty::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT * pResult) {
    if ((void *)wParam == &m_LPProperty) {
        ...
        if (pItem->m_propName == "이름") {
            pSDC->szName = pItem->m_curValue;
        } else if (pItem->m_propName == "시간") {
            int iHour = 0, iMin = 0;
            sscanf(pItem->m_curValue, "%d시%d분", &iHour, &iMin);
            pSDC->dwWhen = CONVERT_SEC(iHour, iMin, 0);
        } else if (pItem->m_propName == "종류") {
            pSDC->eSkyDayChange = (eSKY_DAYCHANGE)(pItem->m_crColor);
        } else if (pItem->m_propName == "인수1") {
            pSDC->dwParam1 = atoi(pItem->m_curValue);
        } else if (pItem->m_propName == "색1") {
            pSDC->dwParam1 = ::_RGB_To_D3DCOLOR(pItem->m_crColor, (DWORD)255);
        } else if (pItem->m_propName == "인수2") {
            pSDC->dwParam2 = atoi(pItem->m_curValue);
        } else if (pItem->m_propName == "색2") {
            pSDC->dwParam2 = ::_RGB_To_D3DCOLOR(pItem->m_crColor, (DWORD)255);
        } else if (pItem->m_propName == "지연시간") {
            pSDC->fHowLong = (float)(atof(pItem->m_curValue));
        }
...
void CFormViewProperty::UpdateInfo() {
    ...
    if (NULL == pSDC) {
        ...
        if (pItem0) {
            pItem0->m_curValue = pSDC->szName.c_str();
        }
        if (pItem1) {
        ...
        if (pItem2) {
            pItem2->m_crColor = pSDC->eSkyDayChange;
        }

        if (pItem3) {
            pItem3->m_curValue.Format("%d", pSDC->dwParam1);
        }
        if (pItem4) {
            pItem4->m_crColor = ::_D3DCOLOR_To_RGB(pSDC->dwParam1);
        }
        if (pItem5) {
            pItem5->m_curValue.Format("%d", pSDC->dwParam2);
        }
        if (pItem6) {
            pItem6->m_crColor = ::_D3DCOLOR_To_RGB(pSDC->dwParam2);
        }

        if (pItem7) {
            pItem7->m_curValue.Format("%.2f", pSDC->fHowLong);
        }
    }
...
void CFormViewProperty::OnSize(UINT nType, int cx, int cy) {
    ...
    if (NULL == m_LPProperty.GetSafeHwnd()) {
        return;
    }
