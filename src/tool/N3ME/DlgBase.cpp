...
void CDlgBase::UpdateInfo() {
    ...
    CN3Base * pBase = NULL;
    if (iCount == 1) {
        pBase = m_pMapMng->GetSelOutputObj(0);
    }
    if (pBase == NULL) {
    ...
    if (pBase->Type() & OBJ_CHARACTER) {
        ...
        //            if(pSkin) pItem->m_curValue = pSkin->FilePath().c_str();
        ...
        if (nPart >= 0 && nPart < nPartCount) {
            ...
            //    if (pMesh) {
            //        pItem->m_curValue = pMesh->FilePath().c_str();
            //    } else {
            //        pItem->m_curValue = "";
            //    }
            //}
            ...
            //    if (pSkin) {
            //        pItem->m_curValue = pSkin->FilePath().c_str();
            //    } else {
            //        pItem->m_curValue = "";
            //    }
            //}
        ...
        if ((void *)wParam == &m_LPCPart && (pBase->Type() & OBJ_CHARACTER)) {
            ...
            if (nPart >= 0 && nPart < nPartCount) {
                ...
                if (pItem->m_propName == "Part Type") {
                    ...
                    this->UpdateInfo();
                    //} else if (pItem->m_propName == "Mesh File" && pItem->m_curValue.GetLength() > 0) {
                    //    pPart->MeshSet(nLOD, CN3BaseFileAccess::ToRelative(pItem->m_curValue.GetString()));
                    //    this->UpdateInfo();
                    //} else if (pItem->m_propName == "Skin File" && pItem->m_curValue.GetLength() > 0) {
                    //    pPart->SkinSet(nLOD, CN3BaseFileAccess::ToRelative(pItem->m_curValue.GetString()));
                    //    this->UpdateInfo();
                }
            }
        }
