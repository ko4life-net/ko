...
LPSOUNDINFO CDlgSetSound::GetSoundGroup(DWORD dwID) {
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        if (pSoundInfo && pSoundInfo->dwID == dwID) {
            return pSoundInfo;
        }
    }
    return NULL;
...
void CDlgSetSound::OnBtnDeleteSoundgroup() {
    ...
    if (SndInfoID == m_iIdx_Min) {
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            if (m_iIdx_Min > pSoundInfo->dwID) {
                m_iIdx_Min = pSoundInfo->dwID;
            }
        }
    }
    ...
    if (SndInfoID == m_iIdx_Max) {
        ...
        for (int i = 0; i < cnt; i++) {
            pSoundInfo = (LPSOUNDINFO)m_ListSoundGroup.GetItemDataPtr(i);
            if (m_iIdx_Max < pSoundInfo->dwID) {
                m_iIdx_Max = pSoundInfo->dwID;
            }
        }
    }
...
void CDlgSetSound::AddSoundGroup(char * szTitle, LPSOUNDINFO pSoundInfo) {
    ...
    if (pSoundInfo->dwID < m_iIdx_Min) {
        m_iIdx_Min = pSoundInfo->dwID;
    }
    if (pSoundInfo->dwID > m_iIdx_Max) {
        m_iIdx_Max = pSoundInfo->dwID;
    }
}
...
void CDlgSetSound::OnBtnInputInfo() {
    ...
    if (m_pRefSoundMgr->m_pCurrSound->m_Rect.left == 0 && m_pRefSoundMgr->m_pCurrSound->m_Rect.right == 0 &&
        m_pRefSoundMgr->m_pCurrSound->m_Rect.top == 0 && m_pRefSoundMgr->m_pCurrSound->m_Rect.bottom == 0) {
        return;
    }
...
void CDlgSetSound::AddSoundInfo(CSoundCell * pSound) {
    ...
    if (idx >= 0) {
        ...
        char szBuff2[256];
        sprintf(szBuff2, "(%d,%d)->(%d,%d) : %s", pSound->m_Rect.left, pSound->m_Rect.top, pSound->m_Rect.right,
                pSound->m_Rect.bottom, szBuff1);
