...
BOOL CDlgSetDTex::OnInitDialog() {
    ...
    pVertices[2].Set(m_fTexSurfaceSize - 1.0f, m_fTexSurfaceSize - 1.0f, 0.1f, 0.5f, 0x00000000, DTexSize / DTexSize,
                     DTexSize / DTexSize);
...
void CDlgSetDTex::OnMouseMove(UINT nFlags, CPoint point) {
    ...
    if (rt.PtInRect(point) && (index > -1)) {
        ...
        int GroupID;
        if (pDTex) {
            GroupID = pDTex->m_Attr[ix][iy].Group;
        }
...
void CDlgSetDTex::OnBtnAddgroup() {
    ...
    for (i = 0; i < NumGroup; i++) {
        ...
        if (str == dlg.m_GroupName) {
            break;
        }
    }
    if (i == NumGroup) {
        m_GroupList.InsertString(i, dlg.m_GroupName);
    }
}
...
void CDlgSetDTex::OnBtnDelgroup() {
    ...
    if (dlg.DoModal() == IDOK) {
        ...
        //Do we need to update the map information as well? Since the group information is invalid, it should be deleted....
...
void CDlgSetDTex::OnBtnDeldtex() {
    ...
    if (index == count - 1) {
        m_FileList.SetCurSel(index - 1);
    } else {
        m_FileList.SetCurSel(index);
    }

    //Do we need to update the map information as well? Missing texture information is updated with another texture from the same group.
    CLyTerrain * pRefTerrain = pFrm->GetMapMng()->GetTerrain();
    if (pRefTerrain) {
        int ix, iz;
        for (ix = 0; ix < pRefTerrain->m_iHeightMapSize; ix++) {
...
void CDlgSetDTex::OnBtnLoadTileset() {
    ...
    if (dlg.DoModal() == IDOK) {
        ...
        if (dlg.m_SelFileName == m_TileSetName) {
            return;
        }
