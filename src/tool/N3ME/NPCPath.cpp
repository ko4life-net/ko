...
bool CNPCPath::CheckValid() {
    ...
    if (m_LTStartVertex.x < 0 || m_LTStartVertex.x > MapSize || m_LTStartVertex.z < 0 || m_LTStartVertex.z > MapSize) {
        return false;
    }

    if (m_RBStartVertex.x < 0 || m_RBStartVertex.x > MapSize || m_RBStartVertex.z < 0 || m_RBStartVertex.z > MapSize) {
        return false;
    }

    if (m_LTActVertex.x < 0 || m_LTActVertex.x > MapSize || m_LTActVertex.z < 0 || m_LTActVertex.z > MapSize) {
        return false;
    }

    if (m_RBActVertex.x < 0 || m_RBActVertex.x > MapSize || m_RBActVertex.z < 0 || m_RBActVertex.z > MapSize) {
        return false;
    }
