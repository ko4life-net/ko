...
bool CMainFrame::IsDuplicated(CString & PathName) {
    ...
    std::list<CDlgEditPartBillBoard *>::iterator itPartBillBoard;
    for (itPartBillBoard = m_pEditPartBillBoard.begin(); itPartBillBoard != m_pEditPartBillBoard.end();
         itPartBillBoard++) {
...
void CMainFrame::TickRender() {
    ...
    if (m_bRenderGrid) {
        m_Eng.RenderGrid(mtxScale);
    }
    if (m_bRenderAxis) {
        m_Eng.RenderAxis(true);
    }
