...
struct CZipAutoHandle {
    ...
    bool CreateMapping(HANDLE hFile) {
        m_hFileMap = CreateFileMapping(hFile, NULL, PAGE_READWRITE, 0, 0, _T("ZipArchive Mapping File"));
        if (!m_hFileMap) {
            return false;
        }
...
bool CZipCentralDir::RemoveDataDescr(bool bFromBuffer) {
    ...
    for (int i = 0; i < m_headers.GetSize(); i++) {
        ...
        uToCopy = (i == (m_headers.GetSize() - 1) ? uSize : m_headers[i + 1]->m_uOffset) - pHeader->m_uOffset -
                  uExtraHeaderLen;
