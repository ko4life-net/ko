...
bool CZipArchive::IsClosed(bool bArchive) {
    return bArchive ? (m_storage.GetCurrentDisk() == -1)
                    : (!m_storage.m_pFile || m_storage.m_pFile->m_hFile == CFile::hFileNull);
}
...
bool CZipArchive::ExtractFile(WORD uIndex, LPCTSTR lpszPath, bool bFullPath, LPCTSTR lpszNewName,
                              ZIPCALLBACKFUN pCallback, void * pUserData, DWORD nBufSize) {
...
void CZipArchive::DeleteFiles(CStringArray & aNames, bool bCaseSensitive) {
    ...
    for (WORD i = 0; i < GetNoEntries(); i++) {
        ...
        for (int j = 0; j < aNames.GetSize(); j++) {
            bool bEqual = (bCaseSensitive ? aNames[j].Collate(szFileName) : aNames[j].CollateNoCase(szFileName)) == 0;
            if (bEqual) {
                indexes.Add(i);
                break;
            }
...
bool CZipArchive::AddNewFile(LPCTSTR lpszFilePath, int iLevel, bool bFullPath, ZIPCALLBACKFUN pCallback,
                             void * pUserData, unsigned long nBufSize) {
    if (!nBufSize) {
        return false;
    }
...
int CZipArchive::WideToSingle(LPCTSTR lpWide, CZipAutoBuffer & szSingle) {
    ...
    int iLen = WideCharToMultiByte(CP_ACP, 0, lpWide, wideLen, szSingle, 0, NULL, NULL);
    if (iLen > 0) {
        szSingle.Allocate(iLen, true);
        iLen = WideCharToMultiByte(CP_ACP, 0, lpWide, wideLen, szSingle, iLen, NULL, NULL);
        ASSERT(iLen != 0);
...
int CZipArchive::SingleToWide(CZipAutoBuffer & szSingle, CString & szWide) {
    ...
    if (iLen > 0) {
        iLen = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, szSingle, singleLen, szWide.GetBuffer(iLen), iLen);
...
const z_crc_t * CZipArchive::GetCRCTable() {
