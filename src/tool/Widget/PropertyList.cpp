...
int CPropertyList::AddPropItem(const char * szPropName, const char * szVal, int nItemType, const char * szCmbItems,
                               COLORREF crColor) {
...
void CPropertyList::OnSelchange() {
    ...
    if (pItem->m_nItemType == PIT_COMBO) {
        ...
        if (m_ComboBox) {
            ...
            rect.bottom += 300;
            m_ComboBox.Create(CBS_DROPDOWNLIST | CBS_NOINTEGRALHEIGHT | WS_VISIBLE | WS_CHILD | WS_BORDER, rect, this,
                              IDC_PROPCMBBOX);
...
void CPropertyList::DisplayPushButton(CRect region) {
    ...
    if (m_ButtonPush) {
        ...
        m_ButtonPush.Create("...", BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD, region, this, IDC_PROP_BUTTON);
    ...
    if (pItem->m_nItemType == PIT_COLOR) {
        ...
        if (!fsDir.empty()) {
            if (fsDir.is_absolute()) {
                ...
                szInitialDir = (fs::current_path() / fsDir).string();
            }
        }
...
CPropertyItem * CPropertyList::GetPropItem(const char * szPropName) {
    ...
    for (int i = 0; i < nC; i++) {
        ...
        if (lstrcmpi(pItem->m_propName, szPropName) == 0) {
            return pItem;
        }
    }
...
CPropertyItem::CPropertyItem(const char * szPropName, const char * szVal, int nItemType,
                             const char * szComboItemsOrFilter, COLORREF crColor) {
...
void CPropertyItem::D3DColorValueSet(D3DCOLORVALUE color) {
    m_crColor =
        (((DWORD)(color.r * 255.0f)) << 0) | (((DWORD)(color.g * 255.0f)) << 8) | (((DWORD)(color.b * 255.0f)) << 16);
