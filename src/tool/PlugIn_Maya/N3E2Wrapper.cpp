...
void CN3E2Wrapper::SceneExport() {
    ...
    HWND      hWnd = M3dView::applicationShell();
    int       rval =
        DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_EXPORT_OPTION), hWnd, DlgProcPane, NULL); // 컨트롤 패널 대화상자..
    if (rval != 1) {
        return;
    }
    ...
    if (NULL == g_pEng->s_lpD3DDev) {
        ...
        MessageBox(::GetActiveWindow(), "VideoMemory 가 부족합니다. VideoMemory 확보 후 다시 시도하세요.",
                   "Data Export Error", MB_OK);
        return;
    }
    ...
    for (int i = 0; !IterNodes1.isDone(); IterNodes1.next(), i++) {
        mObjects.append(IterNodes1.item());
    }
    for (int i = 0; !IterNodes2.isDone(); IterNodes2.next(), i++) {
        mObjects.append(IterNodes2.item());
    }
    for (int i = 0; !IterNodes3.isDone(); IterNodes3.next(), i++) {
        mObjects.append(IterNodes3.item());
    }
    for (int i = 0; !IterNodes4.isDone(); IterNodes4.next(), i++) {
        mObjects.append(IterNodes4.item());
    }
    ...
    for (int i = 0; i < nObjectCount && FALSE == m_bCancelExport; i++) {
        ...
        if (TRUE == m_bCancelExport) {
            break; // Export Cancel 버튼을 누르면 취소..
        }

        MObject   mObj = mObjects[i];
        MFn::Type mType = mObj.apiType();

        if (m_Option.bExportSelectedOnly && IsSelected(mSelList, mObj) == false) {
            continue; // 선택된 것만 Export 해야 하면..
        }
        ...
        } else if ((MFn::kPointLight == mType || MFn::kDirectionalLight == mType || MFn::kAmbientLight == mType ||
                   ...
                   TRUE == m_Option.bExportLight) // 라이트
        {
            ...
            if (TRUE == m_Option.bExportCharacter) {
                bHaveJoint = this->HaveJoint(mMesh);
            }
            if (true == bHaveJoint) // 관절에 연결된 메시면 지나간다..
            {
                ...
                if (mMesh.parentCount() > 0 && mMesh.parent(0).apiType() == MFn::kTransform) {
                    ...
                    if (mTM.childCount() == 1) {
                        bProcessShape = true; // 첫번째 자식일 경우만 처리..
                    } else if (mTM.childCount() > 1 && mTM.child(0) == mMesh.object()) {
                        bProcessShape = true; // 첫번째 자식일 경우만 처리..
                    }
                }
                if (bProcessShape) {
    ...
    if (bSaveOK) {
        ...
        if (m_pScene->CameraCount() <= 0) {
            m_pScene->DefaultCameraAdd(); // 기본 카메라
        }
...
bool CN3E2Wrapper::ProcessIMesh(MFnMesh & mMesh, CN3IMesh * pIMesh) {
    ...
    for (int i = 0; i < nPC; i++) {
        ...
        nVI = mVITmp[0];
        Normals[nVI] += vNTmp;
        iNormalCounts[nVI]++;   // 면의 Normal 값 추가.
...
bool CN3E2Wrapper::ProcessVMesh(MFnMesh & mMesh, CN3VMesh * pVMesh) {
    ...
    for (int i = 0; i < nPC; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            pvDests[i * 3 + j].Set((float)(mPosTmp.x * 0.01), (float)(mPosTmp.y * 0.01), -(float)(mPosTmp.z * 0.01));
        }
    }
...
bool CN3E2Wrapper::ProcessSkin(MFnSkinCluster & mSkin, CN3Skin * pSkin) {
    ...
    if (false == this->ProcessIMesh(mMeshOutput, pSkin)) {
        return false; // Indexed Mesh 처리.
    }
...
*/
    ...
    for (int i = 0; !mGIt.isDone(); mGIt.next(), i++) // 루프를 돌면서 컴포넌트(Geometry 의 한점...)을 처리한다.
    {
        ...
        if (nAffect > 0) {
            ...
            if (nAffect > 1) {
                pVBone[i].pfWeights = new float[nAffect];
            }

            for (int j = 0; j < nAffect; j++) {
                pVBone[i].pnJoints[j] = s_nJoints[j];
                if (nAffect > 1) {
                    pVBone[i].pfWeights[j] = s_fWeights[j];
                }
            }
        }
    }
...
void CN3E2Wrapper::ProcessJointTransform(CN3Joint * pJoint, __Vector3 * pvTrans, __Quaternion * pqRot,
                                         __Vector3 * pvScale, bool bProcessChild) {
    if (NULL == pJoint) {
        return;
    }
    ...
    if (bProcessChild && pvScale) // 하위 노드 처리 ??
    {
        ...
        for (int i = 0; i < nCC; i++) {
            this->ProcessJointTransform((CN3Joint *)pJoint->Child(i), NULL, NULL, pvScale,
                                        true); // 하위 노드들은 스케일만 처리해 준다..
        }
    }
}

void CN3E2Wrapper::ProcessMatrix(MFnTransform & mTransform, __Matrix44 & mtx, __Vector3 & vPos, __Quaternion & qtRot,
                                 __Vector3 & vScale) {
    // 행렬 변환..
...
bool CN3E2Wrapper::HaveJoint(MFnMesh & mMesh) {
    ...
    int          nMC = 0;
    for (int i = 0; i < nCC; i++) {
        if (mTransform.child(i).apiType() == MFn::kMesh) {
            nMC++;
        }
    }
...
int CN3E2Wrapper::ProcessTransform(MFnTransform & mTransform, CN3Transform * pTransform) {
    ...
    if (FALSE == m_Option.bAnimationKey) {
        return -1; // Animation Key 처리 옵션이 켜질 경우만 한다..
    }
    ...
    for (int i = 0; i < nAK; i++) {
        ...
        nKType = -1;
        if (NULL != strstr(szTmp, "translatex")) {
            if (nTranslateCount == 2) {
                nKType = 3;
            } else {
                nKType = 0;
            }
        } else if (NULL != strstr(szTmp, "translatey")) {
            if (nTranslateCount == 2) {
                nKType = 4;
            } else {
                nKType = 1;
            }
        } else if (NULL != strstr(szTmp, "translatez")) {
            if (nTranslateCount == 2) {
                nKType = 5;
            } else {
                nKType = 2;
            }
        } else if (NULL != strstr(szTmp, "rotatex")) {
            nKType = 3;
        } else if (NULL != strstr(szTmp, "rotatey")) {
            nKType = 4;
        } else if (NULL != strstr(szTmp, "rotatez")) {
            nKType = 5;
        } else if (NULL != strstr(szTmp, "scalex")) {
            nKType = 6;
        } else if (NULL != strstr(szTmp, "scaley")) {
            nKType = 7;
        } else if (NULL != strstr(szTmp, "scalez")) {
            nKType = 8;
        } else if (NULL != strstr(szTmp, "jointorientx")) {
            nKType = 9;
        } else if (NULL != strstr(szTmp, "jointorienty")) {
            nKType = 10;
        } else if (NULL != strstr(szTmp, "jointorientz")) {
            nKType = 11;
        } else if (NULL != strstr(szTmp, "visibility")) {
            nKType = -1;
        } else // 이름으로 판단할수 없다면.. ID 로 판단한다...??
        {
            ...
            else if (NULL != strstr(szTmp, "animCurveTA")) {
                nKType = 3 + nR;
                nR++;
            } // Rotate
            else if (NULL != strstr(szTmp, "animCurveTU")) {
                nKType = 6 + nS;
                nS++;
            } // Scale
            else {
        ...
        if (nKType == 0) {
            mACs[0][0].setObject(mAC.object());
        } else if (nKType == 1) {
            mACs[0][1].setObject(mAC.object());
        } else if (nKType == 2) {
            mACs[0][2].setObject(mAC.object());
        } else if (nKType == 3) {
            mACs[1][0].setObject(mAC.object());
        } else if (nKType == 4) {
            mACs[1][1].setObject(mAC.object());
        } else if (nKType == 5) {
            mACs[1][2].setObject(mAC.object());
        } else if (nKType == 6) {
            mACs[2][0].setObject(mAC.object());
        } else if (nKType == 7) {
            mACs[2][1].setObject(mAC.object());
        } else if (nKType == 8) {
            mACs[2][2].setObject(mAC.object());
        }

        // Joint Orient Key 값..
        else if (nKType == 9) {
            mACJointOrients[0].setObject(mAC.object());
        } else if (nKType == 10) {
            mACJointOrients[1].setObject(mAC.object());
        } else if (nKType == 11) {
            mACJointOrients[2].setObject(mAC.object());
        }
        ...
        if (nKs[0] > 0 || nKs[1] > 0 || nKs[2] > 0) {
            if (nKs[0] <= 0 || nKs[1] <= 0 || nKs[2] <= 0) {
                ...
                if (i == 0) {
                    ...
                    {
                        this->ProcessAnimKey(mACs[i], &pTransform->m_KeyRot, true, 0.01f,
                                             false); // ViewTarget Position Key 를 처리한다..
                    } else {
                        MTransformationMatrix::RotationOrder rotOrder =
                            mTransform.rotationOrder(); // 회전 순서에 따라 처리가 달라져야 한다..
                    ...
                    this->ProcessAnimKey(mACs[i], &pTransform->m_KeyScale, false, 1.0f, false,
                                         MTransformationMatrix::kXYZ); // Scale Animation Key 를 처리한다..
                }                                                      // end of if(i == 0)
...
void CN3E2Wrapper::ProcessAnimKey(MFnAnimCurve * pmACs, CN3AnimKey * pKey, bool bReverseZ, float fScale,
                                  bool bQuaternion, MTransformationMatrix::RotationOrder mRotOrder) {
    // 가장 긴것을 찾는다..
    ...
    if (bQuaternion) {
        pKey->Alloc(nFrmMax, m_Option.fSamplingRate, KEY_QUATERNION); // 로테이션이면 쿼터니언으로 할당.
    } else {
        pKey->Alloc(nFrmMax, m_Option.fSamplingRate, KEY_VECTOR3); // 벡터로 할당..
    }

    MTime mTime;
    mTime.setUnit(MTime::kNTSCFrame);
    if (bQuaternion) {
        double         dRs[4];
        __Quaternion * pQt;
        //        MMatrix mMtx;
        ...
        // 회전순서
        if (MTransformationMatrix::kXYZ == mRotOrder) {
            nRotSeqs[0] = 0;
            nRotSeqs[1] = 1;
            nRotSeqs[2] = 2;
        } else if (MTransformationMatrix::kYZX == mRotOrder) {
            nRotSeqs[0] = 1;
            nRotSeqs[1] = 2;
            nRotSeqs[2] = 0;
        } else if (MTransformationMatrix::kZXY == mRotOrder) {
            nRotSeqs[0] = 2;
            nRotSeqs[1] = 0;
            nRotSeqs[2] = 1;
        } else if (MTransformationMatrix::kXZY == mRotOrder) {
            nRotSeqs[0] = 0;
            nRotSeqs[1] = 2;
            nRotSeqs[2] = 1;
        } else if (MTransformationMatrix::kYXZ == mRotOrder) {
            nRotSeqs[0] = 1;
            nRotSeqs[1] = 0;
            nRotSeqs[2] = 2;
        } else if (MTransformationMatrix::kZYX == mRotOrder) {
            nRotSeqs[0] = 2;
            nRotSeqs[1] = 1;
            nRotSeqs[2] = 0;
        } else {
            MessageBox(::GetActiveWindow(), "Not supported rotation order", "Warning - animation key", MB_OK);
        }
        ...
        for (int i = 0; i < nFrmMax; i++) {
            ...
            pmACs[0].evaluate(mTime, dfValue);
            pV->x = (float)dfValue;
            pmACs[1].evaluate(mTime, dfValue);
            pV->y = (float)dfValue;
            pmACs[2].evaluate(mTime, dfValue);
            pV->z = (float)dfValue;

            if (1.0f != fScale) {
                *pV *= fScale; // 단위가 meter 라서 그렇다..
            }
            if (TRUE == bReverseZ) {
                pV->z = -(pV->z); // Z 축을 음수로 한다..
            }
        }
    }
}
...
bool CN3E2Wrapper::ProcessChr(MFnSkinCluster & mSkin) {
    ...
    if (i == nJC) {
        return false; // Root Joint 가 없을 경우..
    }
    ...
    if (mJointRoot.parentCount() > 0 && mJointRoot.parent(0).hasFn(MFn::kTransform)) {
        ...
        szJointFN = fs::path("Chr") / (szJointFileName + ".n3joint");
    } else {
        szJointFN = mJointRoot.name().asUTF8(); // 뼈의 이름을 알아보고..
    }
    ...
    if (false == bOverlapped) // 중복되지 않는다면..새로 만든다..
    {
        ...
        pJoint->FilePathSet(szJointFN); // 파일 이름 설정..
...
*/
    } else if (false == bCollisionMesh) // 충돌 체크 메시 아니면....정상적으로 진행..
    {
        ...
        if (mGT.parentCount() > 0 && mGT.parent(0).hasFn(MFn::kTransform)) // 그룹 트랜스폼의 상위 노드가 있을 경우..
        {
            ...
            for (int i = 0; i < nC; i++) {
                if (!mCT.child(i).hasFn(MFn::kTransform)) {
                    continue;
                }

                MFnTransform mTTmp(mCT.child(i));
                if (mTTmp.object() == mGT.object()) {
                    break; // 캐릭터 트랜스폼 밑의 차일드의 이름과 그룹의 이름이 같을 경우..
                }
            ...
            for (int i = 0; i < nGTC; i++) {
                if (!mGT.child(i).hasFn(MFn::kTransform)) {
                    continue;
                }

                MFnTransform mTTmp(mGT.child(i));
                if (mTTmp.name() == mMT.name()) {
                    break; // Mesh Transform 과 Mesh 그룹의 차일드 메시가 이름이 같으면..
                }
                nLOD++;
            }
        } else // pair of if(mGT.parentCount() > 0 && mGT.parent(0).apiType() == MFn::kTransform) // 그룹 트랜스폼의 상위 노드가 있을 경우..
        {
            ...
            for (int i = 0; i < nC; i++) {
                if (mGT.child(i).apiType() != MFn::kTransform) {
                    continue;
                }
...
void CN3E2Wrapper::ProcessName(MObject mObj, std::string & szName) {
    if (mObj.hasFn(MFn::kWorld) == FALSE && mObj.hasFn(MFn::kDependencyNode)) // 최상위 노드가 아니면..
    {
        ...
        if (mObj.hasFn(MFn::kDagNode)) {
            ...
            if (mDAG.parentCount() > 0) {
                this->ProcessName(mDAG.parent(0), szName);
            }
        }
    }
}
