...
void CPVSManager::UpdatePosAll(float fx, float fy, float fz) {
    ...
    while (it != m_pPvsList.end()) {
        pVol = *it++;
        pVol->UpdatePosAll(pVol->Pos().x + fx - m_vBackupTotalMove.x, pVol->Pos().y + fy - m_vBackupTotalMove.y,
                           pVol->Pos().z + fz - m_vBackupTotalMove.z);
    }
    ...
    while (siit != m_plShapeInfoList.end()) {
        pSI = *siit++;
        pSI->PosSet(pSI->Pos().x + fx - m_vBackupTotalMove.x, pSI->Pos().y + fy - m_vBackupTotalMove.y,
                    pSI->Pos().z + fz - m_vBackupTotalMove.z);
    }
