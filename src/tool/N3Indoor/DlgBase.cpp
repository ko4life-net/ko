...
void CDlgBase::DoDataExchange(CDataExchange * pDX) {
    ...
    DDX_Control(pDX, IDC_CB_PART_CHARACTE, m_CBChrPart);
...
ON_CBN_SELCHANGE(IDC_CB_PART_CHARACTE, OnSelchangeCbPartCharacte)
...
void CDlgBase::UpdateInfo() {
    ...
    if (pBase->Type() & OBJ_TRANSFORM) {
        ...
        if (pItem) {
            pItem->VectorSet(pSI->Pos());
        }
        pItem = m_LPTransform.GetPropItem("Scale");
        if (pItem) {
            pItem->VectorSet(pSI->Scale());
        }
    ...
    if (pBase->Type() & OBJ_SHAPE) {
        ...
        if (pItem) {
            pItem->m_curValue.Format("%d", pSI->m_iEventID);
        }
        pItem = m_LPShape.GetPropItem("Event Type");
        if (pItem) {
            pItem->m_curValue.Format("%d", pSI->m_iEventType);
        }
        pItem = m_LPShape.GetPropItem("NPC ID");
        if (pItem) {
            pItem->m_curValue.Format("%d", pSI->m_iNPC_ID);
        }
    ...
    if (pBase != NULL) {
        ...
        if ((void *)wParam == &m_LPShape && (pBase->Type() & OBJ_SHAPE)) {
            ...
            if (pItem->m_propName == "소속" && pItem->m_curValue.GetLength() > 0) {
                ...
                pSI->m_iEventType = atoi(pItem->m_curValue);
...
void CDlgBase::OnSelchangeCbPartCharacte() {
    // TODO: Add your control notification handler code here
    this->UpdateInfo();
}
