...
void CPvsMgr::DeletePvsLinkedObjByiID(int iID) {
    ...
    while (itBase != m_pPvsList.end()) {
        pvsBase = *itBase++;
        if (!pvsBase->IsKindOf(RUNTIME_CLASS(CPortalVol))) {
            continue;
        }
...
void CPvsMgr::TotalShapeRender() {
    ...
    if (pSh) {
        ...
        pSh->ScaleSet(vec);
        __Quaternion qt;
        D3DXQuaternionRotationMatrix(&qt, &m_MtxShapeRot);
    ...
    while (it != pVolMy->m_pPvsList.end()) {
        ...
        if (pBase->IsKindOf(RUNTIME_CLASS(CPortalVol))) {
            ...
            if ((pVol->m_iPriority == -1) ||
                (pVol->m_iPriority > iRecursive)) // 우선순위가 정해지지 않았거나 현재 우선순위보다 크면..
            {
...
#define SMALL_OFFSET 0.6f;
    ...
    case WALL_ZB:
        for (int i = 0; i < 4; i++) {
            Col.Wvec[i] = pWall->m_pvVertex[i];
        }
        Col.Wvec[i].z = pWall->m_pvVertex[i].z + SMALL_OFFSET;
        break;
    ...
    case WALL_XL:
        ...
        Col.Wvec[i].x = pWall->m_pvVertex[i].x + SMALL_OFFSET;
        break;
    ...
    case WALL_YT:
        ...
        Col.Wvec[i].y = pWall->m_pvVertex[i].y - SMALL_OFFSET;
        break;
...
void CPvsMgr::PrepareCollisionDetectTwo(CPortalVol * pVol, CPortalWall * pWall, e_WallType eWT, __Collision & Col) {
    ...
    case WALL_ZB: // 뒤쪽..
        ...
        Col.Vvec[5].z = pVol->m_pvVertex[3].z;
        Col.Vvec[6] = pWall->m_pvVertex[2];
        Col.Vvec[6].z = pVol->m_pvVertex[6].z;
        Col.Vvec[7] = pWall->m_pvVertex[3];
    ...
    case WALL_ZF: // 앞쪽..
        ...
        Col.Vvec[5] = pWall->m_pvVertex[1];
        Col.Vvec[5].z = pVol->m_pvVertex[1].z;
        Col.Vvec[6] = pWall->m_pvVertex[2];
        Col.Vvec[6].z = pVol->m_pvVertex[4].z;
        Col.Vvec[7] = pWall->m_pvVertex[3];
    ...
    case WALL_XL: // 왼쪽..
        ...
        Col.Vvec[5] = pWall->m_pvVertex[1];
        Col.Vvec[5].x = pVol->m_pvVertex[2].x;
        Col.Vvec[6] = pWall->m_pvVertex[2];
        Col.Vvec[6].x = pVol->m_pvVertex[5].x;
        Col.Vvec[7] = pWall->m_pvVertex[3];
    ...
    case WALL_XR: // 오른쪽..
        ...
        Col.Vvec[5] = pWall->m_pvVertex[1];
        Col.Vvec[5].x = pVol->m_pvVertex[0].x;
        Col.Vvec[6] = pWall->m_pvVertex[2];
        Col.Vvec[6].x = pVol->m_pvVertex[7].x;
        Col.Vvec[7] = pWall->m_pvVertex[3];
    ...
    case WALL_YT: // 위쪽
        ...
        Col.Vvec[5] = pWall->m_pvVertex[1];
        Col.Vvec[5].y = pVol->m_pvVertex[1].y;
        Col.Vvec[6] = pWall->m_pvVertex[2];
        Col.Vvec[6].y = pVol->m_pvVertex[3].y;
        Col.Vvec[7] = pWall->m_pvVertex[3];
    ...
    case WALL_YB: // 아래쪽
        ...
        Col.Vvec[5] = pWall->m_pvVertex[1];
        Col.Vvec[5].y = pVol->m_pvVertex[6].y;
        Col.Vvec[6] = pWall->m_pvVertex[2];
        Col.Vvec[6].y = pVol->m_pvVertex[4].y;
        Col.Vvec[7] = pWall->m_pvVertex[3];
        Col.Vvec[7].y = pVol->m_pvVertex[5].y;
        break;
    }
...
bool CPvsMgr::CheckPvsVolumnWall(const __Vector3 & vOrig, const __Vector3 & vDir, CPortalVol * pVolMy, e_WallType eWT) {
    ...
    if (IntersectTriangle(vOrig, vDir, pVolMy->m_pvVertex[pVolMy->m_pIndex[i + 3]],
                          pVolMy->m_pvVertex[pVolMy->m_pIndex[i + 4]], pVolMy->m_pvVertex[pVolMy->m_pIndex[i + 5]], ft,
                          fu, fv, NULL)) {
        return true;
    }
...
bool CPvsMgr::IntersectTriangle(const __Vector3 & vOrig, const __Vector3 & vDir, const __Vector3 & v0,
