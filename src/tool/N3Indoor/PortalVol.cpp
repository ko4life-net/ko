...
bool CPortalVol::IsInVolumnExExEx(__Vector3 vOrig, __Vector3 vDir, e_WallType eWT, __Vector3 & vPick) {
    ...
    if (IntersectTriangle(vOrig, vDir, m_pvVertex[m_pIndex[i]], m_pvVertex[m_pIndex[i + 1]],
                          m_pvVertex[m_pIndex[i + 2]], ft, fu, fv, &vPick)) {
        return true;
    }
    if (IntersectTriangle(vOrig, vDir, m_pvVertex[m_pIndex[i + 3]], m_pvVertex[m_pIndex[i + 4]],
                          m_pvVertex[m_pIndex[i + 5]], ft, fu, fv, &vPick)) {
        return true;
    }
...
bool CPortalVol::IntersectTriangle(const __Vector3 & vOrig, const __Vector3 & vDir, const __Vector3 & v0,
