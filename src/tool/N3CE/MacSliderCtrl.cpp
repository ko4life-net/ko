    ...
    if (lpcd->dwDrawStage == CDDS_ITEMPREPAINT) {
        ...
        if (lpcd->dwItemSpec == TBCD_TICS) {
            *pResult = CDRF_DODEFAULT;
            ...
            CRect rcTmp;
            this->CalcRectToFill(rect, rcTmp, m_AnimData.fFrmPlugTraceStart, m_AnimData.fFrmPlugTraceEnd);
...
//
{
    ...
    m_crShadow = RGB(byRedFace + ((byRedShadow - byRedFace) >> 1), byGreenFace + ((byGreenShadow - byGreenFace) >> 1),
