...
void CDlgChrProperty::UpdateInfo() {
    ...
    if (nPart >= 0 && nPart < nPartCount) {
        ...
        if (pItem) {
            pItem->m_curValue = pPart->FilePath().c_str();
        }

        pItem = m_LPCPart.GetPropItem("Part Type");
        if (pItem) {
            pItem->m_crColor = pPart->m_Type;
        }
    ...
    if (nPlug >= 0 && nPlug < nPlugCount) {
        ...
        if (pItem) {
            pItem->m_curValue = pPlug->FilePath().c_str();
        }

        pItem = m_LPCPlug.GetPropItem("Plug Type");
        if (pItem) {
            pItem->m_crColor = pPlug->m_Type;
        }
...
BOOL CDlgChrProperty::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT * pResult) {
    ...
    if ((void *)wParam == &m_LPChr) {
        ...
        if (pItem->m_propName == "Joint File" && pItem->m_curValue.GetLength() > 0) {
            pChr->JointSet(CN3BaseFileAccess::ToRelative(pItem->m_curValue.GetString()));
            this->UpdateInfo();
            //} else if (pItem->m_propName == "Collision Mesh File" && pItem->m_curValue.GetLength() > 0) {
            //    pChr->CollisionMeshSet(CN3BaseFileAccess::ToRelative(pItem->m_curValue.GetString()));
            //    this->UpdateInfo();
            ...
            //    pChr->CollisionSkinSet(CN3BaseFileAccess::ToRelative(pItem->m_curValue.GetString()));
            //    this->UpdateInfo();
    ...
    if ((void *)wParam == &m_LPCPart) {
        ...
        if (nPart >= 0 && nPart < nPartCount) {
            ...
            if (pItem->m_propName == "Part File") {
                pPart->NameSet(pItem->m_curValue);
            } else if (pItem->m_propName == "Part Type") {
                CN3CPart * pPD = pChr->Part(nPart);
                if (pPD) {
                    if (pItem->m_curValue == "머리카락") {
                        pPD->m_Type = PART_HAIR;
                    } else if (pItem->m_curValue == "얼굴") {
                        pPD->m_Type = PART_FACE;
                    } else if (pItem->m_curValue == "상체") {
                        pPD->m_Type = PART_UPPER;
                    } else if (pItem->m_curValue == "하체") {
                        pPD->m_Type = PART_LOWER;
                    } else if (pItem->m_curValue == "손") {
                        pPD->m_Type = PART_HAND;
                    } else if (pItem->m_curValue == "발") {
                        pPD->m_Type = PART_FOOT;
                    } else if (pItem->m_curValue == "??") {
                        pPD->m_Type = PART_UNKNOWN;
                    }
                }
                this->UpdateInfo();
    ...
    if ((void *)wParam == &m_LPCPlug) {
        ...
        if (nPlug >= 0 && nPlug < nPlugCount) {
            ...
            if (pItem->m_propName == "Plug Name") {
                pPlug->NameSet(pItem->m_curValue);
            } else if (pItem->m_propName == "Plug Type") {
                pPlug->m_Type = (e_PlugType)pItem->m_crColor;
