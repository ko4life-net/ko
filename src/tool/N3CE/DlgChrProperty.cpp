...
void CDlgChrProperty::UpdateInfo() {
    ...
    for (int i = 0; i < nPartCount; i++) {
        CString szTmp;
        if (pChr->Part(i)) {
            szTmp.Format("Part(%d) : %s", i, pChr->Part(i)->Name());
        } else {
    ...
    if (nPlug >= 0 && nPlug < nPlugCount) {
        ...
        if (pItem) {
            if (pPlug->PMesh()) {
                pItem->m_curValue = pPlug->PMesh()->Name();
            } else {
                pItem->m_curValue = "";
            }
        }
        ...
        if (pItem) {
            if (pPlug->Tex()) {
                pItem->m_curValue = pPlug->Tex()->Name();
            } else {
                pItem->m_curValue = "";
            }
        }
