...
void CViewProperty::UpdateInfo() {
    if (m_LPTransform.GetSafeHwnd() == NULL) {
        return;
    }
    ...
    if (pBase->Type() & OBJ_CAMERA) {
        ...
        if (pItem) {
            pItem->VectorSet(pC->EyePos());
        }
        pItem = m_LPCamera.GetPropItem("At");
        if (pItem) {
            pItem->VectorSet(pC->AtPos());
        }
        pItem = m_LPCamera.GetPropItem("Up");
        if (pItem) {
            pItem->VectorSet(pC->UpVector());
        }
        pItem = m_LPCamera.GetPropItem("Field Of View");
        if (pItem) {
            pItem->m_curValue.Format("%f", pC->m_Data.fFOV);
        }
        pItem = m_LPCamera.GetPropItem("Near Plane");
        if (pItem) {
            pItem->m_curValue.Format("%f", pC->m_Data.fNP);
        }
    ...
    if (pBase->Type() & OBJ_LIGHT) {
        ...
        if (pItem) {
            pItem->m_curValue.Format("%d", pL->m_Data.nNumber);
        }
        pItem = m_LPLight.GetPropItem("Type");
        if (pItem) {
            pItem->m_crColor = pL->m_Data.Type;
        }
        pItem = m_LPLight.GetPropItem("Direction");
        if (pItem) {
            pItem->VectorSet(pL->m_Data.Direction);
        }
        pItem = m_LPLight.GetPropItem("Ambient Color");
        if (pItem) {
            pItem->D3DColorValueSet(pL->m_Data.Ambient);
        }
        pItem = m_LPLight.GetPropItem("Diffuse Color");
        if (pItem) {
            pItem->D3DColorValueSet(pL->m_Data.Diffuse);
        }
        pItem = m_LPLight.GetPropItem("Specular Color");
        if (pItem) {
            pItem->D3DColorValueSet(pL->m_Data.Specular);
        }
        pItem = m_LPLight.GetPropItem("Range");
        if (pItem) {
            pItem->m_curValue.Format("%f", pL->m_Data.Range);
        }
        pItem = m_LPLight.GetPropItem("Attenuation0");
        if (pItem) {
            pItem->m_curValue.Format("%f", pL->m_Data.Attenuation0);
        }
        pItem = m_LPLight.GetPropItem("Attenuation1");
        if (pItem) {
            pItem->m_curValue.Format("%f", pL->m_Data.Attenuation1);
        }
        pItem = m_LPLight.GetPropItem("Attenuation2");
        if (pItem) {
            pItem->m_curValue.Format("%f", pL->m_Data.Attenuation2);
        }
    ...
    if (pBase->Type() & OBJ_SHAPE) {
        ...
        pItem = m_LPShape.GetPropItem("Event ID");
        if (pItem) {
            pItem->m_curValue.Format("%d", pS->m_iEventID);
        }
        pItem = m_LPShape.GetPropItem("Event Type");
        if (pItem) {
            pItem->m_curValue.Format("%d", pS->m_iEventType);
        }
        pItem = m_LPShape.GetPropItem("NPC ID");
        if (pItem) {
            pItem->m_curValue.Format("%d", pS->m_iNPC_ID);
        }
        ...
        for (int i = 0; i < nPartCount; i++) {
            ...
            if (pPMesh) {
                szTmp.Format("Part%2d : %s", i, pPMesh->m_szName.c_str());
            }
        ...
        if (nPart >= 0 && nPart < nPartCount) {
            ...
            if (pItem) {
                if (pPMesh) {
                    pItem->m_curValue.Format("%4d, %4d", pPMesh->GetMaxNumIndices() / 3, pPMesh->GetMinNumVertices());
                } else {
                    pItem->m_curValue = "";
                }
            }
            ...
            if (pItem) {
                ...
                if (pMC) {
                    pItem->m_curValue = pMC->m_szName.c_str();
                } else {
                    pItem->m_curValue = "";
                }
            }
    ...
    if (pBase->Type() & OBJ_CHARACTER) {
        ...
        if (pItem) {
            ...
            if (pJoint) {
                pItem->m_curValue = pJoint->m_szName.c_str();
            } else {
                pItem->m_curValue = "";
            }
        }
        ...
        //    if (pSkin) {
        //        pItem->m_curValue = pSkin->m_szName.c_str();
        //    } else {
        //        pItem->m_curValue = "";
        //    }
        //}
        ...
        if (pPart) {
            ...
            if (pItem) {
                pItem->m_curValue = pPart->m_szName.c_str();
            }
        ...
        if (nPlug >= 0 && nPlug < nPlugCount) {
            ...
            if (pItem) {
                pItem->m_curValue = pPlug->m_szName.c_str();
            }
            ...
            if (pItem) {
                pItem->m_curValue.Format("%d", pPlug->m_nJointIndex);
            }

            pItem = m_LPCPlug.GetPropItem("Plug Offset");
            if (pItem) {
                pItem->VectorSet(pPlug->Position());
            }

            pItem = m_LPCPlug.GetPropItem("Plug Scale");
            if (pItem) {
                pItem->VectorSet(pPlug->Scale());
            }

            pItem = m_LPCPlug.GetPropItem("Plug Mesh File");
            if (pItem) {
                if (pPlug->PMesh()) {
                    pItem->m_curValue = pPlug->PMesh()->m_szName.c_str();
                } else {
                    pItem->m_curValue = "";
                }
            }
            ...
            if (pItem) {
                if (pPlug->Tex()) {
                    pItem->m_curValue = pPlug->Tex()->m_szName.c_str();
                } else {
                    pItem->m_curValue = "";
                }
            }
...
BOOL CViewProperty::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT * pResult) {
    ...
    if (pBase != NULL) {
        ...
        if ((void *)wParam == &m_LPCamera && (pBase->Type() & OBJ_CAMERA)) {
            ...
            CPropertyItem * pItem = (CPropertyItem *)lParam;
            if (pItem->m_propName == "Eye") {
                pC->EyePosSet(__Vector3(pItem->VectorGet()));
            } else if (pItem->m_propName == "At") {
                pC->AtPosSet(__Vector3(pItem->VectorGet()));
            } else if (pItem->m_propName == "Up") {
                pC->UpVectorSet(__Vector3(pItem->VectorGet()));
            } else if (pItem->m_propName == "Field Of View") {
                pC->m_Data.fFOV = (float)atof(pItem->m_curValue);
            } else if (pItem->m_propName == "Near Plane") {
        ...
        if ((void *)wParam == &m_LPLight && (pBase->Type() & OBJ_LIGHT)) {
            ...
            if (pItem->m_propName == "On") {
                if (pItem->m_curValue != "0" && lstrcmpi(pItem->m_curValue, "off")) {
                    pL->m_Data.bOn = TRUE;
                } else {
                    pL->m_Data.bOn = FALSE;
                }
            }
            if (pItem->m_propName == "Number") {
                pL->m_Data.nNumber = atoi(pItem->m_curValue);
            }
            if (pItem->m_propName == "Type") {
                if (pItem->m_curValue == "Point") {
                    pL->m_Data.Type = D3DLIGHT_POINT;
                } else if (pItem->m_curValue == "Directional") {
                    pL->m_Data.Type = D3DLIGHT_DIRECTIONAL;
                } else if (pItem->m_curValue == "Spot") {
                    pL->m_Data.Type = D3DLIGHT_SPOT;
                }
            } else if (pItem->m_propName == "Direction") {
                pL->m_Data.Direction = pItem->VectorGet();
            } else if (pItem->m_propName == "Ambient Color") {
                pL->m_Data.Ambient = pItem->D3DColorValueGet();
            } else if (pItem->m_propName == "Diffuse Color") {
                pL->m_Data.Diffuse = pItem->D3DColorValueGet();
            } else if (pItem->m_propName == "Specular Color") {
                pL->m_Data.Specular = pItem->D3DColorValueGet();
            } else if (pItem->m_propName == "Range") {
                pL->m_Data.Range = (float)atof(pItem->m_curValue);
            } else if (pItem->m_propName == "Attenuation0") {
                pL->m_Data.Attenuation0 = (float)atof(pItem->m_curValue);
            } else if (pItem->m_propName == "Attenuation1") {
                pL->m_Data.Attenuation1 = (float)atof(pItem->m_curValue);
            } else if (pItem->m_propName == "Attenuation2") {
                pL->m_Data.Attenuation2 = (float)atof(pItem->m_curValue);
            }
        }
        ...
        if ((void *)wParam == &m_LPShape && (pBase->Type() & OBJ_SHAPE)) {
            ...
            if (pItem->m_propName == "소속" && pItem->m_curValue.GetLength() > 0) {
            ...
            } else if (pItem->m_propName == "Event Type" && pItem->m_curValue.GetLength() > 0) {
                pS->m_iEventType = atoi(pItem->m_curValue);
                ...
                if (pPD->Mesh()) {
                    pPD->Mesh()->ReGenerateSmoothNormal();
                }
            }

            else if (pItem->m_propName == "Make Sharp") {
                if (pPD->Mesh()) {
                    pPD->Mesh()->ReGenerateSharpNormal();
                }
            }
        }
        ...
        if ((void *)wParam == &m_LPChr && (pBase->Type() & OBJ_CHARACTER)) {
            ...
            if (pItem->m_propName == "Joint File" && pItem->m_curValue.GetLength() > 0) {
                ...
                //    pC->CollisionSkinSet(CN3BaseFileAccess::ToRelative(pItem->m_curValue.GetString()));
                //    this->UpdateInfo();
        ...
        if ((void *)wParam == &m_LPCPart && (pBase->Type() & OBJ_CHARACTER)) {
            ...
            if (pPart) {
                if (pItem->m_propName == "Part File") {
                    pPart->m_szName = pItem->m_curValue;
                }
        ...
        if ((void *)wParam == &m_LPCPlug && (pBase->Type() & OBJ_CHARACTER)) {
            ...
            if (nPlug >= 0 && nPlug < nPlugCount) {
                ...
                if (pItem->m_propName == "Plug Name") {
                    pPlug->m_szName = pItem->m_curValue;
                }
                //                else if(pItem->m_propName == "Plug Type") pPlug->m_Type = (e_PlugType)pItem->m_crColor;
                    ...
                    if (pJ) {
                        pJ->NodeCount(nJC); // Joint Node Count;
                    }
                    if (nJI >= 0 && nJI < nJC) {
                        pPlug->m_nJointIndex = nJI;
                    }
                } else if (pItem->m_propName == "Plug Offset") {
                    pPlug->PositionSet(__Vector3(pItem->VectorGet()));
                } else if (pItem->m_propName == "Plug Rotation") {
                    __Matrix44 mtx;
                    mtx.Identity();
        ...
        if ((void *)wParam == &m_LPMaterial) {
            ...
            if (pMtl) {
                ...
                if (pItem->m_propName == "Src Blend") {
                    pMtl->dwSrcBlend = pItem->m_crColor;
                }
                if (pItem->m_propName == "Dest Blend") {
                    pMtl->dwDestBlend = pItem->m_crColor;
                }
                if (pItem->m_propName == "Color Operation") {
                    pMtl->dwColorOp = pItem->m_crColor;
                }
                if (pItem->m_propName == "Color Arg1") {
                    pMtl->dwColorArg1 = pItem->m_crColor;
                }
                if (pItem->m_propName == "Color Arg2") {
                    pMtl->dwColorArg2 = pItem->m_crColor;
                }
                if (pItem->m_propName == "Ambient Color") {
                    pMtl->Ambient = pItem->D3DColorValueGet();
                }
                if (pItem->m_propName == "Diffuse Color") {
                    pMtl->Diffuse = pItem->D3DColorValueGet();
                }
                if (pItem->m_propName == "Specular Color") {
                    pMtl->Specular = pItem->D3DColorValueGet();
                }
                if (pItem->m_propName == "Emissive Color") {
                    pMtl->Emissive = pItem->D3DColorValueGet();
                }
            }
        }
    }
...
void CViewProperty::OnInitialUpdate() {
    ...
    D3DBLEND_INVDESTALPHA|D3DBLEND_DESTCOLOR|D3DBLEND_INVDESTCOLOR|D3DBLEND_SRCALPHASAT|D3DBLEND_BOTHSRCALPHA|D3DBLEND_BOTHINVSRCALPHA|";
        ...
        strTmp =
            "Null|D3DTOP_DISABLE|D3DTOP_SELECTARG1|D3DTOP_SELECTARG2|D3DTOP_MODULATE|D3DTOP_MODULATE2X|D3DTOP_MODULATE4X|\
    ...
    D3DTOP_BUMPENVMAP|D3DTOP_BUMPENVMAPLUMINANCE|D3DTOP_DOTPRODUCT|D3DTOP_MULTIPLYADD|D3DTOP_LERP|";
        ...
        for (int i = 0; i < MAX_CHR_LOD; i++) {
            strTmp.Format("LOD : %d", i);
            m_CBChrLOD.AddString(strTmp);
        }
