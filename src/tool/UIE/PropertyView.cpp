...
void CPropertyView::OnInitialUpdate() {
    ...
    if (FALSE == bInitCtrl) {
        ...
        m_UIImage.AddPropItem("Animate Frame", "", PIT_EDIT,
                              ""); // Animate frame 초당 몇 프레임으로 에니메이트 될 것인가?
...
BOOL CPropertyView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT * pResult) {
    ...
    if ((void *)wParam == (&m_UIString) && UI_TYPE_STRING == pUI->UIType()) {
        ...
        if (pItem->m_propName == "Line") {
            ...
            if (0 == pItem->m_crColor) {
                dwStyle |= UISTYLE_STRING_SINGLELINE;
            } else if (1 == pItem->m_crColor) {
                dwStyle &= (~UISTYLE_STRING_SINGLELINE);
            }
        ...
        } else if (pItem->m_propName == "Text") {
            pString->SetString((LPCTSTR)pItem->m_curValue);
        }
    }
    // List
    ...
    if ((void *)wParam == (&m_UIEdit) && UI_TYPE_EDIT == pUI->UIType()) {
        ...
        if (pItem->m_propName == "Style") {
            ...
            if (1 == pItem->m_crColor) {
                dwStyle |= UISTYLE_EDIT_PASSWORD;
            }
            if (2 == pItem->m_crColor) {
                dwStyle |= UISTYLE_EDIT_NUMBERONLY;
            }
    ...
    if ((void *)wParam == (&m_UIProgress) && UI_TYPE_PROGRESS == pUI->UIType()) {
        ...
        if (pItem->m_propName == "Style") {
            ...
            if (0 == pItem->m_crColor) {
                ...
                dwStyle |= UISTYLE_PROGRESS_TOP2BOTTOM;
            } else if (3 == pItem->m_crColor) {
                dwStyle |= UISTYLE_PROGRESS_BOTTOM2TOP;
            }
            pProg->SetStyle(dwStyle);
        } else if (pItem->m_propName == "MaxValue") {
            pProg->SetRange(pProg->GetMin(), atoi(pItem->m_curValue));
        } else if (pItem->m_propName == "MinValue") {
            pProg->SetRange(atoi(pItem->m_curValue), pProg->GetMax());
        } else if (pItem->m_propName == "CurValue") {
            pProg->SetCurValue(atoi(pItem->m_curValue));
    ...
    if ((void *)wParam == (&m_UITrackBar) && UI_TYPE_TRACKBAR == pUI->UIType()) {
        ...
        } else if (pItem->m_propName == "Max pos") {
            pTB->SetRangeMax(atoi(pItem->m_curValue));
        } else if (pItem->m_propName == "Min pos") {
            pTB->SetRangeMin(atoi(pItem->m_curValue));
        } else if (pItem->m_propName == "Cur pos") {
            pTB->SetCurrentPos(atoi(pItem->m_curValue));
        } else if (pItem->m_propName == "Page size") {
            pTB->SetPageSize(atoi(pItem->m_curValue));
        } else if (pItem->m_propName == "Delete bkgnd image") {
            pTB->DeleteBkImage();
            GetDocument()->UpdateAllViews(NULL);
        }
    ...
    if ((void *)wParam == (&m_UIScrollBar) && UI_TYPE_SCROLLBAR == pUI->UIType()) {
        ...
        } else if (pItem->m_propName == "Line size") {
            pSB->SetLineSize(atoi(pItem->m_curValue));
        }
    }
    // Icon slot
...
void CPropertyView::Resize() {
    ...
    if (m_UIBase.IsWindowVisible()) {
        SetPropertyListPos(&m_UIBase, ptWndPos); // Property list window 위치조정
    }
    if (m_UIImage.IsWindowVisible()) {
        SetPropertyListPos(&m_UIImage, ptWndPos); // Property list window 위치조정
    }
    if (m_UIString.IsWindowVisible()) {
        SetPropertyListPos(&m_UIString, ptWndPos); // Property list window 위치조정
    }
    if (m_UIButton.IsWindowVisible()) {
        SetPropertyListPos(&m_UIButton, ptWndPos); // Property list window 위치조정
    }
    if (m_UIStatic.IsWindowVisible()) {
        SetPropertyListPos(&m_UIStatic, ptWndPos); // Property list window 위치조정
    }
    if (m_UIEdit.IsWindowVisible()) {
        SetPropertyListPos(&m_UIEdit, ptWndPos); // Property list window 위치조정
    }
    if (m_UIProgress.IsWindowVisible()) {
        SetPropertyListPos(&m_UIProgress, ptWndPos); // Property list window 위치조정
    }
    if (m_UITrackBar.IsWindowVisible()) {
        SetPropertyListPos(&m_UITrackBar, ptWndPos); // Property list window 위치조정
    }
    if (m_UIScrollBar.IsWindowVisible()) {
        SetPropertyListPos(&m_UIScrollBar, ptWndPos); // Property list window 위치조정
    }
    if (m_UIArea.IsWindowVisible()) {
        SetPropertyListPos(&m_UIArea, ptWndPos); // Property list window 위치조정
    }
    if (m_UIIconSlot.IsWindowVisible()) {
        SetPropertyListPos(&m_UIIconSlot, ptWndPos);
    }
    if (m_UIList.IsWindowVisible()) {
        SetPropertyListPos(&m_UIList, ptWndPos);
    }
}
...
void CPropertyView::UpdateUIBaseInfo() {
    ...
    if (pItem) {
        pItem->m_curValue = pUIBase->m_szID.c_str();
    }

    pItem = m_UIBase.GetPropItem("Region left"); // region
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcRegion.left);
    }
    pItem = m_UIBase.GetPropItem("Region top");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcRegion.top);
    }
    pItem = m_UIBase.GetPropItem("Region right");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcRegion.right);
    }
    pItem = m_UIBase.GetPropItem("Region bottom");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcRegion.bottom);
    }

    pItem = m_UIBase.GetPropItem("MoveRect left"); // region
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcMovable.left);
    }
    pItem = m_UIBase.GetPropItem("MoveRect top");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcMovable.top);
    }
    pItem = m_UIBase.GetPropItem("MoveRect right");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcMovable.right);
    }
    pItem = m_UIBase.GetPropItem("MoveRect bottom");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->m_rcMovable.bottom);
    }

    pItem = m_UIBase.GetPropItem("Tooltip text"); // tooltip text
    if (pItem) {
        pItem->m_curValue = pUIBase->m_szToolTip.c_str();
    }
    ...
    if (pItem) {
        if (pUIBase->IsVisible()) {
            pItem->m_curValue = _T("Show");
        } else {
            pItem->m_curValue = _T("Hide");
        }
    }
...
void CPropertyView::UpdateUIImageInfo() {
    ...
    CN3UIImage * pUI = NULL;
    if (UI_TYPE_IMAGE == pUIBase->UIType()) {
        pUI = (CN3UIImage *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        pItem->m_curValue.Format("%f", pUI->m_frcUVRect.top);
    }
    pItem = m_UIImage.GetPropItem("UV right");
    if (pItem) {
        pItem->m_curValue.Format("%f", pUI->m_frcUVRect.right);
    }
    pItem = m_UIImage.GetPropItem("UV bottom");
    if (pItem) {
        pItem->m_curValue.Format("%f", pUI->m_frcUVRect.bottom);
    }

    pItem = m_UIImage.GetPropItem("Color"); // m_Color
    if (pItem) {
        pItem->D3DColorSet(pUI->m_Color);
    }

    pItem = m_UIImage.GetPropItem("Animate Frame");
    if (pItem) {
        pItem->m_curValue.Format("%f", pUI->m_fAnimFrame);
    }

    pItem = m_UIImage.GetPropItem("Make Animation"); // animate
    if (pItem) {
        if (UISTYLE_IMAGE_ANIMATE & pUI->m_dwStyle) {
            pItem->m_curValue = "Animate Image";
        } else {
            pItem->m_curValue = "Normal Image";
        }
    }
...
void CPropertyView::UpdateUIStringInfo() {
    ...
    if (UI_TYPE_STRING == pUIBase->UIType()) {
        pUI = (CN3UIString *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        if (UISTYLE_STRING_SINGLELINE & pUI->m_dwStyle) {
            pItem->m_crColor = 0;
        } else {
            pItem->m_crColor = 1;
        }
    ...
    if (pItem) {
        if (UISTYLE_STRING_ALIGNCENTER & pUI->m_dwStyle) {
            pItem->m_crColor = 1;
        } else if (UISTYLE_STRING_ALIGNRIGHT & pUI->m_dwStyle) {
            pItem->m_crColor = 2;
        } else {
            pItem->m_crColor = 0;
        }
    ...
    if (pItem) {
        if (UISTYLE_STRING_ALIGNVCENTER & pUI->m_dwStyle) {
            pItem->m_crColor = 1;
        } else if (UISTYLE_STRING_ALIGNBOTTOM & pUI->m_dwStyle) {
            pItem->m_crColor = 2;
        } else {
            pItem->m_crColor = 0;
        }
    ...
    if (pItem) {
        if (pUI->m_pDFont) {
            pItem->m_curValue.Format("%d", pUI->m_pDFont->GetFontHeight());
        } else {
            pItem->m_curValue = _T("");
        }
    }
    ...
    if (pItem) {
        ...
        if (pUI->m_pDFont) {
            ...
            if (D3DFONT_BOLD & dwFlag) {
                pItem->m_curValue += "굵게 ";
            }
            if (D3DFONT_ITALIC & dwFlag) {
                pItem->m_curValue += "기울임꼴";
            }
        }
        if (pItem->m_curValue.GetLength() == 0) {
            pItem->m_curValue = _T("보통");
        }
    }
    ...
    if (pItem) {
        pItem->m_curValue = pUI->GetString().c_str();
    }
...
void CPropertyView::UpdateUIListInfo() {
    ...
    CN3UIList * pUI = NULL;
    if (UI_TYPE_LIST == pUIBase->UIType()) {
        pUI = (CN3UIList *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        pItem->D3DColorSet(pUI->FontColor());
    }
...
void CPropertyView::UpdateUIButtonInfo() {
    ...
    if (UI_TYPE_BUTTON == pUIBase->UIType()) {
        pUI = (CN3UIButton *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        if (UISTYLE_BTN_NORMAL & pUI->m_dwStyle) {
            pItem->m_crColor = 0;
        } else if (UISTYLE_BTN_CHECK & pUI->m_dwStyle) {
            pItem->m_crColor = 1;
        }
    }
    pItem = m_UIButton.GetPropItem("ClickRect left"); // click rect
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetClickRect().left);
    }
    pItem = m_UIButton.GetPropItem("ClickRect top");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetClickRect().top);
    }
    pItem = m_UIButton.GetPropItem("ClickRect right");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetClickRect().right);
    }
    pItem = m_UIButton.GetPropItem("ClickRect bottom");
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetClickRect().bottom);
    }
...
void CPropertyView::UpdateUIIconSlotInfo() {
    ...
    if (UI_TYPE_ICONSLOT != pUIBase->UIType()) {
        return;
    }
    ...
    if (pItem) {
        pItem->m_curValue.Format("%d", pUIBase->GetReserved());
    }
...
void CPropertyView::UpdateUIAreaInfo() // Area정보 갱신
{
    ...
    CN3UIArea * pUI = NULL;
    if (UI_TYPE_AREA == pUIBase->UIType()) {
        pUI = (CN3UIArea *)pUIBase;
    } else {
        return;
    }
...
void CPropertyView::UpdateUIStaticInfo() {
    ...
    if (UI_TYPE_STATIC == pUIBase->UIType() || UI_TYPE_EDIT == pUIBase->UIType()) {
        pUI = (CN3UIStatic *)pUIBase; // edit도 static에서 상속 받은 것이다.
    } else {
        return;
    }
...
void CPropertyView::UpdateUIEditInfo() {
    ...
    CN3UIEdit * pUI = NULL;
    if (UI_TYPE_EDIT == pUIBase->UIType()) {
        pUI = (CN3UIEdit *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        if (UISTYLE_EDIT_PASSWORD & pUI->m_dwStyle) {
            pItem->m_crColor = 1;
        } else if (UISTYLE_EDIT_NUMBERONLY & pUI->m_dwStyle) {
            pItem->m_crColor = 2;
        } else {
            pItem->m_crColor = 0;
        }
...
void CPropertyView::UpdateUIProgressInfo() {
    ...
    if (UI_TYPE_PROGRESS == pUIBase->UIType()) {
        pUI = (CN3UIProgress *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        if (UISTYLE_PROGRESS_LEFT2RIGHT & pUI->m_dwStyle) {
            pItem->m_crColor = 0;
        } else if (UISTYLE_PROGRESS_RIGHT2LEFT & pUI->m_dwStyle) {
            pItem->m_crColor = 1;
        } else if (UISTYLE_PROGRESS_TOP2BOTTOM & pUI->m_dwStyle) {
            pItem->m_crColor = 2;
        } else if (UISTYLE_PROGRESS_BOTTOM2TOP & pUI->m_dwStyle) {
            pItem->m_crColor = 3;
        }
    }
    pItem = m_UIProgress.GetPropItem("MaxValue"); // MaxValue
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetMax());
    }
    pItem = m_UIProgress.GetPropItem("MinValue"); // MinValue
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetMin());
    }
    pItem = m_UIProgress.GetPropItem("CurValue"); // CurValue
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetCurValue());
    }
...
void CPropertyView::UpdateUITrackBarInfo() {
    ...
    if (UI_TYPE_TRACKBAR == pUIBase->UIType()) {
        pUI = (CN3UITrackBar *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        if (UISTYLE_TRACKBAR_VERTICAL & pUI->m_dwStyle) {
            pItem->m_crColor = 1;
        } else {
            pItem->m_crColor = 0;
        }
    ...
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetMaxPos());
    }
    pItem = m_UITrackBar.GetPropItem("Min pos"); // Min pos
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetMinPos());
    }
    pItem = m_UITrackBar.GetPropItem("Cur pos"); // Cur pos
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetPos());
    }
    pItem = m_UITrackBar.GetPropItem("Page size"); // Page size
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetPageSize());
    }
...
void CPropertyView::UpdateUIScrollBarInfo() {
    ...
    if (UI_TYPE_SCROLLBAR == pUIBase->UIType()) {
        pUI = (CN3UIScrollBar *)pUIBase;
    } else {
        return;
    }
    ...
    if (pItem) {
        if (UISTYLE_SCROLLBAR_VERTICAL & pUI->m_dwStyle) {
            pItem->m_crColor = 1;
        } else {
            pItem->m_crColor = 0;
        }
    ...
    if (pItem) {
        pItem->m_curValue.Format("%d", pUI->GetLineSize());
    }
