...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext * pContext) {
    ...
    if (m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CUIEView), CSize(100, 100), pContext) == FALSE) {
        return FALSE;
    }
    ...
    if (m_wndSplitterLeft.CreateView(0, 0, RUNTIME_CLASS(CHierarchyView), CSize(iW, iH), pContext) == FALSE) {
        return FALSE;
    }
    if (m_wndSplitterLeft.CreateView(1, 0, RUNTIME_CLASS(CPropertyView), CSize(iW, iH), pContext) == FALSE) {
        return FALSE;
    }
...
void CMainFrame::EnableAccelerator(BOOL bEnable) {
    ...
    if (bEnable) {
        m_hAccelTable = m_hDefaultAccelTable;
    } else {
        m_hAccelTable = NULL;
    }
}
