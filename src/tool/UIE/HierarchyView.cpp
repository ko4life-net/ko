...
void CHierarchyView::OnSelchanged(NMHDR * pNMHDR, LRESULT * pResult) {
    ...
    if (NULL == pNMTreeView->itemOld.hItem) {
        return;
    }
    ...
    case UI_TYPE_IMAGE: {
        ...
        if (UI_TYPE_IMAGE == pParentUI->UIType()) {
            ...
            if (CN3UIButton::BS_NORMAL == pUIBase->GetReserved()) {
                str = _T("Normal Image");
            } else if (CN3UIButton::BS_DOWN == pUIBase->GetReserved()) {
                str = _T("Down Image");
            } else if (CN3UIButton::BS_ON == pUIBase->GetReserved()) {
                str = _T("On Image");
            } else if (CN3UIButton::BS_DISABLE == pUIBase->GetReserved()) {
                str = _T("Disable Image");
            }
            ...
            if (CN3UIProgress::IMAGETYPE_BKGND == pUIBase->GetReserved()) {
                str = _T("Background Image");
            } else if (CN3UIProgress::IMAGETYPE_FRGND == pUIBase->GetReserved()) {
                str = _T("Foreground Image");
            }
        } else if (UI_TYPE_TRACKBAR == pParentUI->UIType()) {
            if (CN3UITrackBar::IMAGETYPE_BKGND == pUIBase->GetReserved()) {
                str = _T("Background Image");
            } else if (CN3UITrackBar::IMAGETYPE_THUMB == pUIBase->GetReserved()) {
                str = _T("Thumb Image");
            }
        }

        if (UISTYLE_IMAGE_ANIMATE & pUIBase->m_dwStyle) {
            str = "(Animate)" + str;
        }
    } break;
