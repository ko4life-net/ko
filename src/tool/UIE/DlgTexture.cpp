...
int CDlgTexture::OnCreate(LPCREATESTRUCT lpCreateStruct) {
    ...
    ASSERT(m_pTexViewer);
    if (FALSE == m_pTexViewer->Create(NULL, "TexViewer", WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this, 1000)) {
        return -1;
    }
...
BOOL CDlgTexture::OnInitDialog() {
    ...
    if (m_iImageTypeCount > 0) {
        ...
        for (int i = 0; i < m_iImageTypeCount; ++i) {
            m_ImageType.AddString(m_szImageTypeNames[i]);
        }
        m_ImageType.SetCurSel(0);
        m_pTexViewer->SetImageTypeIndex(0);
    } else {
        m_ImageType.ShowWindow(SW_HIDE);
    }
    Resize();
...
LRESULT CDlgTexture::OnUpdateInfo(WPARAM wParam, LPARAM lParam) {
    ...
    if (rcTmp.left > rcTmp.right) {
        rcSelected.left = rcTmp.right;
        rcSelected.right = rcTmp.left;
    }
    if (rcTmp.top > rcTmp.bottom) {
        rcSelected.top = rcTmp.bottom;
        rcSelected.bottom = rcTmp.top;
    }
    ...
    if (-1 == rcSelected.left) {
        strRect.Format("Left :\nTop :\nRight :\nBottom :\nWidth :\nHeight :\n");
    } else {
        strRect.Format("Left : %d\nTop : %d\nRight : %d\nBottom : %d\nWidth : %d\nHeight : %d\n", rcSelected.left,
                       rcSelected.top, rcSelected.right, rcSelected.bottom, rcSelected.Width(), rcSelected.Height());
    }
...
void CDlgTexture::Resize() {
    ...
    if (pOKBtn && pCancelBtn) {
        ...
        if (iTexViewerWidth < 0) {
            iTexViewerWidth = 0;
        } else if (iTexViewerWidth > pFrm->m_Eng.s_DevParam.BackBufferWidth) {
            iTexViewerWidth = pFrm->m_Eng.s_DevParam.BackBufferWidth;
        }
        if (iTexViewerHeight < 0) {
            iTexViewerHeight = 0;
        } else if (iTexViewerHeight > pFrm->m_Eng.s_DevParam.BackBufferHeight) {
            iTexViewerHeight = pFrm->m_Eng.s_DevParam.BackBufferHeight;
        }
        ...
        pWnd->MoveWindow(ptCtrl.x, ptCtrl.y, rc.Width(), rc.Height());
        ptCtrl.y += rc.Height();
        ...
        if (m_iImageTypeCount > 0) {
            ...
            m_ImageType.MoveWindow(ptCtrl.x, ptCtrl.y, rc.Width(), rc.Height());
            ptCtrl.y += rc.Height();
        }

        ptCtrl.y = rcClient.bottom;
        pOKBtn->GetWindowRect(&rc);
        pOKBtn->MoveWindow(ptCtrl.x, ptCtrl.y - rc.Height(), rc.Width(), rc.Height());
        ptCtrl.y -= rc.Height();
        pCancelBtn->GetWindowRect(&rc);
        pCancelBtn->MoveWindow(ptCtrl.x, ptCtrl.y - rc.Height(), rc.Width(), rc.Height());
        ptCtrl.y -= rc.Height();
    }
}
...
void CDlgTexture::SetImageTypes(int iCount, char ** pszNames) {
    ...
    for (int i = 0; i < iCount; ++i) {
        lstrcpy(m_szImageTypeNames[i], pszNames[i]);
    }
}
