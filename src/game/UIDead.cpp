...
bool CUIDead::Load(HANDLE hFile) {
    ...
    m_pTextAlive = (CN3UIString *)(this->GetChildByID("Text_Alive"));
    __ASSERT(m_pTextAlive, "NULL UI Component!!!");
    m_pTextTown = (CN3UIString *)(this->GetChildByID("Text_Town"));
    __ASSERT(m_pTextTown, "NULL UI Component!!!");
...
void CUIDead::MsgSend_Revival(BYTE byType) {
    ...
    if (CGameProcedure::s_pPlayer->m_iSendRegeneration >= 2) {
...
void CUIDead::MsgRecv_Revival(DataPack * pDataPack, int & iOffset) {
    ...
    CGameProcedure::s_pProcMain->InitPlayerPosition(
        vPosPlayer); // 플레이어 위치 초기화.. 일으켜 세우고, 기본동작을 취하게 한다.
