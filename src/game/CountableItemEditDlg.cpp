...
void CCountableItemEditDlg::SetVisibleWithNoSound(bool bVisible, bool bWork, bool bReFocus) {
    if (bWork) { //여기서는 ReceiveMessage에서 유아이가 보이는지를 체크를 해서 일단 앞쪽에 둔다.
...
bool CCountableItemEditDlg::Load(HANDLE hFile) {
    ...
    m_pBtnOk = (CN3UIButton *)(this->GetChildByID("btn_ok"));
    __ASSERT(m_pBtnOk, "NULL UI Component!!!");
    m_pBtnCancel = (CN3UIButton *)(this->GetChildByID("btn_cancel"));
