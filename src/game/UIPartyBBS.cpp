...
bool CUIPartyBBS::Load(HANDLE hFile) {
    ...
    m_pBtn_Refresh = (CN3UIButton *)pParty->GetChildByID("btn_refresh");
    __ASSERT(m_pBtn_Refresh, "NULL UI Component!!!");

    m_pBtn_Close = (CN3UIButton *)GetChildByID("btn_exit");
    __ASSERT(m_pBtn_Close, "NULL UI Component!!!");
    m_pBtn_Register = (CN3UIButton *)pParty->GetChildByID("btn_add");
    ...
    if (dwMsg == UIMSG_BUTTON_CLICK) {
        if (pSender == m_pBtn_Refresh) {
            ...
            CGameProcedure::MessageBoxPost(szMsg, "", MB_YESNO,
                                           BEHAVIOR_PARTY_BBS_REGISTER_CANCEL); // 기사단 해체 물어보기..
...
void CUIPartyBBS::RequestParty() {
    ...
    for (int i = 0; i < PARTY_BBS_MAXLINE; i++, it++) {
        ...
        if (i == m_iCurIndex) {
            ...
            if (0 != lstrcmpi(IPB.szID.c_str(), CGameProcedure::s_pPlayer->m_InfoBase.szID
                                                    .c_str())) { //나 자신에게는 파티 신청을 못하게 한다...
                std::string szMsg;
