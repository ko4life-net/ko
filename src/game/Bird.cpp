...
void CBird::Tick() {
    ...
    if (m_fFactor1 > 2 * D3DX_PI) {
        m_fFactor1 -= (2 * D3DX_PI);
    }

    m_fFactor2 += (m_fFactorSpeed2 * s_fSecPerFrm);
    if (m_fFactor2 > 2 * D3DX_PI) {
        m_fFactor2 -= (2 * D3DX_PI);
    }

    m_fFactorY1 += (m_fFactorYSpeed1 * s_fSecPerFrm);
    if (m_fFactorY1 > 2 * D3DX_PI) {
        m_fFactorY1 -= (2 * D3DX_PI);
    }

    m_fFactorY2 += (m_fFactorYSpeed2 * s_fSecPerFrm);
    if (m_fFactorY2 > 2 * D3DX_PI) {
    ...
    float z =
        (m_fFactorSpeed2 * cosf(m_fFactor2) * sinf(m_fRadian) + m_fRadianSpeed * sinf(m_fFactor2) * cosf(m_fRadian));
