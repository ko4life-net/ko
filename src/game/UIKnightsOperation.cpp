...
bool CUIKnightsOperation::Load(HANDLE hFile) {
    ...
    m_pList_Knights = (CN3UIList *)(this->GetChildByID("List_Knights"));
    __ASSERT(m_pList_Knights, "NULL UI Component!!");
    m_pBtn_Join = (CN3UIButton *)(this->GetChildByID("Btn_Join"));
    __ASSERT(m_pBtn_Join, "NULL UI Component!!");
    m_pBtn_Create = (CN3UIButton *)(this->GetChildByID("Btn_Create"));
    __ASSERT(m_pBtn_Create, "NULL UI Component!!");
    m_pBtn_Destroy = (CN3UIButton *)(this->GetChildByID("Btn_Destroy"));
    __ASSERT(m_pBtn_Destroy, "NULL UI Component!!");
    m_pBtn_Withdraw = (CN3UIButton *)(this->GetChildByID("Btn_Withdraw"));
    __ASSERT(m_pBtn_Withdraw, "NULL UI Component!!");
    m_pEdit_KnightsName = (CN3UIEdit *)(this->GetChildByID("Edit_KnightsName"));
    __ASSERT(m_pEdit_KnightsName, "NULL UI Component!!");
...
void CUIKnightsOperation::KnightsInfoDelete(int iID) {
    ...
    if (it != m_KnightsMapBase.end()) {
        m_KnightsMapBase.erase(it);
    }
}
...
void CUIKnightsOperation::KnightsListAdd(int iID, const std::string & szName, const std::string & szChiefName,
                                         int iMemberCount, int iPoint) {
...
void CUIKnightsOperation::KnightsListUpdate() {
    ...
    for (; it != itEnd; it++) {
        ...
        sprintf(szBuff, "%16s %12s %4d %8d", pKIE->szName.c_str(), pKIE->szChiefName.c_str(), pKIE->iMemberCount,
                pKIE->iPoint);
...
void CUIKnightsOperation::ChangeUIByDuty(e_KnightsDuty eDuty) {
    if (eDuty == KNIGHTS_DUTY_CHIEF) // 기사단장이면 기사단 해체도 가능하다..
    {
        ...
        if (m_pBtn_Withdraw) {
            m_pBtn_Withdraw->SetState(UI_STATE_BUTTON_DISABLE); // 기사 단장이 탈퇴한다????!!!!
        }
        if (m_pBtn_Join) {
        ...
        if (m_pBtn_Withdraw) {
            m_pBtn_Withdraw->SetState(UI_STATE_BUTTON_NORMAL); // 기사 단장이 탈퇴한다????!!!!
        }
        if (m_pBtn_Join) {
...
void CUIKnightsOperation::Close() {
    ...
    if (m_pEdit_KnightsName) {
        m_pEdit_KnightsName->KillFocus(); // 이래야 다른곳에 문제가 안생긴다..
    }
}
...
void CUIKnightsOperation::MsgSend_KnightsCreate() {
    ...
    CAPISocket::MP_AddByte(byBuff, iOffset,
                           N3_SP_KNIGHTS_CREATE); // 생성 Send - s1(Name Length) str1 | Recv - b1(1:성공 0:실패)
...
void CUIKnightsOperation::MsgSend_KnightsDestroy() {
    ...
    CAPISocket::MP_AddByte(byBuff, iOffset, N3_KNIGHTS);
    CAPISocket::MP_AddByte(byBuff, iOffset,
                           N3_SP_KNIGHTS_DESTROY); // 생성 Send - s1(Name Length) str1 | Recv - b1(1:성공 0:실패)
...
void CUIKnightsOperation::MsgSend_KnightsJoin() {
    ...
    if (iCurSel < 0 && iCurSel >= m_KnightsListExt.size()) {
        return;
    }
...
void CUIKnightsOperation::EnableKnightsUIs(bool bEnable) {
    ...
    if (bEnable) {
        eState = UI_STATE_BUTTON_NORMAL;
    } else {
        eState = UI_STATE_BUTTON_DISABLE;
    }

    if (m_pBtn_Up) {
        m_pBtn_Up->SetState(eState);
    }
    if (m_pBtn_Down) {
        m_pBtn_Down->SetState(eState);
    }
    if (m_pBtn_Close) {
        m_pBtn_Close->SetState(eState);
    }

    if (m_pList_Knights) {
        m_pList_Knights->SetState(eState);
    }
    if (m_pBtn_Join) {
        m_pBtn_Join->SetState(eState);
    }
    if (m_pBtn_Create) {
        m_pBtn_Create->SetState(eState);
    }
    if (m_pBtn_Withdraw) {
        m_pBtn_Withdraw->SetState(eState);
    }
    if (m_pEdit_KnightsName) {
        m_pEdit_KnightsName->SetState(eState);
    }
}