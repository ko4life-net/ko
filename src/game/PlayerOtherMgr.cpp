...
void CPlayerOtherMgr::Tick(const __Vector3 & vPosPlayer) {
    ...
    for (; it != itEnd;) {
        ...
        if (iLOD >= 0 && iLOD < MAX_CHR_LOD) {
            iLODTotal += MAX_CHR_LOD - iLOD; // 자동 LOD 계산할때 필요한 값..
        }

        float fDist = pNPC->Distance(vPosPlayer);
        if (fDist < SOUND_RANGE_TO_SET) {
            pNPC->SetSoundAndInitFont(); // SOUND_RANGE 안에 있으면.
        } else if (fDist > SOUND_RANGE_TO_RELEASE) {
            pNPC->ReleaseSoundAndFont();
        }
        it++;
    }
    ...
    if (iLODTotal >= 100) {
        iLODDelta = 3;
    } else if (iLODTotal >= 60) {
        iLODDelta = 2;
    } else if (iLODTotal >= 30) {
        iLODDelta = 1;
    }
...
void CPlayerOtherMgr::CorpseRemove(CPlayerNPC * pCorpse, bool bRemoveImmediately) {
    ...
    if (pCorpse->m_fTimeAfterDeath >= TIME_CORPSE_REMAIN - TIME_CORPSE_REMOVE) {
        return;
    }
    ...
    if (bRemoveImmediately) {
        ...
        pCorpse->m_fTimeAfterDeath =
            TIME_CORPSE_REMAIN - TIME_CORPSE_REMOVE; // 자 이제 없어질 시간이다.. 나머지는 Tick 에서 한다..
    }
}
...
CPlayerNPC * CPlayerOtherMgr::CorpseGetNearstNPC(bool bMustHaveItem, e_Nation eNation,
                                                 const __Vector3 & vPosPlayer) // 가장 가까운 적 시체 가져오기..
{
    ...
    for (; it2 != itEnd2; it2++) {
        ...
        if (bMustHaveItem && pNPC->m_iDroppedItemID <= 0) {
            continue;
        }
...
void CPlayerOtherMgr::MoveToCorpsesForcely(CPlayerNPC * pNPC, bool bErase) {
    ...
    if (bErase) {
        pNPC->m_fTimeAfterDeath = TIME_CORPSE_REMAIN - 10.0f; // 죽은 시간을 세팅...
    } else {
        pNPC->m_fTimeAfterDeath = 0.1f;
    }
...
CPlayerNPC * CPlayerOtherMgr::PickAllPrecisely(int ixScreen, int iyScreen, int & iIDResult, __Vector3 * pvPick) {
    ...
    if (!m_NPCs.empty() || !m_UPCs.empty()) {
        ...
        for (; it_u != itEnd_u; it_u++) {
            NPCs.push_back(it_u->second);
        }
        qsort(&(NPCs[0]), NPCs.size(), 4, SortByCameraDistance);
    } else {
        return NULL;
