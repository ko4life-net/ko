...
bool CUITradeSellBBS::Load(HANDLE hFile) {
    ...
    __ASSERT(m_pBtn_PageDown, "NULL UI Component!!!");
    m_pBtn_Refresh = (CN3UIButton *)(this->GetChildByID("btn_refresh"));
    __ASSERT(m_pBtn_Refresh, "NULL UI Component!!!");
    m_pBtn_Close = (CN3UIButton *)(this->GetChildByID("btn_exit"));
    __ASSERT(m_pBtn_Close, "NULL UI Component!!!");
    m_pBtn_Register = (CN3UIButton *)(this->GetChildByID("btn_add"));
    ...
    m_pBtn_Trade = (CN3UIButton *)(this->GetChildByID("btn_sale"));
    __ASSERT(m_pBtn_Trade, "NULL UI Component!!!");
    m_pBtn_RegisterCancel = (CN3UIButton *)(this->GetChildByID("btn_delete"));
    __ASSERT(m_pBtn_RegisterCancel, "NULL UI Component!!!");

    m_pImage_Sell = (CN3UIImage *)(this->GetChildByID("img_sell gold"));
    __ASSERT(m_pImage_Sell, "NULL UI Component!!!");
    m_pImage_Buy = (CN3UIImage *)(this->GetChildByID("img_buy gold"));
    __ASSERT(m_pImage_Buy, "NULL UI Component!!!");
    m_pImage_Sell_Title = (CN3UIImage *)(this->GetChildByID("img_sell"));
    __ASSERT(m_pImage_Sell_Title, "NULL UI Component!!!");
    m_pImage_Buy_Title = (CN3UIImage *)(this->GetChildByID("img_buy"));
    __ASSERT(m_pImage_Buy_Title, "NULL UI Component!!!");
...
void CUITradeSellBBS::MsgRecv_TradeBBS(DataPack * pDataPack, int & iOffset) {
    ...
    if (bySubType == N3_SP_TYPE_BBS_OPEN) {
        ...
        if (byBBSKind == N3_SP_TRADE_BBS_BUY) {
            ...
            if (m_pImage_Sell_Title) {
                m_pImage_Sell_Title->SetVisible(false);
            }
            if (m_pImage_Buy_Title) {
...
void CUITradeSellBBS::MsgRecv_RefreshData(DataPack * pDataPack, int & iOffset) {
    ...
    for (int i = 0; i < TRADE_BBS_MAX_LINE; i++) {
        ...
        if (iLen > 0) {
            CAPISocket::Parse_GetString(pDataPack->m_pData, iOffset, Info.szID, iLen);
        }
        iLen = CAPISocket::Parse_GetShort(pDataPack->m_pData, iOffset);
        if (iLen > 0) {
            CAPISocket::Parse_GetString(pDataPack->m_pData, iOffset, Info.szTitle, iLen);
        }
        iLen = CAPISocket::Parse_GetShort(pDataPack->m_pData, iOffset);
        if (iLen > 0) {
            CAPISocket::Parse_GetString(pDataPack->m_pData, iOffset, Info.szExplanation, iLen);
        }
...
void CUITradeSellBBS::OnButtonRegisterCancel() {
    ...
    for (int i = 0; i < TRADE_BBS_MAX_LINE; i++, it++) {
        ...
        if (i == m_iCurIndex) {
            ...
            if (0 == lstrcmpi(ITSB.szID.c_str(),
                              CGameProcedure::s_pPlayer->m_InfoBase.szID.c_str())) { //자기것만 등록해제하게..
                MsgSend_RegisterCancel(ITSB.sIndex);
                break;
            } else if (AUTHORITY_MANAGER ==
                       CGameProcedure::s_pProcMain->s_pPlayer->m_InfoBase
                           .iAuthority) { //운영자에게는 해제 권한을 준다...(도배나 욕설등의 게시물 삭제를 위해서...)
                MsgSend_RegisterCancel(ITSB.sIndex);
                break;
            }
        }
...
void CUITradeSellBBS::OnButtonWhisper() {
    ...
    for (int i = 0; i < TRADE_BBS_MAX_LINE; i++, it++) {
        ...
        if (i == m_iCurIndex) {
            __InfoTradeSellBBS ITSB = (*it);
            if (0 !=
                lstrcmpi(ITSB.szID.c_str(),
                         CGameProcedure::s_pPlayer->m_InfoBase.szID.c_str())) { //나 자신에게는 귓속말을 못하게 한다...
