...
class CMachineBase : public CN3Shape {
  ...
  public:
    ...
    void SetDirRadian(float fDirRadian) {
        m_fDirRadian = fDirRadian;
        ReCalcMatrix();
    }
  ...
  public:
    ...
    void ToggleMoveForward(BOOL bOn) {
        if (bOn) {
            m_dwMachineState |= MS_FORWARD;
        } else {
            m_dwMachineState &= (~MS_FORWARD);
        }
    }
    void ToggleMoveBackward(BOOL bOn) {
        if (bOn) {
            m_dwMachineState |= MS_BACKWARD;
        } else {
            m_dwMachineState &= (~MS_BACKWARD);
        }
    }
    void ToggleTurnRight(BOOL bOn) {
        if (bOn) {
            m_dwMachineState |= MS_TURNRIGHT;
        } else {
            m_dwMachineState &= (~MS_TURNRIGHT);
        }
    }
    void ToggleTurnLeft(BOOL bOn) {
        if (bOn) {
            m_dwMachineState |= MS_TURNLEFT;
        } else {
            m_dwMachineState &= (~MS_TURNLEFT);
        }
    }
