...
BOOL CUIQuestMenu::MoveOffset(int iOffsetX, int iOffsetY) {
    ...
    m_rcLImage.left += iOffsetX;
    m_rcLImage.top += iOffsetY;
    m_rcLImage.right += iOffsetX;
    m_rcLImage.bottom += iOffsetY;

    m_rcRImage.left += iOffsetX;
    m_rcRImage.top += iOffsetY;
    m_rcRImage.right += iOffsetX;
    m_rcRImage.bottom += iOffsetY;

    m_rcCImage.left += iOffsetX;
    m_rcCImage.top += iOffsetY;
...
void CUIQuestMenu::Open(DataPack * pDataPack, int & iOffset) {
    ...
    if (m_pTextSample == NULL) {
        return;
    }
    if (m_pImageCenter == NULL || m_pImageLeft == NULL || m_pImageRight == NULL) {
        return;
    }
    if (m_pImageBRight == NULL || m_pImageBLeft == NULL || m_pImageBCenter == NULL) {
        return;
    }
