...
CUIImageTooltipDlg::CUIImageTooltipDlg()
    : m_CYellow(D3DCOLOR_RGBA(255, 255, 0, 255))
...
void CUIImageTooltipDlg::InitPos() {
    for (int i = 0; i < MAX_TOOLTIP_COUNT; i++) {
        m_pStr[i] = (CN3UIString *)GetChildByID(std::format("string_{:d}", i));
        __ASSERT(m_pStr[i], "NULL UI Component!!");
    }

    m_pImg = (CN3UIImage *)GetChildByID("mins");
    __ASSERT(m_pImg, "NULL UI Component!!");
}
...
void CUIImageTooltipDlg::SetPosSomething(int xpos, int ypos, int iNum) {
    ...
    int iHeight =
        (m_pStr[iNum - 1]->GetRegion().bottom - m_pStr[0]->GetRegion().top) + 14; // 2는 string간의 간격의 절반..
...
#define ERROR_EXCEPTION                                                                                                \
    {                                                                                                                  \
        ...
        if (spItem->pItemBasic->siAttackInterval *
                (float)((float)spItem->pItemExt->siAttackIntervalPercentage / 100.0f) !=
            0) {
            m_pStr[iIndex]->SetStyle(UI_STR_TYPE_HALIGN, UISTYLE_STRING_ALIGNLEFT);
            float fValue = spItem->pItemBasic->siAttackInterval *
                           (float)((float)spItem->pItemExt->siAttackIntervalPercentage / 100.0f);
        ...
        if ((spItem->pItemBasic->byAttachPoint == ITEM_LIMITED_EXHAUST) &&
            spItem->pItemBasic->cNeedLevel + spItem->pItemExt->siNeedLevel > 1) {
            ::_LoadStringFromResource(IDS_TOOLTIP_NEEDLEVEL, szStr);
            if (SetTooltipTextColor(CGameBase::s_pPlayer->m_InfoBase.iLevel,
        ...
        if (spItem->pItemBasic->byNeedTitle + spItem->pItemExt->siNeedTitle > 0) {
            ...
            if (SetTooltipTextColor(pInfoExt->iTitle,
                                    spItem->pItemBasic->byNeedTitle + spItem->pItemExt->siNeedTitle)) {
                m_pStr[iIndex]->SetColor(m_CWhite);
            } else {
                m_pStr[iIndex]->SetColor(m_CRed);
            }
        ...
        if (iNeedValue > 0) {
            ...
            if (SetTooltipTextColor(pInfoExt->iStrength,
        ...
        if (iNeedValue > 0) {
            ...
            if (SetTooltipTextColor(pInfoExt->iStamina,
        ...
        if (iNeedValue > 0) {
            ...
            if (SetTooltipTextColor(pInfoExt->iDexterity,
        ...
        if (iNeedValue > 0) {
            ...
            if (SetTooltipTextColor(pInfoExt->iIntelligence,
        ...
        if (iNeedValue > 0) {
            ...
            if (SetTooltipTextColor(pInfoExt->iMagicAttak,
