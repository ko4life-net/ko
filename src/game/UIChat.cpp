...
bool CUIChat::Load(HANDLE hFile) {
    ...
    __ASSERT(m_pBtn_Normal, "NULL UI Component!!");
    m_pBtn_Private = GetChildByID("btn_private");
...
void CUIChat::ChangeChattingMode(e_ChatMode eCM) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (bNBDs[i]) {
            if (m_pBtn_Check) {
                if (bICLs[i]) {
                    m_pBtn_Check->SetState(UI_STATE_BUTTON_DOWN);
                } else {
                    m_pBtn_Check->SetState(UI_STATE_BUTTON_NORMAL);
                }
            }
