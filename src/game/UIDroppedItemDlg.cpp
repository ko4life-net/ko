        ...
        if (m_pMyDroppedItem[i] && ((m_pMyDroppedItem[i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE) ||
            ...
            if (pStr) {
                if ((GetState() == UI_STATE_ICON_MOVING) &&
                    (m_pMyDroppedItem[i] == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect)) {
...
int CUIDroppedItemDlg::GetInventoryEmptyInviOrder(__IconItemSkill * spItem) {
    if (spItem == NULL) {
        ...
        for (int i = 0; i < MAX_ITEM_INVENTORY; i++) {
            if (CGameProcedure::s_pProcMain->m_pUIInventory->m_pMyInvWnd[i] &&
                (CGameProcedure::s_pProcMain->m_pUIInventory->m_pMyInvWnd[i]->pItemBasic->dwID ==
...
void CUIDroppedItemDlg::GetItemByIDToInventory(BYTE bResult, int iItemID, int iGold, int iPos, int iItemCount,
    ...
    if (bResult == 0x07) {
        // 메시지 박스 텍스트 표시..
        std::string szMsg;
        ::_LoadStringFromResource(IDS_INV_ITEM_FULL, szMsg);
        CGameProcedure::s_pProcMain->MsgOutput(szMsg, 0xffff3b3b);
    }
