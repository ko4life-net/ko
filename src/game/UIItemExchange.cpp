...
void CUIItemExchange::IconRestore() {
    ...
    case UIWND_DISTRICT_EX_RE_INV:
        if (m_pMyInvWnd[CN3UIWndBase::m_sSelectedIconInfo.UIWndSelect.iOrder] != NULL) {
            pArea = CN3UIWndBase::GetChildAreaByiOrder(UI_AREA_TYPE_REPAIR_INV,
...
#define FAIL_CODE                                                                                                      \
    ...
    case UIMSG_ICON_DOWN_FIRST:
        ...
        if (eUIWnd != UIWND_DISTRICT_EX_RE_INV) {
            FAIL_CODE
        }
...
void CUIItemExchange::UserPressOK() {
    ...
    for (int i = 0; i < iCount; i++) {
        ...
        CAPISocket::MP_AddDword(byBuff, iOffset,
                                m_pMyNpcWnd[i]->pItemBasic->dwID +
                                    m_pMyNpcWnd[i]->pItemExt->dwID); // 아이디 문자열 패킷에 넣기..
    }
...
void CUIItemExchange::ReceiveResultFromServer(int iResult, int iUserGold) {
    ...
    if (iResult == 0x01) {
        for (int i = 0; i < MAX_ITEM_EX_RE_NPC; i++) {
            if (m_pMyNpcWnd[i]) {
                m_pMyNpcWnd[i]->iDurability =
                    m_pMyNpcWnd[i]->pItemBasic->siMaxDurability + m_pMyNpcWnd[i]->pItemExt->siMaxDurability;
