...
#define ACT_WORLD s_pWorldMgr->GetActiveWorld()
  ...
  public:
    ...
    static CN3TableBase<struct __TABLE_ITEM_BASIC> *
        s_pTbl_Items_Basic; // 각 유저의(내 자신과 주위 다른 사람) 클레스별 장착 아이템 리소스 테이블
    static CN3TableBase<struct __TABLE_ITEM_EXT> *
        s_pTbl_Items_Exts[MAX_ITEM_EXTENSION]; // 각 유저의(내 자신과 주위 다른 사람) 클레스별 장착 아이템 리소스 테이블
    static CN3TableBase<struct __TABLE_PLAYER_LOOKS> *
        s_pTbl_UPC_Looks; // 각 유저의(내 자신과 주위 다른 사람) 클레스별 기본 스킨
    static CN3TableBase<struct __TABLE_PLAYER_LOOKS> *
        s_pTbl_NPC_Looks; // NPC, Mob 기본 모습 - 6개의 캐릭터 파트(), 2개의 플러그
