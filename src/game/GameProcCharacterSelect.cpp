...
void CGameProcCharacterSelect::Init() {
    ...
    m_pUICharacterSelect->LoadFromFile(
        pTbl->szCharacterSelect); // UI Manager 에 넣지 않고 따로 처리한다... 이유가 있다..
...
void CGameProcCharacterSelect::AddChr(e_ChrPos eCP, __CharacterSelectInfo * pInfo) {
    ...
    // 얼굴 -
    if (!pLooks->szPartFiles[PART_POS_FACE].empty()) {
    ...
    } else if (!pLooks->szPartFiles[PART_POS_HAIR_HELMET].empty()) { // 아이템이 없으면 기본 머리..
...
void CGameProcCharacterSelect::AddChrPart(CN3Chr * pChr, const __TABLE_PLAYER_LOOKS * pLooks, e_PartPosition ePartPos,
    ...
    if (fsResrcFile.empty()) {
        pPart = pChr->PartSet(ePartPos, pLooks->szPartFiles[ePartPos]); // 기본 파트
    } else {
