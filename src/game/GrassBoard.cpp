...
void CGrassBoard::Render(CN3Texture ** ppTex) {
    ...
    for (int i = 0; i < m_ucTexNum; ++i) {
        pGrass = &m_sGrassInfo[i];
        if (ppTex[pGrass->iTexIndex] == NULL) {
            continue;
        }
...
void CGrassBoard::SetInfo(__Vector3 vBoardPosion, unsigned short usData) {
    ...
    for (int i = 0; i < m_ucTexNum; ++i) {
        ...
        mRand = rand();
        pGrass->vPos.x = (mRand % 40) / 10.000 + vBoardPosion.x;

        pGrass->dwAlpColor = 0x00ffffff;
        pGrass->iTexIndex = InputGrass[iCount];
        if (++iCount >= GrassInputCount) {
            iCount = 0;
        }
...
void CGrassBoard::FindGrassIndex(const unsigned char uCGrassMngOrder, int * pnInputGrass, int & nGrassTotNum) {
    ...
    if (uCGrassMngOrder & 0x01) {
        pnInputGrass[nGrassTotNum] = 0;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x02) {
        pnInputGrass[nGrassTotNum] = 1;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x04) {
        pnInputGrass[nGrassTotNum] = 2;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x08) {
        pnInputGrass[nGrassTotNum] = 3;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x10) {
        pnInputGrass[nGrassTotNum] = 4;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x20) {
        pnInputGrass[nGrassTotNum] = 5;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x40) {
        pnInputGrass[nGrassTotNum] = 6;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x80) {
        pnInputGrass[nGrassTotNum] = 7;
        nGrassTotNum++;
    }
}
