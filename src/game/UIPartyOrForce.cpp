...
bool CUIPartyOrForce::Load(HANDLE hFile) {
    ...
    for (int i = 0; i < MAX_PARTY_OR_FORCE; i++) // 빈곳을 찾자..
    {
        ...
        __ASSERT(m_pProgress_HPs[i], "NULL UI Component!!!");
        ...
        __ASSERT(m_pProgress_HPReduce[i], "NULL UI Component!!!");
        ...
        __ASSERT(m_pProgress_ETC[i], "NULL UI Component!!!");
        ...
        __ASSERT(m_pStatic_IDs[i], "NULL UI Component!!!");
        if (m_pStatic_IDs[i]) {
        ...
        __ASSERT(m_pAreas[i], "NULL UI Component!!!");
    }
...
bool CUIPartyOrForce::ReceiveMessage(CN3UIBase * pSender, DWORD dwMsg) {
    if (dwMsg == UIMSG_BUTTON_CLICK) {
        ...
        if (pIP) {
            ...
            CGameProcedure::s_pProcMain->TargetSelect(-1, false);
        }
    }
...
void CUIPartyOrForce::Render() {
    ...
    if (m_iIndexSelected < 0 || m_iIndexSelected >= m_Members.size() || m_iIndexSelected >= MAX_PARTY_OR_FORCE) {
        return;
    }
    if (NULL == m_pStatic_IDs[m_iIndexSelected] || NULL == m_pProgress_HPs[m_iIndexSelected]) {
        return;
    }
...
bool CUIPartyOrForce::TargetByIndex(int iIndex) {
    if (iIndex < 0 || iIndex >= m_Members.size()) {
        return false;
    }
...
const __InfoPartyOrForce * CUIPartyOrForce::MemberAdd(int iID, const std::string szID, int iLevel, e_Class eClass,
                                                      int iHP, int iHPMax) {
...
void CUIPartyOrForce::MemberInfoReInit() // 파티원 구성이 변경될때.. 순서 및 각종 정보 업데이트..
{
    ...
    if (m_Members.empty()) {
        this->SetVisible(false); // 멤버가 없으면 숨긴다.
    } else {
        this->SetVisible(true); // 멤버가 있으면 보인다.
    }
}
...
const __InfoPartyOrForce * CUIPartyOrForce::MemberInfoGetSelected() {
    ...
    if (m_iIndexSelected < 0 || m_iIndexSelected > m_Members.size()) {
        return NULL;
    }

    it_PartyOrForce it = m_Members.begin(), itEnd = m_Members.end();
    for (int i = 0; i < m_iIndexSelected; i++) {
        it++;
    }
...
void CUIPartyOrForce::MemberHPChange(int iID, int iHP, int iHPMax) {
    ...
    for (int i = 0; it != itEnd && i < MAX_PARTY_OR_FORCE; it++, i++) {
        ...
        if (pIP->iID == iID) {
            ...
            if (m_pProgress_HPs[i]) {
                m_pProgress_HPs[i]->SetCurValue(pIP->iHP * 100 / pIP->iHPMax, 0.7f, 50.0f);
            }
            if (m_pProgress_HPReduce[i]) {
                m_pProgress_HPReduce[i]->SetCurValue(pIP->iHP * 100 / pIP->iHPMax, 0.7f, 50.0f);
            }
            if (m_pProgress_ETC[i]) {
                m_pProgress_ETC[i]->SetCurValue(pIP->iHP * 100 / pIP->iHPMax, 0.7f, 50.0f);
            }
            break;
        }
    }
...
void CUIPartyOrForce::MemberStatusChange(int iID, e_PartyStatus ePS, bool bSuffer) {
    ...
    for (int i = 0; it != itEnd && i < MAX_PARTY_OR_FORCE; it++, i++) {
        ...
        if (pIP->iID == iID) {
            ...
            } else if (PARTY_STATUS_DOWN_ETC == ePS) {
                pIP->bSufferDown_Etc = bSuffer;
            }
            break;
        }
    }
