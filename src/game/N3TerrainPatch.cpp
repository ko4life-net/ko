...
void CN3TerrainPatch::SetLevel(int level) {
    ...
    HRESULT hr;
    if (level == 1) {
        hr = CN3Base::s_lpD3DDev->CreateVertexBuffer(m_VBSize[level - 1] * sizeof(__VertexT2), 0, FVF_VNT2,
                                                     D3DPOOL_MANAGED, &m_pVB, NULL);
        hr = CN3Base::s_lpD3DDev->CreateVertexBuffer(m_VBSize[level - 1] * sizeof(__VertexT1), 0, FVF_VNT1,
                                                     D3DPOOL_MANAGED, &m_pLightMapVB, NULL);
...
void CN3TerrainPatch::Tick() {
    if (m_ti_LBPoint.x < 0 || m_ti_LBPoint.y < 0 || m_ti_LBPoint.x >= (m_pRefTerrain->m_ti_MapSize - 1) ||
        m_ti_LBPoint.y >= (m_pRefTerrain->m_ti_MapSize - 1)) {
        m_bIsRender = FALSE;
    }
    ...
    if (m_iLevel == 1) {
        ...
        for (ix = 0; ix < PATCH_TILE_SIZE; ix++) {
            for (iz = 0; iz < PATCH_TILE_SIZE; iz++) {
                ...
                if (m_pTileTexIndx[0][TileCount] < 0 || m_pTileTexIndx[0][TileCount] > m_pRefTerrain->m_NumTileTex ||
                ...
                if ((ix + iz) % 2 == 0) {
                    pVertices[VertexIdx].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz).fHeight,
                                             (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[0], v1[0], u2[0], v2[0]);
                    pVertices[VertexIdx + 1].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz + 1).fHeight,
                                                 (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[1], v1[1], u2[1],
                                                 v2[1]);
                    pVertices[VertexIdx + 2].Set(
                        (float)((tx + 1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx + 1, tz + 1).fHeight,
                        (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[2], v1[2], u2[2], v2[2]);
                    pVertices[VertexIdx + 3].Set((float)((tx + 1) * TILE_SIZE),
                                                 m_pRefTerrain->GetMapData(tx + 1, tz).fHeight, (float)(tz * TILE_SIZE),
                                                 0.0f, 1.0f, 0.0f, u1[3], v1[3], u2[3], v2[3]);
                    VertexIdx += 4;
                    TileCount++;
                    ...
                    if (m_pRefTerrain->GetLightMap(tx, tz)) {
                        ...
                        if (!pLightMapVertices) {
                            m_pLightMapVB->Lock(0, 0, (VOID **)&pLightMapVertices, 0);
                        }
                        ...
                        pLightMapVertices[VBIndx].Set((float)(tx * TILE_SIZE),
                                                      m_pRefTerrain->GetMapData(tx, tz).fHeight,
                                                      (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, euv);
                        pLightMapVertices[VBIndx + 1].Set((float)(tx * TILE_SIZE),
                                                          m_pRefTerrain->GetMapData(tx, tz + 1).fHeight,
                                                          (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, suv);
                        pLightMapVertices[VBIndx + 2].Set((float)((tx + 1) * TILE_SIZE),
                                                          m_pRefTerrain->GetMapData(tx + 1, tz + 1).fHeight,
                                                          (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, suv);
                        pLightMapVertices[VBIndx + 3].Set((float)((tx + 1) * TILE_SIZE),
                                                          m_pRefTerrain->GetMapData(tx + 1, tz).fHeight,
                                                          (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, euv);

                        m_NumLightMapTex++;
                    }
                } else {
                    pVertices[VertexIdx].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz + 1).fHeight,
                                             (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[1], v1[1], u2[1],
                                             v2[1]);
                    pVertices[VertexIdx + 1].Set(
                        (float)((tx + 1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx + 1, tz + 1).fHeight,
                        (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[2], v1[2], u2[2], v2[2]);
                    pVertices[VertexIdx + 2].Set((float)((tx + 1) * TILE_SIZE),
                                                 m_pRefTerrain->GetMapData(tx + 1, tz).fHeight, (float)(tz * TILE_SIZE),
                                                 0.0f, 1.0f, 0.0f, u1[3], v1[3], u2[3], v2[3]);
                    pVertices[VertexIdx + 3].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz).fHeight,
                                                 (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[0], v1[0], u2[0], v2[0]);
                    VertexIdx += 4;
                    TileCount++;
                    ...
                    if (m_pRefTerrain->GetLightMap(tx, tz)) {
                        ...
                        pLightMapVertices[VBIndx].Set((float)(tx * TILE_SIZE),
                                                      m_pRefTerrain->GetMapData(tx, tz + 1).fHeight,
                                                      (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, suv);
                        pLightMapVertices[VBIndx + 1].Set((float)((tx + 1) * TILE_SIZE),
                                                          m_pRefTerrain->GetMapData(tx + 1, tz + 1).fHeight,
                                                          (float)((tz + 1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, suv);
                        pLightMapVertices[VBIndx + 2].Set((float)((tx + 1) * TILE_SIZE),
                                                          m_pRefTerrain->GetMapData(tx + 1, tz).fHeight,
                                                          (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, euv);
                        pLightMapVertices[VBIndx + 3].Set((float)(tx * TILE_SIZE),
                                                          m_pRefTerrain->GetMapData(tx, tz).fHeight,
                                                          (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, euv);
                        m_NumLightMapTex++;
                    }
    ...
    if (m_iLevel == 2) {
        ...
        for (ix = 0; ix < m_NumCell; ix++) {
            for (iz = 0; iz < m_NumCell; iz++) {
                ...
                pVertices[VertexIdx].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz).fHeight,
                                         (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, UVConvert(tu), UVConvert(tv));
                //tu, tv);
                VertexIdx++;
                ...
                tz = cz - HalfCell;
                pVertices[VertexIdx].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz).fHeight,
                                         (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, UVConvert(tu - HalfUV),
                                         UVConvert(tv + HalfUV));
                //tu - HalfUV, tv + HalfUV);
                VertexIdx++;
                ...
                if (((iz + 1) == m_NumCell) && (m_IsBlunt[1] == false)) {
                    for (int i = 0; i < 3; i++) {
                        tx++;
                        pVertices[VertexIdx].Set(
                            (float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz).fHeight, (float)(tz * TILE_SIZE),
                            0.0f, 1.0f, 0.0f, UVConvert((float)(tx % UNITUV) / (float)UNITUV), UVConvert(tv - HalfUV));
        ...
        for (ix = 0; ix < m_NumCell; ix++) {
            for (iz = 0; iz < m_NumCell; iz++) {
                ...
                if (((iz + 1) == m_NumCell) && (m_IsBlunt[1] == false)) {
                    tx += HalfCell;
                    pVertices[VertexIdx].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx, tz).fHeight,
                                             (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, UVConvert(tu),
                                             UVConvert(tv - HalfUV));
                    //(tu), (tv - HalfUV));
                    VertexIdx++;
                    FaceCount++;
...
void CN3TerrainPatch::Render() {
    ...
    if (m_iLevel == 1) {
        ...
        for (int i = 0; i < TotalTile; i++) {
            ...
            if ((m_pTileTexIndx[0][i] < 0) || m_pTileTexIndx[0][i] >= m_pRefTerrain->m_NumTileTex ||
                (m_pIsTileFull[i] == false)) // 0: 컬러맵, 1:무늬 or 0:부분타일 1:NONE...
            {
                ...
                if (m_pTileTexIndx[1][i] >= 0 &&
                    m_pTileTexIndx[1][i] < m_pRefTerrain->m_NumTileTex) //0: tile, 1: tile..
                {
            ...
            if ((!m_pRefTerrain->m_bAvailableTile) && m_pTileTexIndx[0][i] >= 0 &&
                m_pTileTexIndx[0][i] < m_pRefTerrain->m_NumTileTex && m_pTileTexIndx[1][i] >= 0 &&
