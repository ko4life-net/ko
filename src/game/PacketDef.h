...
#define N3_CHAT_SELECT_TARGET                                                                                          \
    0x35 // Send - s1(ID String Length) str1(ID String) | Recv - s1(ID String Length) str1(ID String) 문자열 길이가 0이면 실패..
...
enum e_SubPacket_Knights {
    ...
    N3_SP_KNIGHTS_GRADE_CHANGE_ALL =
        0x0C, // 전 기사단의 등급 및 순위 변경 Recv - s1(count) Loop { s1(Knights ID) b2(Grade Rank) }
...
#define N3_FRIEND_INFO                                                                                                 \
    0x49 // 친구 정보.. Send s1(유저카운트) s1(이름길이), str1(유저이름) | Receive s1(유저카운트) ㅣLoop { s1(이름길이), str1(유저이름), b1(접속, 파티 or 연산) }
#define N3_NOAH_CHANGE 0x4a // 노아 Change..
enum e_SubPacket_NoahChange {
    N3_SP_NOAH_GET = 0x01,  // 노아 습득,
...
enum e_Trade_BBS_Sub {
    N3_SP_TYPE_REGISTER = 0x01,        // 물건 등록하기
