...
bool CUICharacterCreate::Load(HANDLE hFile) {
    ...
    m_pArea_Character = (CN3UIArea *)(this->GetChildByID("area_character"));
    __ASSERT(m_pArea_Character, "NULL UI Component!!");
    ...
    for (int i = 0; i < MAX_STATS; i++) {
        m_pStr_Stats[i] = (CN3UIString *)(this->GetChildByID(szTexts[i]));
        __ASSERT(m_pStr_Stats[i], "NULL UI Component!!");
        m_pArea_Stats[i] = (CN3UIArea *)(this->GetChildByID(szAreas[i]));
        __ASSERT(m_pArea_Stats[i], "NULL UI Component!!");
        m_pImg_Stats[i] = (CN3UIImage *)(this->GetChildByID(szImgs[i]));
        __ASSERT(m_pImg_Stats[i], "NULL UI Component!!");
        if (m_pArea_Stats[i]) {
            ::_LoadStringFromResource(dwResrcIDs[i], m_pArea_Stats[i]->m_szToolTip);
        }
    }
    m_pStr_Bonus = (CN3UIString *)(this->GetChildByID("text_bonus"));
    __ASSERT(m_pStr_Bonus, "NULL UI Component!!");

    m_pBtn_Face_Left = (CN3UIButton *)(this->GetChildByID("btn_face_left"));
    __ASSERT(m_pBtn_Face_Left, "NULL UI Component!!");
    m_pBtn_Face_Right = (CN3UIButton *)(this->GetChildByID("btn_face_right"));
    __ASSERT(m_pBtn_Face_Right, "NULL UI Component!!");
    m_pBtn_Hair_Left = (CN3UIButton *)(this->GetChildByID("btn_hair_left"));
    __ASSERT(m_pBtn_Hair_Left, "NULL UI Component!!");
    m_pBtn_Hair_Right = (CN3UIButton *)(this->GetChildByID("btn_hair_right"));
    __ASSERT(m_pBtn_Hair_Right, "NULL UI Component!!");
    ...
    for (int i = 0; i < MAX_RACE_SELECT; i++) {
        ...
        if (m_pBtn_Races[i]) {
            ::_LoadStringFromResource(dwResrcID_Races[i], m_pBtn_Races[i]->m_szToolTip);
        }
    }

    std::string szBtns[MAX_CLASS_SELECT] = {"btn_class_warrior", "btn_class_rogue", "btn_class_mage",
                                            "btn_class_priest"};
    ...
    for (int i = 0; i < MAX_CLASS_SELECT; i++) {
        ...
        __ASSERT(m_pBtn_Classes[i], "NULL UI Component!!");
        m_pImg_Disable_Classes[i] = (CN3UIImage *)(this->GetChildByID(szImgs2[i]));
        __ASSERT(m_pImg_Disable_Classes[i], "NULL UI Component!!");
    ...
    if (dwMsg == UIMSG_BUTTON_CLICK) {
        ...
        if (pSender->m_szID == "btn_cancel") {
            CGameProcedure::ProcActiveSet(
                (CGameProcedure *)CGameProcedure::s_pProcCharacterSelect); // 캐릭터 선택 프로시저로 한다..
            return true;
...
void CUICharacterCreate::UpdateClassButtons(e_Class eClass) {
    ...
    for (int i = 0; i < MAX_STATS; i++) {
        if (m_pImg_Stats[i]) {
            m_pImg_Stats[i]->SetVisible(bVisibles[i]);
        }
    }
}
