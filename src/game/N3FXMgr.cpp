        ...
        if (pBundle->m_iMoveType != FX_BUNDLE_MOVE_NONE && pBundle->m_dwState == FX_BUNDLE_STATE_LIVE) {
            ...
            if (dwToMe == 1 || dwToMe == 2) {
                ...
                if (dwToMe == 2 && ((pBundle->m_vPos - s_pPlayer->Position()).Magnitude() < 16.0f)) {
                    ...
                    if (true == s_pPlayer->CheckCollisionByBox(
                                    pBundle->m_vPos,
                                    pBundle->m_vPos + pBundle->m_vDir * pBundle->m_fVelocity * CN3Base::s_fSecPerFrm,
                                    &vCol, NULL))
                ...
                for (; it != itEnd; it++) {
                    pUPC = it->second;
                    if (dwToMe == 1 && pUPC->m_InfoBase.eNation == s_pPlayer->m_InfoBase.eNation) {
                        continue;
                    }
                    ...
                    if (true == pUPC->CheckCollisionByBox(pBundle->m_vPos,
                                                          pBundle->m_vPos + pBundle->m_vDir * pBundle->m_fVelocity *
                                                                                CN3Base::s_fSecPerFrm,
                                                          &vCol, NULL)) {
                        bCol = true;
                        pBundle->m_vPos = vCol;
                        pBundle->Stop();
                ...
                if (false == bCol) {
                    __Vector3 vNext =
                        pBundle->m_vPos + pBundle->m_vDir * (pBundle->m_fVelocity * CN3Base::s_fSecPerFrm * 1.2f);
                    ...
                    for (; it2 != itEnd2; it2++) {
                        ...
                        if (dwToMe == 1 && pNPC->m_InfoBase.eNation == s_pPlayer->m_InfoBase.eNation) {
                            continue;
                        } else if (pSNPC && dwToMe == 2 && pSNPC->m_InfoBase.eNation == pNPC->m_InfoBase.eNation) {
                            continue;
                        }
                        ...
                        if (pNPC->IDNumber() == pBundle->m_iTargetID && pNPC->m_pShapeExtraRef) {
                            ...
                            if ((pBundle->m_vPos - vDestPos).Magnitude() <= fDistTmp) {
                                ...
                                pBundle->Stop();
                                int iMagicID =
                                    CGameProcedure::s_pProcMain->m_pMagicSkillMng->GetMagicID(pBundle->m_iIdx);
                ...
                // Object 와 충돌 체크..
                if (bCol == false &&
                    true == ACT_WORLD->CheckCollisionWithShape(pBundle->m_vPos, pBundle->m_vDir,
                                                               pBundle->m_fVelocity * CN3Base::s_fSecPerFrm, &vCol)) {
                    bCol = true;
                    pBundle->m_vPos = vCol;
                ...
                // 지형과 충돌체크..
                if (bCol == false && ACT_WORLD->CheckCollisionWithTerrain(pBundle->m_vPos, pBundle->m_vDir,
                                                                          pBundle->m_fVelocity, &vCol)) {
                    //충돌...
                    //여기서 패킷 날려야 겠구만...
                    bCol = true;
                    pBundle->m_vPos = vCol;
