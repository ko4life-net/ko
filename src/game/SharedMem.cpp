...
void CSharedMemQueue::DestroySMQ() {
    if (m_lpMMFile) {
        ...
        if (m_hrMMFile) {
            CloseHandle(m_hrMMFile);
        }
        if (m_hsMMFile) {
            CloseHandle(m_hsMMFile);
        }
    }
}
...
BOOL CSharedMemQueue::CreateSmq(BOOL Server) {
    ...
    if (!(m_lpMMFile = (char *)MapViewOfFile(m_hrMMFile, FILE_MAP_WRITE, 0, 0, 0))) {
        return FALSE;
    }
    ...
    if (!(m_lpMMFile = (char *)MapViewOfFile(m_hsMMFile, FILE_MAP_WRITE, 0, 0, 0))) {
        return FALSE;
    }
