...
bool CUIStateBar::Load(HANDLE hFile) {
    ...
    m_pProgress_MSP = (CN3UIProgress *)(this->GetChildByID("Progress_MSP"));
    __ASSERT(m_pProgress_MSP, "NULL UI Component!!");
    m_pProgress_ExpC = (CN3UIProgress *)(this->GetChildByID("Progress_ExpC"));
    __ASSERT(m_pProgress_ExpC, "NULL UI Component!!");
    m_pProgress_ExpP = (CN3UIProgress *)(this->GetChildByID("Progress_ExpP"));
    __ASSERT(m_pProgress_ExpP, "NULL UI Component!!");
    ...
    __ASSERT(m_pGroup_MiniMap, "NULL UI Component!!");
    if (m_pGroup_MiniMap) {
        m_pGroup_MiniMap->SetVisible(false);

        m_pImage_Map = (CN3UIImage *)(m_pGroup_MiniMap->GetChildByID("Img_MiniMap"));
        __ASSERT(m_pImage_Map, "NULL UI Component!!");
        m_pBtn_ZoomIn = (CN3UIButton *)(m_pGroup_MiniMap->GetChildByID("Btn_ZoomIn"));
...
void CUIStateBar::UpdateExp(int iExp, int iExpNext, bool bUpdateImmediately) {
    ...
    if (NULL == m_pProgress_ExpC || NULL == m_pProgress_ExpP) {
        return;
    }
...
void CUIStateBar::TickMiniMap() {
    ...
    if (m_vPosPlayer.x < 0 || m_vPosPlayer.x > m_fMapSizeX || m_vPosPlayer.z < 0 || m_vPosPlayer.z > m_fMapSizeZ) {
        return;
    }
...
void CUIStateBar::TickMagicIcon() {
    ...
    for (int i = 0; i < cnt; i++, it++) {
        ...
        if (pMagicImg->fDuration < 0.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 1.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 1.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 2.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 2.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 3.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 3.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 4.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 4.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 5.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 5.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 6.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 6.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 7.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 7.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 8.0f) {
            pMagicImg->pIcon->SetVisible(false);
        } else if (pMagicImg->fDuration < 8.5f) {
            pMagicImg->pIcon->SetVisible(true);
        } else if (pMagicImg->fDuration < 9.0f) {
            pMagicImg->pIcon->SetVisible(false);
        }
    }
