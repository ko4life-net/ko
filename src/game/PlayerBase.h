...
class CPlayerBase : public CGameBase {
  ...
  protected:
    ...
    float
        m_fMoveSpeedPerSec; // 초당 움직임 값.. 이값은 기본값이고 상태(걷기, 달리기, 뒤로, 저주등) 에 따라 가감해서 쓴다..
  ...
  public:
    ...
    bool IsMovingNow() {
        if (PSM_WALK == m_eStateMove || PSM_RUN == m_eStateMove || PSM_WALK_BACKWARD == m_eStateMove) {
            return true;
        }
        return false;
    } // 움직이고 있는지?
    ...
    bool Action(e_StateAction eState, bool bLooping, CPlayerBase * pTarget = NULL,
                bool bForceSet = false);                        // 행동 테이블에 따른 행동을 한다..
    ...
    void DurationColorSet(const _D3DCOLORVALUE & color,
