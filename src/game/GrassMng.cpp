...
void CGrassMng::FindGrassIndex(const unsigned char uCGrassMngOrder, int * pnInputGrass, unsigned char * GrassIndex,
                               int & nGrassTotNum) {
    nGrassTotNum = 0;
    if (uCGrassMngOrder & 0x01) {
        pnInputGrass[nGrassTotNum] = 0, GrassIndex[nGrassTotNum] = 0x01;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x02) {
        pnInputGrass[nGrassTotNum] = 1, GrassIndex[nGrassTotNum] = 0x02;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x04) {
        pnInputGrass[nGrassTotNum] = 2, GrassIndex[nGrassTotNum] = 0x04;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x08) {
        pnInputGrass[nGrassTotNum] = 3, GrassIndex[nGrassTotNum] = 0x08;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x10) {
        pnInputGrass[nGrassTotNum] = 4, GrassIndex[nGrassTotNum] = 0x10;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x20) {
        pnInputGrass[nGrassTotNum] = 5, GrassIndex[nGrassTotNum] = 0x20;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x40) {
        pnInputGrass[nGrassTotNum] = 6, GrassIndex[nGrassTotNum] = 0x40;
        nGrassTotNum++;
    }
    if (uCGrassMngOrder & 0x80) {
        pnInputGrass[nGrassTotNum] = 7, GrassIndex[nGrassTotNum] = 0x80;
        nGrassTotNum++;
    }
}
...
inline bool CGrassMng::IsInRect(float fPoX, float fPoY, float * fRange) {
    return (fPoX > fRange[GRASS_LEFT] && fPoX < fRange[GRASS_RIGHT] && fPoY > fRange[GRASS_TOP] &&
            fPoY < fRange[GRASS_BOTTOM]);
}

void CGrassMng::ChkTileRange(float fCamX, float fCamZ) {
    if (IsInRect(fCamX, fCamZ, m_fChkRange) == true) {
        return; //    고칠게 없어
    }
...
void CGrassMng::FineNewTile(WORD * Tile, int & iCount, float * fCompRange, float * fLargeRange) {
    ...
    if (fCompRange[GRASS_LEFT] != m_fChkRange[GRASS_LEFT]) {
        ...
        if (fCompRange[GRASS_LEFT] > m_fChkRange[GRASS_LEFT]) {
            sx = fLargeRange[GRASS_RIGHT] - TILE_SIZE;
        } else {
            sx = fLargeRange[GRASS_LEFT];
        }

        for (int i = 0; i < GRASS_TILENUM; ++i) {
            Tile[iCount] = sx, Tile[iCount + 1] = (sy += TILE_SIZE);
            iCount += 2;
        }
    }
    ...
    if (fCompRange[GRASS_TOP] != m_fChkRange[GRASS_TOP]) {
        if (fCompRange[GRASS_TOP] > m_fChkRange[GRASS_TOP]) {
            ...
            sy = fLargeRange[GRASS_BOTTOM] - TILE_SIZE;
        }

        int si, ei;
        if (sx == -1) {
            si = 0, ei = GRASS_TILENUM;
        } else if (sx == fLargeRange[GRASS_LEFT]) {
            si = 1, ei = GRASS_TILENUM;
        } else {
            si = 0, ei = GRASS_TILENUM - 1;
        }

        sx = fLargeRange[GRASS_LEFT] + TILE_SIZE * si;

        for (int i = si; i < ei; ++i) {
            Tile[iCount] = (sx += TILE_SIZE), Tile[iCount + 1] = sy;
            iCount += 2;
        }
    }
}