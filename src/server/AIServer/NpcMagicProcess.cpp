...
void CNpcMagicProcess::MagicPacket(char * pBuf, int len, CIOCPort * pIOCP) {
    int index = 0, send_index = 0, magicid = 0, sid = -1, tid = -1, data1 = 0, data2 = 0, data3 = 0, data4 = 0,
        data5 = 0, data6 = 0;
    char send_buff[128];
...
_MAGIC_TABLE * CNpcMagicProcess::IsAvailable(int magicid, int tid, BYTE type) {
    ...
    if (tid >= 0 && tid < MAX_USER) // Compare morals between source and target character.
    {
        ...
        if (!pUser || pUser->m_bLive == USER_DEAD) {
            goto fail_return;
        }
    ...
    } else if (tid >= NPC_BAND) // Compare morals between source and target NPC.
    {
        ...
        if (pTable->bMoral == MORAL_AREA_ENEMY) {
            if (m_pSrcNpc->m_byGroup ==
                0) { // Switch morals. 작업할것 : 몬스터는 국가라는 개념이 없기 때문에.. 나중에 NPC가 이 마법을 사용하면 문제가 됨
                moral = 2;
            } else {
                moral = 1;
            }
...
void CNpcMagicProcess::ExecuteType1(int magicid, int tid, int data1, int data2,
...
void CNpcMagicProcess::ExecuteType8(int magicid, int tid, int sid, int data1, int data2,
