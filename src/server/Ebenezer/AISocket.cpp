    ...
    for (int i = 0; i < byCount; i++) {
        ...
        if (sZoneIndex < 0 || sZoneIndex >= m_pMain->m_ZoneArray.size() || nid < 0 || sPid < 0) {
            TRACE("#### Recv --> NpcUserInfoAll Fail: uid=%d, sid=%d, name=%s, zoneindex=%d, x=%f, z=%f.. \n", nid,
                  sPid, szName, sZoneIndex, fPosX, fPosZ);
            //delete pNpc;
            //pNpc = NULL;
            continue;
        }
        ...
        if (tid >= USER_BAND && tid < NPC_BAND) {
            ...
            */
            ...
            if (result == 0x02) { // user dead
                ...
                wsprintf(buff, "*** User Dead, id=%s, result=%d, AI_HP=%d, GM_HP=%d, x=%d, z=%d",
                         pUser->m_pUserData->m_id, result, nHP, pUser->m_pUserData->m_sHp,
                         (int)pUser->m_pUserData->m_curx, (int)pUser->m_pUserData->m_curz);
                TimeTrace(buff);
                ...
                if (pUser->m_pUserData->m_bFame ==
                    ...
                    m_pMain->Send_Region(pOutBuf, send_index, pUser->m_pUserData->m_bZone, pUser->m_RegionX,
                                         pUser->m_RegionZ);
                    // sungyong tw
                    pUser->Send(pOutBuf, send_index);
                    // ~sungyong tw
...
void CAISocket::RecvMagicAttackResult(char * pBuf) {
    ...
    if (byCommand == 0x01) { // casting
        ...
        m_pMain->Send_Region(send_buff, send_index, pNpc->m_sCurZone, pNpc->m_sRegion_X, pNpc->m_sRegion_Z, NULL,
                             false);
        ...
        if (sid >= USER_BAND && sid < NPC_BAND) {
            ...
            index = 0;
            SetByte(send_buff, WIZ_MAGIC_PROCESS, index);
            m_pMain->Send_Region(send_buff, send_index, pUser->m_pUserData->m_bZone, pUser->m_RegionX, pUser->m_RegionZ,
                                 NULL, false);
            ...
            if (tid >= NPC_BAND) {
                ...
                index = 0;
                SetByte(send_buff, WIZ_MAGIC_PROCESS, index);
                m_pMain->Send_Region(send_buff, send_index, pNpc->m_sCurZone, pNpc->m_sRegion_X, pNpc->m_sRegion_Z,
                                     NULL, false);
                return;
            }
            memset(send_buff, NULL, 1024);
...
void CAISocket::RecvNpcInfo(char * pBuf) {
    ...
    if (pNpc->m_NpcState == NPC_LIVE) { // 살아 있는데 또 정보를 받는 경우
        ...
        wsprintf(strLog, "## time(%d:%d-%d) npc regen check(%d) : nid=%d, name=%s, x=%d, z=%d, rx=%d, rz=%d ## \r\n",
                 t.GetHour(), t.GetMinute(), t.GetSecond(), pNpc->m_NpcState, nid, szName, (int)pNpc->m_fCurX,
                 (int)pNpc->m_fCurZ, pNpc->m_sRegion_X, pNpc->m_sRegion_Z);
