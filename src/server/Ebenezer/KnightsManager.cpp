    ...
    for (; Iter1 != Iter2; Iter1++) {
        if (knightindex < ((*Iter1).second)->m_sIndex) {
            if (nation == KARUS) { // sungyong,, 카루스와 전쟁존의 합침으로 인해서,,,
                if (((*Iter1).second)->m_sIndex >= 15000) {
                    continue;
                }
            }
...
void CKnightsManager::WithdrawKnights(CUser * pUser, char * pBuf) {
    ...
    if (pUser->m_pUserData->m_bKnights < 1 ||
        pUser->m_pUserData->m_bKnights > 30000) { // 기사단에 가입되어 있지 않습니다
        ret_value = 10;
        goto fail_return;
    }
...
void CKnightsManager::ModifyKnightsMember(CUser * pUser, char * pBuf, BYTE command) {
    ...
    if (command == KNIGHTS_ADMIT ||
        command == KNIGHTS_REJECT) { // 기사단, 멤버가입 및 멤버거절, 장교 이상이 할 수 있습니다
...
void CKnightsManager::RecvCreateKnights(CUser * pUser, char * pBuf) {
    ...
    m_pMain->Send_Region(send_buff, send_index, pUser->m_pUserData->m_bZone, pUser->m_RegionX, pUser->m_RegionZ, NULL,
                         false);
    //pUser->Send( send_buff, send_index );
