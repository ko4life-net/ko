...
void CMagicProcess::MagicPacket(char * pBuf, int len) {
    int index = 0, send_index = 0, magicid = 0, sid = -1, tid = -2, data1 = 0, data2 = 0, data3 = 0, data4 = 0,
        data5 = 0, data6 = 0;
    char send_buff[128];
    ...
    if (sid >= NPC_BAND) {
        ...
        if (pUser->m_bResHpType == USER_DEAD || pUser->m_pUserData->m_sHp == 0) {
            TRACE("### Magic&Skill Fail : name=%s(%d), m_bResHpType=%d, hp=%d###\n", pUser->m_pUserData->m_id,
                  pUser->GetSocketID(), pUser->m_bResHpType, pUser->m_pUserData->m_sHp);
            return;
        }
    }
                ...
                if (pTUser->m_bType4Buff[pType4->bBuffType - 1] > 0) {
                    ...
                    if (m_bMagicState == CASTING) {
                        m_pMain->Send_Region(send_buff, send_index, m_pSrcUser->m_pUserData->m_bZone,
                                             m_pSrcUser->m_RegionX, m_pSrcUser->m_RegionZ, NULL, false);
                    } else {
    ...
    if (command == MAGIC_EFFECTING) {
        ...
        if (sid >= 0 && sid < MAX_USER) {
            if (tid >= NPC_BAND || (tid == -1 && (pMagic->bMoral == MORAL_AREA_ENEMY ||
                                                  pMagic->bMoral == MORAL_SELF_AREA))) { // If the target is an NPC.
                ...
                if (m_pSrcUser->m_pUserData->m_sItemArray[RIGHTHAND].nNum != 0) { // Does the magic user have a staff?
                    ...
                    if (pRightHand && m_pSrcUser->m_pUserData->m_sItemArray[LEFTHAND].nNum == 0 &&
                        ...
                        if (pMagic->bType1 == 3) {
                            //
                            total_magic_damage += ((pRightHand->m_sDamage * 0.8f) +
                                                   (pRightHand->m_sDamage * m_pSrcUser->m_pUserData->m_bLevel) / 60);
                            //
                            _MAGIC_TYPE3 * pType3 = NULL;
                            pType3 = m_pMain->m_Magictype3Array.GetData(magicid); // Get magic skill table type 4.
                            if (!pType3) {
        ...
        if (tid < -1 || tid >= MAX_USER) {
            return; // Make sure the target is another player and it exists.
        }
...
return_echo:
    ...
    if (sid >= 0 && sid < MAX_USER) {
        m_pMain->Send_Region(send_buff, send_index, m_pSrcUser->m_pUserData->m_bZone, m_pSrcUser->m_RegionX,
                             m_pSrcUser->m_RegionZ, NULL, false);
    } else if (sid >= NPC_BAND) {
        m_pMain->Send_Region(send_buff, send_index, pMon->m_sCurZone, pMon->m_sRegion_X, pMon->m_sRegion_Z, NULL,
                             false);
    }
}
...
_MAGIC_TABLE * CMagicProcess::IsAvailable(int magicid, int tid, int sid, BYTE type, int data1, int data2, int data3) {
    ...
    if (tid >= 0 && tid < MAX_USER) { // Target existence check routine for player.
        ...
        if (pTable->bType1 != 5) { // If not a Warp/Resurrection spell...
            ...
            if (pUser->m_bResHpType == USER_DEAD && pType->sNeedStone == 0 && pType->bExpRecover == 0) {
                goto fail_return;
            }
        }
    ...
    case MORAL_PARTY: // #4
        ...
        if (pUser) {
            if (pUser->m_sPartyIndex != m_pSrcUser->m_sPartyIndex) {
                goto fail_return;
            }
        }
        break;
    ...
    case MORAL_CLAN: // #14
        ...
        if (pUser) {
            if (pUser->m_pUserData->m_bKnights != m_pSrcUser->m_pUserData->m_bKnights) {
                goto fail_return;
            }
        }
        break;
...
BYTE CMagicProcess::ExecuteType1(int magicid, int sid, int tid, int data1, int data2,
...
BYTE CMagicProcess::ExecuteType2(int magicid, int sid, int tid, int data1, int data2, int data3) {
    ...
    if ((pow((sx - tx), 2) /* + pow((sy - ty), 2)*/ + pow((sz - tz), 2)) >
        total_range) { // Target is out of range, exit.
        result = 0;
        goto packet_send;
    }
    ...
    if (tid == -1) {                                             // If the target was the source's party....
        ...
        if (party_index == 0) { // If none of the members are in the region, return.
            ...
            if (!bFlag) {
                ...
                m_pMain->Send_Region(send_buff, send_index, pMon->m_sCurZone, pMon->m_sRegion_X, pMon->m_sRegion_Z,
                                     NULL, false);
            }
    ...
    for (int j = 0; j < party_index; j++) {                                 // THIS IS WHERE THE FUN STARTS!!!
        ...
        if ((pType->sFirstDamage < 0) && (pType->bDirectType == 1) &&
            ...
            damage = GetMagicDamage(sid, casted_member[j], pType->sFirstDamage,
                                    pType->bAttribute); // Get Magical damage point.
        } else {
            ...
            if (pTUser->m_bResHpType != USER_DEAD) { // 여기도 보호 코딩 했슴...
                ...
                for (int k = 0; k < MAX_TYPE3_REPEAT; k++) { // For continuous damages...
                    if (pTUser->m_bHPInterval[k] == 5) {
                        ...
                        pTUser->m_bHPAmount[k] =
                            duration_damage / (pTUser->m_bHPDuration[k] / pTUser->m_bHPInterval[k]);
...
void CMagicProcess::ExecuteType4(int magicid, int sid, int tid, int data1, int data2, int data3) {
    ...
    for (int j = 0; j < party_index; j++) {                                 // THIS IS WHERE THE FUN STARTS!!!
        ...
        if (pTUser->m_bType4Buff[pType->bBuffType - 1] == 2 &&
            tid == -1) { // Is this buff-type already casted on the player?
            result = 0;  // If so, fail!
            goto fail_return;
        }
        //
...
void CMagicProcess::ExecuteType5(int magicid, int sid, int tid, int data1, int data2, int data3) {
    ...
    if (!pTUser || (pTUser->m_bResHpType == USER_DEAD && pType->bType != RESURRECTION) ||
        (pTUser->m_bResHpType != USER_DEAD && pType->bType == RESURRECTION)) {
        return;
    }
                                 ...
                                 int data3) // Warp, resurrection, and summon spells.
{
    ...
    if (tid == -1) {                                             // If the target was the source's party...
        ...
        if (party_index == 0) {
            return; // If none of the members are in the region, return.
        }
    } else {                                                   // If the target was another single player.
        CUser * pTUser = NULL;                                 // Pointer initialization!
        pTUser = (CUser *)m_pMain->m_Iocport.m_SockArray[tid]; // Get target info.
...
*/
        ...
        case 12:                                                                    // Summon a target within the zone.
            ...
            SetShort(send_buff, (WORD)(m_pSrcUser->m_pUserData->m_curx * 10 /* + myrand(1,3) */),
        ...
        case 13:                                                                    // Summon a target outside the zone.
            ...
            pTUser->ZoneChange(m_pSrcUser->m_pUserData->m_bZone, m_pSrcUser->m_pUserData->m_curx,
                               m_pSrcUser->m_pUserData->m_curz);
        ...
        case 20: // Randomly teleport the source (within 20 meters)
            ...
            if (warp_x < 0.0f) {
                warp_x = 0.0f; // Make sure all positions are within range.
            }
            if (warp_x > 4096) {
                warp_x = 4096; // Change it if it isn't!!!
            }
            if (warp_z < 0.0f) {
                warp_z = 0.0f; // (Warp function does not check this!)
            }
            if (warp_z > 4096) {
                warp_z = 4096;
            }
    ...
    if (result != FAIL) { // In case of SUCCESS....
        ...
        if (sid >= NPC_BAND) {
            ...
            if (attribute != 4) { // Only if the staff has an attribute.
                damage = damage -
                         ((righthand_damage * 0.8f) + (righthand_damage * m_pSrcUser->m_pUserData->m_bLevel) / 60) -
                         ((attribute_damage * 0.8f) + (attribute_damage * m_pSrcUser->m_pUserData->m_bLevel) / 30);
            }
        }
        //
    }
...
BOOL CMagicProcess::UserRegionCheck(int sid, int tid, int magicid, int radius, short mousex, short mousez) {
    ...
    pTUser = (CUser *)m_pMain->m_Iocport.m_SockArray[tid]; // Get target info.
    if (!pTUser) {
        return FALSE; // Check if target exists and not already dead.
    }
    ...
    if (!bFlag) {                                                               // When players attack...
        if (pTUser->m_pUserData->m_bZone == m_pSrcUser->m_pUserData->m_bZone) { // Zone Check!
            if ((pTUser->m_RegionX == m_pSrcUser->m_RegionX) &&
                (pTUser->m_RegionZ == m_pSrcUser->m_RegionZ)) { // Region Check!
                if (radius != 0) {                              // Radius check! ( ...in case there is one :(  )
...
void CMagicProcess::Type4Cancel(int magicid, short tid) {
    ...
    case 7:
        if ((pTUser->m_bStrAmount + pTUser->m_bStaAmount + pTUser->m_bDexAmount + pTUser->m_bIntelAmount +
             pTUser->m_bChaAmount) > 0) {
            pTUser->m_sDuration7 = 0;
            pTUser->m_fStartTime7 = 0.0f;
            pTUser->m_bStrAmount = 0;
    ...
    case 8:
        if ((pTUser->m_bFireRAmount + pTUser->m_bColdRAmount + pTUser->m_bLightningRAmount + pTUser->m_bMagicRAmount +
             pTUser->m_bDiseaseRAmount + pTUser->m_bPoisonRAmount) > 0) {
            pTUser->m_sDuration8 = 0;
            pTUser->m_fStartTime8 = 0.0f;
            pTUser->m_bFireRAmount = 0;
            pTUser->m_bColdRAmount = 0;
