...
void CUser::Parsing(int len, char * pData) {
    ...
    if (m_fHPLastTimeNormal != 0.0f && (currenttime - m_fHPLastTimeNormal) > m_bHPIntervalNormal &&
...
void CUser::SelCharToAgent(char * pBuf) {
    ...
    wsprintf(logfile, "[SelCharToAgent : %d:%d:%d] - acname=%s, name=%s, count=%d, TH: %lu, Rear : %d\r\n", t.GetHour(),
             t.GetMinute(), t.GetSecond(), m_strAccountID, userid, m_pMain->m_iPacketCount, ::GetCurrentThreadId(),
...
void CUser::LogOut() {
    ...
    CUser * pUser = NULL;
    char    send_buf[256];
    memset(send_buf, NULL, 256);

    CTime t = CTime::GetCurrentTime();
    char  logfile[256];
    memset(logfile, 0x00, 256);
    wsprintf(logfile, "[%s : %s Logout : %d:%d:%d]\r\n", m_pUserData->m_Accountid, m_pUserData->m_id, t.GetHour(),
...
void CUser::MoveProcess(char * pBuf) {
    ...
    if (m_pMain->m_ZoneArray[m_iZoneIndex]->IsValidPosition(real_x, real_z, real_y) == FALSE) {
        return;
    }
    ...
    if (m_bResHpType == USER_DEAD || m_pUserData->m_sHp == 0) {
        if (speed != 0) {
            TRACE("### MoveProcess Fail : name=%s(%d), m_bResHpType=%d, hp=%d, speed=%d, x=%d, z=%d ###\n",
                  m_pUserData->m_id, m_Sid, m_bResHpType, m_pUserData->m_sHp, speed, (int)m_pUserData->m_curx,
                  (int)m_pUserData->m_curz);
        }
    }
...
*/
    ...
    if (m_bResHpType == USER_DEAD || m_pUserData->m_sHp == 0) {
        TRACE("### Attack Fail : name=%s(%d), m_bResHpType=%d, hp=%d###\n", m_pUserData->m_id, m_Sid, m_bResHpType,
              m_pUserData->m_sHp);
        return;
    }
    // 비러머글 해킹툴 유저 --;
    pTable = m_pMain->m_ItemtableArray.GetData(
        m_pUserData->m_sItemArray[RIGHTHAND].nNum); // This checks if such an item exists.
    if (!pTable && m_pUserData->m_sItemArray[RIGHTHAND].nNum != 0) {
        return;
    }
        ...
        if (!pTUser || pTUser->m_bResHpType == USER_DEAD || pTUser->m_bAbnormalType == ABNORMAL_BLINKING ||
            ...
            if (damage <= 0) {
                ...
                if (pTUser->m_pUserData->m_sHp == 0) {
                    ...
                    memset(buff, 0x00, 256);
                    send_index = 0;
                    if (pTUser->m_pUserData->m_bFame ==
                        COMMAND_CAPTAIN) { // 지휘권한이 있는 유저가 죽는다면,, 지휘 권한 박탈
                        ...
                        m_pMain->Send_Region(buff, send_index, pTUser->m_pUserData->m_bZone, pTUser->m_RegionX,
                                             pTUser->m_RegionZ);
    ...
    if (tid < NPC_BAND) {
        if (result == 0x02) {
            if (pTUser) {
                ...
                memset(buff, 0x00, 256);
                wsprintf(buff, "*** User Attack Dead, id=%s, result=%d, type=%d, HP=%d", pTUser->m_pUserData->m_id,
                         result, pTUser->m_bResHpType, pTUser->m_pUserData->m_sHp);
                TimeTrace(buff);
            }
        }
    }
...
void CUser::Chat(char * pBuf) {
    ...
    if (m_pUserData->m_bAuthority == 2) {
        return; // this user refused chatting
    }
    type = GetByte(pBuf, index);
    ...
    switch (type) {
    case GENERAL_CHAT:
        m_pMain->Send_NearRegion(send_buff, send_index, (int)m_pUserData->m_bZone, m_RegionX, m_RegionZ,
...
*/
    ...
    //
    memset(send_buff, NULL, 1024);
    send_index = 0; // 이 send_index는 왜 없었을까??? --;
    wsprintf(send_buff, "<------ User Regene ,, nid=%d, name=%s, type=%d ******", m_Sid, m_pUserData->m_id,
             m_bResHpType);
    //TimeTrace(send_buff);
    memset(send_buff, NULL, 1024);
    send_index = 0; // 이거 확인사살로 추가했어요!!!!
    ...
    memset(send_buff, NULL, 1024);
    send_index = 0; // Send Blinking State Change.....
...
void CUser::ZoneChange(int zone, float x, float z) {
    ...
    if (m_pMain->m_byBattleOpen == NATION_BATTLE) { // Battle zone open
            ...
            if (pMap->m_bType == 1 && m_pUserData->m_bNation != zone) { // 상대방 국가로 못넘어 가게..
                if (m_pUserData->m_bNation == KARUS && !m_pMain->m_byElmoradOpenFlag) {
                    TRACE("#### ZoneChange Fail ,,, id=%s, nation=%d, flag=%d\n", m_pUserData->m_id,
                          m_pUserData->m_bNation, m_pMain->m_byElmoradOpenFlag);
                    return;
                } else if (m_pUserData->m_bNation == ELMORAD && !m_pMain->m_byKarusOpenFlag) {
                    TRACE("#### ZoneChange Fail ,,, id=%s, nation=%d, flag=%d\n", m_pUserData->m_id,
                          m_pUserData->m_bNation, m_pMain->m_byKarusOpenFlag);
                    return;
                }
            }
        ...
        if (pMap->m_bType == 1 && m_pUserData->m_bNation != zone) { // 상대방 국가로 못넘어 가게..
            return;
                   ...
                   (zone == ZONE_FRONTIER ||
                    zone == ZONE_BATTLE)) { // You can't go to frontier zone when Battlezone is open.
            return;
        }
    ...
    if (m_pMain->m_nServerNo != pMap->m_nServerNo) {
        ...
        wsprintf(logfile, "[ZoneChange : %d-%d-%d] - sid=%d, acname=%s, name=%s, zone=%d, x=%d, z=%d \r\n", t.GetHour(),
                 t.GetMinute(), t.GetSecond(), m_Sid, m_strAccountID, m_pUserData->m_id, zone, (int)x, (int)z);
        EnterCriticalSection(&g_LogFile_critical);
        m_pMain->m_LogFile.Write(logfile, strlen(logfile));
        LeaveCriticalSection(&g_LogFile_critical);
...
void CUser::RegisterRegion() {
    ...
    if (m_RegionX != iRegX || m_RegionZ != iRegZ) {
        ...
        if (m_State == STATE_GAMESTART) {
            RemoveRegion(old_region_x - m_RegionX,
                         old_region_z - m_RegionZ); // delete user 는 계산 방향이 진행방향의 반대...
...
short CUser::GetDamage(short tid, int magicid) {
    ...
    if (tid < 0 || tid >= MAX_USER) {
        return -1; // Check if target id is valid.
    }
...
void CUser::LevelChange(short level, BYTE type) {
    ...
    if (type) {
        if ((m_pUserData->m_bPoints + m_pUserData->m_bSta + m_pUserData->m_bStr + m_pUserData->m_bDex +
             m_pUserData->m_bIntel + m_pUserData->m_bCha) < (300 + 3 * (level - 1))) {
            m_pUserData->m_bPoints += 3;
        }
        if (level > 9 && (m_pUserData->m_bstrSkill[0] + m_pUserData->m_bstrSkill[1] + m_pUserData->m_bstrSkill[2] +
                          m_pUserData->m_bstrSkill[3] + m_pUserData->m_bstrSkill[4] + m_pUserData->m_bstrSkill[5] +
                          m_pUserData->m_bstrSkill[6] + m_pUserData->m_bstrSkill[7] + m_pUserData->m_bstrSkill[8]) <
                             (2 * (level - 9))) {
...
void CUser::PointChange(char * pBuf) {
    ...
    switch (type) {
    case STR:
        if (m_pUserData->m_bStr == 0xFF) {
            return;
        }
        break;
    case STA:
        if (m_pUserData->m_bSta == 0xFF) {
            return;
        }
        break;
    case DEX:
        if (m_pUserData->m_bDex == 0xFF) {
            return;
        }
        break;
    case INTEL:
        if (m_pUserData->m_bIntel == 0xFF) {
            return;
        }
        break;
    case CHA:
        if (m_pUserData->m_bCha == 0xFF) {
            return;
        }
        break;
    }
...
void CUser::SetUserAbility() {
    ...
    if (m_pUserData->m_sItemArray[LEFTHAND].nNum != 0 && hitcoefficient == 0.0f) {
        pItem =
            m_pMain->m_ItemtableArray.GetData(m_pUserData->m_sItemArray[LEFTHAND].nNum); // 왼손 무기 : 활 적용을 위해
        if (pItem) {
            switch (pItem->m_bKind / 10) { // 무기 타입....
            case WEAPON_BOW:
            case WEAPON_LONGBOW:
...
*/
    ...
    m_sTotalEvasionrate =
        ((1 + p_TableCoefficient->Evasionrate * m_pUserData->m_bLevel * temp_dex) * m_sItemEvasionrate / 100) *
...
void CUser::ItemMove(char * pBuf) {
    ...
    case ITEM_INVEN_SLOT:
        ...
        if (!IsValidSlotPos(pTable, destpos)) {
            ...
            (pTable->m_bSlot == 0x00 &&
             destpos ==
                 RIGHTHAND)) { // 오른손전용 무기(또는 양손쓸수 있고 장착하려는 위치가 오른손) 인데 다른손에 두손쓰는 경우 체크
            if (m_pUserData->m_sItemArray[LEFTHAND].nNum != 0) {
                ...
                if (pTable2) {
                    if (pTable2->m_bSlot == 0x04) {
                        m_pUserData->m_sItemArray[RIGHTHAND].nNum =
                            m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nNum; // 오른손에 넣구..
                        m_pUserData->m_sItemArray[RIGHTHAND].sDuration =
                            m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sDuration;
                        m_pUserData->m_sItemArray[RIGHTHAND].sCount =
                            m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sCount;
                        m_pUserData->m_sItemArray[RIGHTHAND].nSerialNum =
                            m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nSerialNum;
                        ...
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nNum =
                            m_pUserData->m_sItemArray[LEFTHAND].nNum; // 왼손무기를 인벤으로 넣어준다.
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sDuration =
                            m_pUserData->m_sItemArray[LEFTHAND].sDuration;
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sCount =
                            m_pUserData->m_sItemArray[LEFTHAND].sCount;
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nSerialNum =
                            m_pUserData->m_sItemArray[LEFTHAND].nSerialNum;
                        ...
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nNum =
                            m_pUserData->m_sItemArray[destpos].nNum; // Swaping
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sDuration =
                            m_pUserData->m_sItemArray[destpos].sDuration;
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sCount = m_pUserData->m_sItemArray[destpos].sCount;
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nSerialNum =
                            m_pUserData->m_sItemArray[destpos].nSerialNum;
            ...
            (pTable->m_bSlot == 0x00 &&
             destpos ==
                 LEFTHAND)) { // 왼손전용 무기(또는 양손쓸수 있고 장착하려는 위치가 왼손) 인데 다른손에 두손쓰는 경우 체크
            if (m_pUserData->m_sItemArray[RIGHTHAND].nNum != 0) {
                ...
                if (pTable2) {
                    if (pTable2->m_bSlot == 0x03) {
                        ...
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nNum =
                            m_pUserData->m_sItemArray[RIGHTHAND].nNum; // 오른손무기를 인벤으로 넣어준다.
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sDuration =
                            m_pUserData->m_sItemArray[RIGHTHAND].sDuration;
                        m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sCount =
                            m_pUserData->m_sItemArray[RIGHTHAND].sCount;
    ...
    case ITEM_INVEN_INVEN:
        ...
        }
        {
            ...
            m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nSerialNum =
                m_pUserData->m_sItemArray[SLOT_MAX + destpos].nSerialNum;
            if (m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nSerialNum == 0) {
    ...
    case ITEM_SLOT_SLOT:
        ...
        if (m_pUserData->m_sItemArray[destpos].nNum != 0) {
            _ITEM_TABLE * pTable2 =
                m_pMain->m_ItemtableArray.GetData(m_pUserData->m_sItemArray[destpos].nNum); // dest slot exist some item
            if (pTable2) {
                if (pTable2->m_bSlot != 0x00) {
                    ...
                    m_pUserData->m_sItemArray[srcpos].sDuration =
                        m_pUserData->m_sItemArray[destpos].sDuration;                                     // Swaping
                    m_pUserData->m_sItemArray[srcpos].sCount = m_pUserData->m_sItemArray[destpos].sCount; // Swaping
                    m_pUserData->m_sItemArray[srcpos].nSerialNum =
                        m_pUserData->m_sItemArray[destpos].nSerialNum; // Swaping
...
*/
    ...
    if ((dir == ITEM_INVEN_SLOT) &&
        (destpos == HEAD || destpos == BREAST || destpos == SHOULDER || destpos == LEFTHAND || destpos == RIGHTHAND ||
         destpos == LEG || destpos == GLOVE || destpos == FOOT)) {
        UserLookChange(destpos, itemid, m_pUserData->m_sItemArray[destpos].sDuration); // 장착
    }
    if ((dir == ITEM_SLOT_INVEN) && (srcpos == HEAD || srcpos == BREAST || srcpos == SHOULDER || srcpos == LEFTHAND ||
                                     srcpos == RIGHTHAND || srcpos == LEG || srcpos == GLOVE || srcpos == FOOT)) {
...
void CUser::ItemTrade(char * pBuf) {
    ...
    if (m_bResHpType == USER_DEAD || m_pUserData->m_sHp == 0) {
        TRACE("### ItemTrade Fail : name=%s(%d), m_bResHpType=%d, hp=%d, x=%d, z=%d ###\n", m_pUserData->m_id, m_Sid,
              m_bResHpType, m_pUserData->m_sHp, (int)m_pUserData->m_curx, (int)m_pUserData->m_curz);
        result = 0x01;
        goto fail_return;
    }
    ...
    if (type == 0x01) { // buy sequence
        ...
        SendItemWeight();
        ItemLogToAgent(m_pUserData->m_id, pNpc->m_strName, ITEM_MERCHANT_BUY,
                       m_pUserData->m_sItemArray[SLOT_MAX + pos].nSerialNum, itemid, count, pTable->m_sDuration);
    } else { // sell sequence
        if (m_pUserData->m_sItemArray[SLOT_MAX + pos].nNum != itemid) {
            result = 0x02;
            goto fail_return;
...
void CUser::BundleOpenReq(char * pBuf) {
    ...
    if (m_RegionX < 0 || m_RegionZ < 0 || m_RegionX > pMap->GetXRegionMax() || m_RegionZ > pMap->GetZRegionMax()) {
        return;
    }
    ...
    if (pos != 0xFF) { // Common Item
        ...
        if (pGetUser->m_pUserData->m_sItemArray[SLOT_MAX + pos].nNum != 0) {
            if (pTable->m_bCountable != 1) {
                goto fail_return;
            } else if (pGetUser->m_pUserData->m_sItemArray[SLOT_MAX + pos].nNum != itemid) {
                goto fail_return;
            }
        }
        //
        ...
        pGetUser->ItemLogToAgent(pGetUser->m_pUserData->m_id, "MONSTER", ITEM_MONSTER_GET,
...
void CUser::LoyaltyChange(short tid) {
    ...
    char send_buff[256];
    memset(send_buff, NULL, 256);
    short level_difference = 0;
    short loyalty_source = 0;
    short loyalty_target = 0;

    CUser * pTUser = NULL;                                 // Pointer initialization!
    pTUser = (CUser *)m_pMain->m_Iocport.m_SockArray[tid]; // Get target info.
    if (!pTUser) {
        return; // Check if target exists and not already dead.
    }
    ...
    if (m_pUserData->m_iLoyalty < 0) {
        m_pUserData->m_iLoyalty = 0; // Cannot be less than zero.
    }
    if (pTUser->m_pUserData->m_iLoyalty < 0) {
...
void CUser::PartyProcess(char * pBuf) {
    ...
    memset(strid, 0x00, MAX_ID_SIZE + 1);
...
void CUser::PartyCancel() // 거절한 사람한테 온다... 리더를 찾아서 알려주는 함수
{
    ...
    if (leader_id < 0 || leader_id >= MAX_USER) {
        return;
    }
...
void CUser::PartyRequest(int memberid, BOOL bCreate) //리더에게 패킷이 온거다..
{
    ...
    if (memberid < 0 || memberid >= MAX_USER) {
        goto fail_return;
    }
    ...
    if (pUser->m_sPartyIndex != -1) {
        goto fail_return; //이미 파티 가입되어 있으면 안되징...
    }
    ...
    if (!bCreate) { // 기존의 파티에 추가하는 경우
        ...
        if (i == 8) {
            goto fail_return; // 파티 인원 Full
        }
    }

    if (bCreate) {
        if (m_sPartyIndex != -1) {
            goto fail_return; // (생성자)리더가 이미 파티 가입되어 있으면 안되징...
        }
...
void CUser::PartyInsert() // 본인이 추가 된다.  리더에게 패킷이 가는것이 아님
{
    ...
    for (int i = 0; i < 8; i++) { // Send your info to the rest of the party members.
        ...
        if (pParty->uid[i] < 0 || pParty->uid[i] >= MAX_USER) {
            continue;
        }
        pUser = (CUser *)m_pMain->m_Iocport.m_SockArray[pParty->uid[i]];
        if (!pUser) {
...
void CUser::PartyRemove(int memberid) {
    ...
    m_pMain->Send_PartyMember(m_sPartyIndex, send_buff,
                              send_index); // 삭제된 인원을 브로드캐스팅..제거될 사람한테두 패킷이 간다.
...
void CUser::ExchangeReq(char * pBuf) {
    ...
    if (m_bResHpType == USER_DEAD || m_pUserData->m_sHp == 0) {
        TRACE("### ExchangeProcess Fail : name=%s(%d), m_bResHpType=%d, hp=%d, x=%d, z=%d ###\n", m_pUserData->m_id,
              m_Sid, m_bResHpType, m_pUserData->m_sHp, (int)m_pUserData->m_curx, (int)m_pUserData->m_curz);
        goto fail_return;
    }
...
void CUser::ExchangeDecide() {
    ...
    if (!pUser->m_bExchangeOK) {
        ...
        if (bSuccess) {
            ...
            if (getmoney > 0) {
                ItemLogToAgent(m_pUserData->m_id, pUser->m_pUserData->m_id, ITEM_EXCHANGE_GET, 0, ITEM_GOLD, getmoney,
                               0);
            }
            if (putmoney > 0) {
                ItemLogToAgent(m_pUserData->m_id, pUser->m_pUserData->m_id, ITEM_EXCHANGE_PUT, 0, ITEM_GOLD, putmoney,
                               0);
            }
            ...
            for (Iter = pUser->m_ExchangeItemList.begin(); Iter != pUser->m_ExchangeItemList.end(); Iter++) {
                ...
                ItemLogToAgent(m_pUserData->m_id, pUser->m_pUserData->m_id, ITEM_EXCHANGE_GET, (*Iter)->nSerialNum,
                               (*Iter)->itemid, (*Iter)->count, (*Iter)->duration);
            }
            ...
            for (Iter = m_ExchangeItemList.begin(); Iter != m_ExchangeItemList.end(); Iter++) {
                ...
                ItemLogToAgent(m_pUserData->m_id, pUser->m_pUserData->m_id, ITEM_EXCHANGE_PUT, (*Iter)->nSerialNum,
                               (*Iter)->itemid, (*Iter)->count, (*Iter)->duration);
            }
...
BOOL CUser::ExecuteExchange() {
    ...
    for (Iter = pUser->m_ExchangeItemList.begin(); Iter != pUser->m_ExchangeItemList.end(); Iter++) {
        ...
        if ((*Iter)->itemid >= ITEM_NO_TRADE) {
            ...
            if (i == HAVE_MAX &&
                pTable->m_bCountable == 1) { // 중복 허용 아이템인데 기존에 가지고 있지 않은 경우 빈곳에 추가
                for (i = 0; i < HAVE_MAX; i++) {
...
void CUser::ClassChange(char * pBuf) {
    ...
    if (type == CLASS_CHANGE_REQ) { // 전직요청
        ...
        if (m_pUserData->m_bLevel < 30) {
            money = money * 0.4;
        } else if (m_pUserData->m_bLevel >= 30 && m_pUserData->m_bLevel < 60) {
            money = money * 1;
        ...
        if (sub_type == 1) { // 능력치 포인트
            if (m_pMain->m_sDiscount == 1 &&
                m_pMain->m_byOldVictory == m_pUserData->m_bNation) { // 할인시점이고 승리국이라면
                old_money = money;
                money = money * 0.5;
        ...
        for (int j = 0; j < 8; j++) { // Distribute loyalty amongst party members.
            if (pParty->uid[j] != -1 || pParty->uid[j] >= MAX_USER) {
                ...
                if (pUser->m_pUserData->m_iLoyalty < 0) {
                    pUser->m_pUserData->m_iLoyalty = 0; // Cannot be less than zero.
                }
...
void CUser::Dead() {
    ...
    memset(strKnightsName, 0x00, MAX_ID_SIZE + 1);
    CKnights *  pKnights = NULL;
    std::string buff;
    std::string buff2;
    ...
    memset(send_buff, NULL, 1024);
    wsprintf(send_buff, "----> User Dead ,, nid=%d, name=%s, type=%d, x=%d, z=%d ******", m_Sid, m_pUserData->m_id,
             m_bResHpType, (int)m_pUserData->m_curx, (int)m_pUserData->m_curz);
    //TimeTrace(send_buff);
            ...
            if (pTable) {
                if (pTable->m_bSlot != 2) { // 2 == DEFENCE ITEM
                    if (m_pUserData->m_sItemArray[RIGHTHAND].sDuration != 0) {
                        ...
                        ItemDurationChange(RIGHTHAND, pTable->m_sDuration,
                                           m_pUserData->m_sItemArray[RIGHTHAND].sDuration, worerate);
                    }
                }
            }
            ...
            if (pTable) {
                if (pTable->m_bSlot != 2) {
                    if (m_pUserData->m_sItemArray[LEFTHAND].sDuration != 0) {
                        ...
                        ItemDurationChange(LEFTHAND, pTable->m_sDuration, m_pUserData->m_sItemArray[LEFTHAND].sDuration,
                                           worerate);
                    }
                }
            }
        ...
        if (m_pUserData->m_sItemArray[BREAST].nNum != 0) {
            ...
            if (pTable) {
                if (m_pUserData->m_sItemArray[BREAST].sDuration != 0) {
                    ...
                    ItemDurationChange(BREAST, pTable->m_sDuration, m_pUserData->m_sItemArray[BREAST].sDuration,
                                       worerate);
                }
            }
        }
        ...
        if (m_pUserData->m_sItemArray[GLOVE].nNum != 0) {
            ...
            if (pTable) {
                if (m_pUserData->m_sItemArray[GLOVE].sDuration != 0) {
                    ...
                    ItemDurationChange(GLOVE, pTable->m_sDuration, m_pUserData->m_sItemArray[GLOVE].sDuration,
                                       worerate);
                }
            }
        }
...
void CUser::ItemRepair(char * pBuf) {
    ...
    money =
        (int)(((pTable->m_iBuyPrice - 10) / 10000.0f) + pow(pTable->m_iBuyPrice, 0.75)) * quantity / (double)durability;
...
void CUser::SendAllKnightsID() {
    ...
    char send_buff[4096];
    memset(send_buff, 0x00, 4096);
    char temp_buff[4096];
    memset(temp_buff, 0x00, 4096);
...
void CUser::ItemRemove(char * pBuf) {
    ...
    __int64 serial = 0;
    char    send_buff[128];
    memset(send_buff, 0x00, send_index);
...
void CUser::OperatorCommand(char * pBuf) {
    ...
    if (m_pUserData->m_bAuthority != 0) {
        return; // Is this user`s authority operator?
    }
...
void CUser::Type3AreaDuration(float currenttime) {
    ...
    if (m_fAreaLastTime != 0.0f && (currenttime - m_fAreaLastTime) > m_bAreaInterval) { // Did one second pass?
        ...
        if (((currenttime - m_fAreaStartTime) >= pType->sDuration) ||
            m_bResHpType == USER_DEAD) { // Did area duration end?
            m_bAreaInterval = 5;
            m_fAreaStartTime = 0.0f;
    ...
    SetShort(send_buff, 0, send_index);
    SetShort(send_buff, 0, send_index);
    m_pMain->Send_Region(send_buff, send_index, m_pUserData->m_bZone, m_RegionX, m_RegionZ, NULL,
                         false); // Send packet to region.
}
...
void CUser::WarehouseProcess(char * pBuf) {
    ...
    case WAREHOUSE_INPUT:
        ...
        if (m_pUserData->m_sWarehouseArray[reference_pos + destpos].nNum && !pTable->m_bCountable) {
            goto fail_return;
        }
        ...
        m_pUserData->m_sWarehouseArray[reference_pos + destpos].sDuration =
            m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sDuration;
        m_pUserData->m_sWarehouseArray[reference_pos + destpos].nSerialNum =
            m_pUserData->m_sItemArray[SLOT_MAX + srcpos].nSerialNum;
        ...
        if (pTable->m_bCountable) {
            ...
            m_pUserData->m_sWarehouseArray[reference_pos + destpos].sCount =
                m_pUserData->m_sItemArray[SLOT_MAX + srcpos].sCount;
        }
        ...
        SendItemWeight();
        ItemLogToAgent(m_pUserData->m_Accountid, m_pUserData->m_id, ITEM_WAREHOUSE_PUT, 0, itemid, count,
                       m_pUserData->m_sWarehouseArray[reference_pos + destpos].sDuration);
        break;
    case WAREHOUSE_OUTPUT:
        ...
        if (m_pUserData->m_sItemArray[SLOT_MAX + destpos].nNum && !pTable->m_bCountable) {
            goto fail_return;
        }
        ...
        m_pUserData->m_sItemArray[SLOT_MAX + destpos].nNum = itemid;
        m_pUserData->m_sItemArray[SLOT_MAX + destpos].sDuration =
            m_pUserData->m_sWarehouseArray[reference_pos + srcpos].sDuration;
        m_pUserData->m_sItemArray[SLOT_MAX + destpos].nSerialNum =
            m_pUserData->m_sWarehouseArray[reference_pos + srcpos].nSerialNum;
        if (pTable->m_bCountable) {
            ...
            m_pUserData->m_sItemArray[SLOT_MAX + destpos].sCount =
                m_pUserData->m_sWarehouseArray[reference_pos + srcpos].sCount;
        }
        if (!pTable->m_bCountable) {
        ...
        SendItemWeight();
        ItemLogToAgent(m_pUserData->m_id, m_pUserData->m_Accountid, ITEM_WAREHOUSE_GET, 0, itemid, count,
                       m_pUserData->m_sItemArray[SLOT_MAX + destpos].sDuration);
...
void CUser::InitType4() {
    ...
    //
    m_sDuration1 = 0;
    m_fStartTime1 = 0.0f; // Used for Type 4 Durational Spells.
    m_sDuration2 = 0;
    m_fStartTime2 = 0.0f;
    m_sDuration3 = 0;
    m_fStartTime3 = 0.0f;
...
void CUser::Home() {
    ...
    //
    else if (m_pUserData->m_bNation != m_pUserData->m_bZone && m_pUserData->m_bZone > 100 &&
             m_pUserData->m_bZone < 200) { // Battle Zone...
        x = pHomeInfo->BattleZoneX + myrand(0, pHomeInfo->BattleZoneLX);
        z = pHomeInfo->BattleZoneZ + myrand(0, pHomeInfo->BattleZoneLZ);
...
void CUser::FriendReport(char * pBuf) {
    int   index = 0;
    short usercount = 0, idlen = 0; // Basic Initializations.
    int   send_index = 0;
    char  send_buff[256];
    memset(send_buff, NULL, 256);
    char userid[MAX_ID_SIZE + 1];
...
void CUser::AllPointChange() {
    ...
    if (m_pUserData->m_bLevel < 30) {
        temp_money = temp_money * 0.4;
    } else if (m_pUserData->m_bLevel >= 30 && m_pUserData->m_bLevel < 60) {
        temp_money = temp_money * 1;
    } else if (m_pUserData->m_bLevel >= 60 && m_pUserData->m_bLevel <= 90) {
        temp_money = temp_money * 1.5;
    }
...
void CUser::GoldChange(short tid, int gold) {
    ...
    if (m_pUserData->m_bZone < 3) {
        return; // Money only changes in Frontier zone and Battle zone!!!
    }
    //
    //
    if (m_pUserData->m_bZone == ZONE_SNOW_BATTLE) {
    ...
    int  send_index = 0;
    BYTE s_type = 0;
    BYTE t_type = 0; // 1 -> Get gold    2 -> Lose gold
...
void CUser::PartyBBSRegister(char * pBuf) {
    ...
    if (m_sPartyIndex != -1) {
        goto fail_return; // You are already in a party!
    }
    if (m_bNeedParty == 2) {
        goto fail_return; // You are already on the BBS!
    }
    ...
    send_index = 0;
    memset(send_buff, NULL, 256); // Now, let's find out which page the user is on.
    for (i = 0; i < MAX_USER; i++) {
        pUser = (CUser *)m_pMain->m_Iocport.m_SockArray[i];
        if (!pUser) {
            continue; // Protection codes.
        }
        if (pUser->m_pUserData->m_bNation != m_pUserData->m_bNation) {
            continue;
        }
...
void CUser::PartyBBSDelete(char * pBuf) {
    ...
    if (m_bNeedParty == 1) {
        goto fail_return; // You don't need anymore
    }
...
void CUser::PartyBBSNeeded(char * pBuf, BYTE type) {
    ...
    memset(send_buff, NULL, 256);
    short page_index = 0;
    short start_counter = 0;
    BYTE  valid_counter = 0;
    int   i = 0, j = 0;
    short BBS_Counter = 0;
...
void CUser::MarketBBSRegister(char * pBuf) {
    ...
    for (i = 0; i < MAX_BBS_POST; i++) {
        if (buysell_index == MARKET_BBS_BUY) { // Buy
            ...
            goto fail_return; // Error
        }
    }

    if (result == 0) {
        goto fail_return; // No spaces available
    }
...
void CUser::KickOut(char * pBuf) {
    ...
    char accountid[MAX_ID_SIZE + 1];
    memset(accountid, 0x00, MAX_ID_SIZE + 1);
    char send_buff[256];
    memset(send_buff, 0x00, 256);
    CUser * pUser = NULL;
    idlen = GetShort(pBuf, index);
...
{                                    // (actually, this only takes care of the first event :(  )
    ...
    pNpc = m_pMain->m_arNpcArray.GetData(nid);
    if (!pNpc) {
        return; // Better to check than not to check ;)
    }
...
void CUser::ItemLogToAgent(const char * srcid, const char * tarid, int type, __int64 serial, int itemid, int count,
                           int durability) {
    int  send_index = 0, retvalue = 0;
    char send_buff[1024];
    memset(send_buff, 0x00, 1024);
...
BOOL CUser::GiveItem(int itemid, short count) {
    ...
    if (pos != 0xFF) { // Common Item
        ...
        if (m_pUserData->m_sItemArray[SLOT_MAX + pos].nNum != 0) {
            if (pTable->m_bCountable != 1) {
                return FALSE;
            } else if (m_pUserData->m_sItemArray[SLOT_MAX + pos].nNum != itemid) {
                return FALSE;
            }
        }
        /*    이건 필요할 때 주석 빼도록....
...
void CUser::KickOutZoneUser(BOOL home) {
    ...
    if (home) {
        ...
        if (random >= 0 && random < 3000) {
            ...
            yourmama = 1;
        } else if (random >= 6000 && random < 9001) {
            yourmama = 2;
        }
...
BOOL CUser::CheckEditBox() {
    ...
    memset(notepadid, 0x00, MAX_COUPON_ID_LENGTH);
    char postitid[MAX_COUPON_ID_LENGTH];
    memset(postitid, 0x00, MAX_COUPON_ID_LENGTH);
...
void CUser::RecvEditBox(char * pBuf) {
    ...
    int   index = 0;
    int   selevent = -1;
    short coupon_length = 0; // char coupon_id[MAX_COUPON_ID_LENGTH];
    char  send_buff[256];
    memset(send_buff, NULL, 256);
        ...
        if (m_pUserData->m_sItemArray[i].nNum == itemid) {
            if (!pTable->m_bCountable) { // Non-countable item. Automatically return TRUE
                ...
                if (m_pUserData->m_sItemArray[i].sCount >= min &&
                    m_pUserData->m_sItemArray[i].sCount <= max) { // Countable items. Make sure the amount is
                    return TRUE;                                  // same or higher.
                } else {
                    return FALSE;
                }
...
void CUser::RecvDeleteChar(char * pBuf) {
    ...
    memset(strCharID, 0x00, MAX_ID_SIZE + 1);
    char send_buff[256];
    memset(send_buff, 0x00, 256);
