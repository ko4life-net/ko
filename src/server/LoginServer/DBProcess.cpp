...
BOOL CDBProcess::LoadVersionList() {
    ...
    while (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
        ...
        if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
            ...
            pInfo->fsFile = szFile;
            pInfo->szPatchFileName = szPatchFileName;
...
BOOL CDBProcess::InsertVersion(short sVersion, const fs::path & fsFile, const char * szCompName,
    ...
    std::wstring szCompName2(szCompName, szCompName + strlen(szCompName));

    wchar_t szSQL[1024]{};
    swprintf_s(
        szSQL,
        L"INSERT INTO %s (sVersion, strFileName, strCompressName, sHistoryVersion) VALUES (%hd, N'%s', '%s', %hd)",
        szTableName.c_str(), sVersion, fsFile.c_str(), szCompName2.c_str(), sHistoryVersion);
...
BOOL CDBProcess::DeleteVersion(const fs::path & fsFile) {
    ...
    wchar_t szSQL[1024]{};
    swprintf_s(szSQL, L"DELETE FROM %s WHERE strFileName = N'%s'", szTableName.c_str(), fsFile.c_str());
