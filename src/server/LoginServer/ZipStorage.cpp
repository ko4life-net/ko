...
DWORD CZipStorage::GetFreeVolumeSpace() {
    ...
    if (!GetDiskFreeSpace(CZipArchive::GetDrive(m_file.GetFilePath()), &SectorsPerCluster, &BytesPerSector,
...
void CZipStorage::UpdateSpanMode(WORD uLastDisk) {
    ...
    if (uLastDisk) {
        ...
        if (m_iSpanMode == suggestedAuto) {
            m_iSpanMode = CZipArchive::IsDriveRemovable(m_file.GetFilePath()) ? pkzipSpan : tdSpan;
        } else {
            m_iSpanMode = tdSpan;
        }
