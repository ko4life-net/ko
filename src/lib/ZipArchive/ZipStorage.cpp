...
void CZipStorage::Open(LPCTSTR szPathName, int iMode, int iVolumeSize) {
    ...
    {
        OpenFile(szPathName, CFile::modeNoTruncate |
                                 ((iMode == CZipArchive::openReadOnly) ? CFile::modeRead : CFile::modeReadWrite));
...
void CZipStorage::ChangeDisk(int iNumber) {
    ...
    OpenFile(m_iSpanMode == pkzipSpan ? ChangePkzipRead() : ChangeTdRead(), CFile::modeNoTruncate | CFile::modeRead);
}
...
DWORD CZipStorage::GetFreeVolumeSpace() {
    ...
    if (!GetDiskFreeSpace(CZipArchive::GetDrive(m_pFile->GetFilePath()), &SectorsPerCluster, &BytesPerSector,
...
void CZipStorage::UpdateSpanMode(WORD uLastDisk) {
    ...
    if (uLastDisk) {
        ...
        if (m_iSpanMode == suggestedAuto) {
            m_iSpanMode = CZipArchive::IsDriveRemovable(m_pFile->GetFilePath()) ? pkzipSpan : tdSpan;
        } else {
            m_iSpanMode = tdSpan;
        }
