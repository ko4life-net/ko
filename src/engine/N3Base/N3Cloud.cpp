...
void CN3Cloud::Tick() {
    ...
    if (fCloudLayer >= 0.05) {
        ...
        if (m_pVertices[0].tu > 10.0f) {
            for (int i = 0; i < NUM_CLOUD_VERTEX; ++i) {
                m_pVertices[i].tu -= 10.0f;
            }
        }
        if (m_pVertices[0].tv > 10.0f) {
            for (int i = 0; i < NUM_CLOUD_VERTEX; ++i) {
                m_pVertices[i].tv -= 10.0f;
            }
        }
    }
...
void CN3Cloud::Render() {
    ...
    s_lpD3DDev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, 8, 10, CloudIndex, D3DFMT_INDEX16, m_pVertices,
    ...
    if (CLOUD_NONE != m_eCloud3) {
        ...
        if (Alpha < color2) {
            color2 = (Alpha & 0xff000000) |
                     (color2 &
                      0x00ffffff); // 기존 색 변화의 alpha값이 구름 교체alpha값보다 큰 경우 구름 교체 alpha값으로 대체
        }
        // render cloud 2
        for (int i = 0; i < 4; ++i) {
        ...
        // render cloud 3
        D3DCOLOR color3 =
            ((0xff - (color2 >> 24)) << 24) | (color2 & 0x00ffffff); // color2의 alpha값을 0xff에서 뺀 값으로 바꿈
        for (int i = 4; i < NUM_CLOUD_VERTEX; ++i) {
...
void CN3Cloud::Init(const std::string * pszFNs) {
    ...
    */
    ...
    const float fSmallHeight =
        fBigHeight + (fBigLenth - fSmallLength) * fSqrt3; // 구름 절두면체의 위 높이(지평선 0.0f 기준)
    ...
    m_pVertices[3].Set(-fBigLenth, fBigHeight, fBigLenth, BigColor, fTexUVLeft, fTexUVBottom, fTexUVLeft, fTexUVBottom);
    // small
    m_pVertices[4].Set(-fSmallLength, fSmallHeight, -fSmallLength, SmallColor, fTexUVLeft + fTexOffsetU,
                       fTexUVTop + fTexOffsetV, fTexUVLeft + fTexOffsetU, fTexUVTop + fTexOffsetV);
    m_pVertices[5].Set(fSmallLength, fSmallHeight, -fSmallLength, SmallColor, fTexUVRight - fTexOffsetU,
                       fTexUVTop + fTexOffsetV, fTexUVRight - fTexOffsetU, fTexUVTop + fTexOffsetV);
    m_pVertices[6].Set(fSmallLength, fSmallHeight, fSmallLength, SmallColor, fTexUVRight - fTexOffsetU,
                       fTexUVBottom - fTexOffsetV, fTexUVRight - fTexOffsetU, fTexUVBottom - fTexOffsetV);
    m_pVertices[7].Set(-fSmallLength, fSmallHeight, fSmallLength, SmallColor, fTexUVLeft + fTexOffsetU,
                       fTexUVBottom - fTexOffsetV, fTexUVLeft + fTexOffsetU, fTexUVBottom - fTexOffsetV);
}

void CN3Cloud::SetCloud(e_CLOUDTEX eCloud1, e_CLOUDTEX eCloud2, float fSec) {
    if (CLOUD_NONE == eCloud1 || CLOUD_NONE == eCloud2) {
        return;
    }
    ...
    if (eCloud1 == m_eCloud1) {
        ++iSameCount;
    } else if (eCloud1 == m_eCloud2) {
        ++iSameCount;
    }
    if (eCloud2 == m_eCloud1) {
        ++iSameCount;
    } else if (eCloud2 == m_eCloud2) {
        ++iSameCount;
    }
