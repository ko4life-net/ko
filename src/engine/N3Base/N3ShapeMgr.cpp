    ...
    if (iSC > 0) {
        ...
        for (int i = 0; i < iSC; i++) {
            ...
            if (dwType & OBJ_SHAPE_EXTRA) {
                pShape = new CN3ShapeExtra(); // 성문등 확장된 Object 로 쓸경우..
            } else {
                pShape = new CN3Shape();
            }
            ...
            if (pShape->m_iEventID) //  ID 가 있는 오브젝트 ... NPC 로 쓸수 있다..
            {
                ...
                TRACE("  Load OBject Event : ID(%d) Type(%d) CtrlID(%d) Status(%d)\n", pShape->m_iEventID,
                      pShape->m_iEventType, pShape->m_iNPC_ID, pShape->m_iNPC_Status);
...
bool CN3ShapeMgr::Save(HANDLE hFile) {
    if (false == SaveCollisionData(hFile)) {
        return false;
    }
    ...
    for (int i = 0; i < iSC; i++) {
        ...
        if (m_Shapes[i]->m_iEventID || m_Shapes[i]->m_iEventType || m_Shapes[i]->m_iNPC_ID ||
            m_Shapes[i]->m_iNPC_Status) {
        ...
        for (int j = 0; j < 3; j++) // 걸쳐 있는 메시 만큼 생성...
        {
            ...
            for (int z = zz1; z <= zz2; z++) // 범위만큼 처리..
            {
                ...
                for (int x = xx1; x <= xx2; x++) {
                    ...
                    bool bWriteID = false;
                    if (dwOC0 & dwOC1) {
                        bWriteID = false; // 두 끝점이 같은 변의 외부에 있다.
                    } else if (dwOC0 == 0 && dwOC1 == 0) {
                        bWriteID = true; // 선분이 사각형 내부에 있음
                    } else if ((dwOC0 == 0 && dwOC1 != 0) || (dwOC0 != 0 && dwOC1 == 0)) {
                        bWriteID = true;             // 선분 한점은 셀의 내부에 한점은 외부에 있음.
                    } else if ((dwOC0 & dwOC1) == 0) // 두 꿑점 모두 셀 외부에 있지만 판단을 다시 해야 한다.
                    {
                        float fXCross =
                            vEdge[j][0].x + (fZMax - vEdge[j][0].z) * (vEdge[j][1].x - vEdge[j][0].x) /
                                                (vEdge[j][1].z - vEdge[j][0].z); // 위의 변과의 교차점을 계산하고..
                        if (fXCross < fXMin) {
                    ...
                    if (false == bWriteID) // 만약 걸린게 없다면... 위에서 직선을 쏘아서 충돌인지 체크한다.
                    {
                        ...
                        for (float fZ = zz3; fZ <= zz4 && false == bWriteID; fZ += 0.25f) {
                            for (float fX = xx3; fX <= xx4 && false == bWriteID; fX += 0.25f) {
                                ...
                                if (::_IntersectTriangle(vPos, vDir, m_pvCollisions[i * 3], m_pvCollisions[i * 3 + 1],
                                                         m_pvCollisions[i * 3 + 2])) // 폴리곤 충돌 체크..
                                {
                                 ...
                                 __Vector3 *       pVec)               // 충돌한 면 의 폴리곤 __Vector3[3]
{
    ...
    for (int i = 0; i < iSubCellCount; i++) {
        ...
        for (int j = 0; j < ppCells[i]->nCCPolyCount; j++) {
            ...
            if (false == ::_IntersectTriangle(vPos, vDir, m_pvCollisions[nIndex0], m_pvCollisions[nIndex1],
                                              m_pvCollisions[nIndex2], fT, fU, fV, &vColTmp)) {
                continue;
            }
            if (false == ::_IntersectTriangle(vPosNext, vDir, m_pvCollisions[nIndex0], m_pvCollisions[nIndex1],
                                              m_pvCollisions[nIndex2])) {
    ...
    if (fDistClosest != FLT_MAX) {
        ...
        if (m_ShapesToRender.size() > 0) {
            ...
            for (int i = 0; it != itEnd; it++, i++) {
                pShape = *it;
                if (NULL == pShape->CollisionMesh()) {
                    continue;
                }
                if ((pShape->Pos() - vPos).Magnitude() > pShape->Radius() * 2) {
                    continue; // 멀리 떨어진거면 지나간다..
                }
            ...
            int iSC = Shapes.size();
            if (iSC > 1) {
                qsort(&(Shapes[0]), iSC, 4, SortByCameraDistance); // 카메라 거리에 따라 정렬하고..
            }

            CN3VMesh * pVMesh = NULL;
            for (int i = 0; i < iSC; i++) {
                pVMesh = Shapes[i]->CollisionMesh();
                if (true == pVMesh->CheckCollision(Shapes[i]->m_Matrix, vPos, vPosNext, pvCol, pvNormal)) {
                    return true;
                }
            }
...
bool CN3ShapeMgr::CheckCollisionCamera(__Vector3 & vEyeResult, const __Vector3 & vAt, float fNP) {
    ...
    if (false == this->CheckCollision(vAt, vDir, fD, &vCol)) {
        return false;
    }

    float fDelta = (vEyeResult - vCol).Magnitude(); // 충돌점과 처다보는 거리를 재보고..
    if (fDelta < fNP * 2.0f) {
        return false; // 너무 가까이 붙으면 돌아간다..
    }
...
void CN3ShapeMgr::RenderCollision(const __Vector3 & vPos) {
    ...
    for (int i = 0; i < 9; i++) {
        if (NULL == ppCell[i] || ppCell[i]->nCCPolyCount <= 0) {
            continue;
        }
...
CN3Shape * CN3ShapeMgr::Pick(int iXScreen, int iYScreen, bool bMustHaveEvent, __Vector3 * pvPick) {
    ...
    for (int i = 0; i < iSC; i++) {
        if (bMustHaveEvent && Shapes[i]->m_iEventID <= 0) {
            continue; // 이벤트가 있어야 한다면...
        }
...
int CN3ShapeMgr::SubCellPathThru(const __Vector3 & vFrom, const __Vector3 & vAt, int iMaxSubCell,
                                 __CellSub ** ppSubCells) // 벡터 사이에 걸친 셀포인터 돌려준다..
{
    if (NULL == ppSubCells) {
        return 0;
    }
    ...
    for (int z = zz1; z <= zz2; z++) // 범위만큼 처리..
    {
        ...
        for (int x = xx1; x <= xx2; x++) {
            ...
            bPathThru = false;
            if (dwOC0 & dwOC1) {
                bPathThru = false; // 두 끝점이 같은 변의 외부에 있다.
            } else if (dwOC0 == 0 && dwOC1 == 0) {
                bPathThru = true; // 선분이 사각형 내부에 있음
            } else if ((dwOC0 == 0 && dwOC1 != 0) || (dwOC0 != 0 && dwOC1 == 0)) {
                bPathThru = true;            // 선분 한점은 셀의 내부에 한점은 외부에 있음.
            } else if ((dwOC0 & dwOC1) == 0) // 두 꿑점 모두 셀 외부에 있지만 판단을 다시 해야 한다.
            {
                float fXCross = vFrom.x + (fZMax - vFrom.z) * (vAt.x - vFrom.x) /
                                              (vAt.z - vFrom.z); // 위의 변과의 교차점을 계산하고..
                if (fXCross < fXMin) {
            ...
            int nZ = z / CELL_MAIN_DEVIDE;
            if (nX < 0 || nX >= MAX_CELL_MAIN || nZ < 0 && nZ >= MAX_CELL_MAIN) {
                continue; // 메인셀바깥에 있음 지나간다.
            }
            if (NULL == m_pCells[nX][nZ]) {
                continue; // 메인셀이 널이면 지나간다..
            }
...
float CN3ShapeMgr::GetHeightNearstPos(
    const __Vector3 & vPos, float fDist,
    __Vector3 * pvNormal) // 가장 가까운 높이값을 돌려준다. 없으면 -FLT_MAX 을 돌려준다.
{
    __CellSub * pCell = this->SubCell(vPos.x, vPos.z); // 서브셀을 가져온다..
    if (NULL == pCell || pCell->nCCPolyCount <= 0) {
        return -FLT_MAX; // 없음 말자.
    }

    __Vector3 vPosV = vPos;
    vPosV.y = 5000.0f;          // 꼭대기에 위치를 하고..
    ...
    for (int i = 0; i < pCell->nCCPolyCount; i++) {
        ...
        // 충돌된 점이 있으면..
        if (true == ::_IntersectTriangle(vPosV, vDir, m_pvCollisions[nIndex0], m_pvCollisions[nIndex1],
                                         m_pvCollisions[nIndex2], fT, fU, fV, &vColTmp)) {
...
float CN3ShapeMgr::GetHeight(float fX, float fZ,
                             __Vector3 * pvNormal) // 가장 높은 곳을 돌려준다.. 없으면 -FLT_MAX값을 돌려준다.
{
