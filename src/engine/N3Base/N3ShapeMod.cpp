...
CN3SPart * CN3ShapeMod::GetPartByPMeshFileName(const fs::path & fsFile) {
    if (fsFile.empty()) {
        return NULL;
    }
...
BOOL CN3ShapeMod::LoadStateInfo(FILE * stream) // 상태 정보를 읽어온다.(text로부터)
{
    ...
    for (int i = 0; i < m_iModPartCount; ++i) {
        ...
        for (int j = 0; j < m_iStateCount; ++j) {
            ...
            __ASSERT(result != EOF, "잘못된 N3ShapeMod 세팅 파일");
            result = fscanf(stream, "Rot(%f, %f, %f, %f)\n", &vAxis.x, &vAxis.y, &vAxis.z, &fDegree);
            __ASSERT(result != EOF, "잘못된 N3ShapeMod 세팅 파일");
            if (vAxis.x == 0.0f && vAxis.y == 0.0f && vAxis.z == 0.0f) {
                qRot.Identity();
            } else {
                qRot.RotationAxis(vAxis, D3DXToRadian(fDegree));
            }
            ...
            if (vPos.x != 0.0f || vPos.y != 0.0f || vPos.z != 0.0f) {
                m_pModParts[i].bPos = true;
            }
            if (qRot.w != 0.0f) {
                m_pModParts[i].bRot = true;
            }
            if (vScale.x != 1.0f || vScale.y != 1.0f || vScale.z != 1.0f) {
                m_pModParts[i].bScale = true;
            }
        }
    }
    ...
    if (iPartCount > 0) {
        m_pMatchPart2ModPart = new __ModPart *[iPartCount];
    }
    ...
    if (m_fTimeChanged > 0.0f) {
        ...
        if (m_fTimeChanging > m_fTimeChanged) {
            ...
            for (int i = 0; i < m_iModPartCount; ++i) {
                pMP = m_pModParts + i;
                if (pMP->bPos) {
                    pMP->CurStateInfo.vPos =
                        pMP->pStateInfos[m_iPrevState].vPos +
                        (pMP->pStateInfos[m_iCurState].vPos - pMP->pStateInfos[m_iPrevState].vPos) * m_fTimeChanging /
                            m_fTimeChanged;
                }
                if (pMP->bRot) {
                    pMP->CurStateInfo.qRot.Slerp(pMP->pStateInfos[m_iPrevState].qRot,
                                                 pMP->pStateInfos[m_iCurState].qRot, m_fTimeChanging / m_fTimeChanged);
                }
                if (pMP->bScale) {
                    pMP->CurStateInfo.vScale =
                        pMP->pStateInfos[m_iPrevState].vScale +
                        (pMP->pStateInfos[m_iCurState].vScale - pMP->pStateInfos[m_iPrevState].vScale) *
                            m_fTimeChanging / m_fTimeChanged;
                }
            }
        }
    }
...
BOOL CN3ShapeMod::SetState(int iState, float fSec) {
    if (iState < 0 || iState >= m_iStateCount) {
        return FALSE;
    }
    if (m_iCurState == iState) {
        return FALSE;
    }
    m_fTimeChanged = fSec;
    m_fTimeChanging = 0.0f;
