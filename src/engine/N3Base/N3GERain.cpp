...
// vVelocity : 빗줄기의 속도
{
    ...
    for (int i = 0; i < iRainCount; ++i) {
        pVertices[i * 2 + 0].Set(fWidth * (rand() % 10000 - 5000) / 10000.f,
                                 ...
                                 fWidth * (rand() % 10000 - 5000) / 10000.f, dwColorA);
        pVertices[i * 2 + 1].Set(pVertices[i * 2 + 0].x + vAdd.x, pVertices[i * 2 + 0].y + vAdd.y,
                                 pVertices[i * 2 + 0].z + vAdd.z, dwColorB);
    }
    m_pVB->Unlock();
}
