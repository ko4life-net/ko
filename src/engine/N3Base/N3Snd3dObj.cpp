...
void CN3Snd3dObj::SetMaxDistance(D3DVALUE max) {
    if (m_pDS3DBuff) {
        m_pDS3DBuff->SetMaxDistance(max, DS3D_IMMEDIATE);
    }
}
...
void CN3Snd3dObj::SetMinDistance(D3DVALUE min) {
    if (m_pDS3DBuff) {
        m_pDS3DBuff->SetMinDistance(min, DS3D_IMMEDIATE);
    }
}
...
void CN3Snd3dObj::SetConeOutSizeVolume(LONG vol) {
    if (m_pDS3DBuff) {
        m_pDS3DBuff->SetConeOutsideVolume(vol, DS3D_IMMEDIATE);
    }
}
...
void CN3Snd3dObj::SetConeOrientation(D3DVECTOR * pDir) {
    if (m_pDS3DBuff) {
        m_pDS3DBuff->SetConeOrientation(pDir->x, pDir->y, pDir->z, DS3D_IMMEDIATE);
    }
}
