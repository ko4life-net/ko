...
BOOL CN3UIEdit::CreateEditWindow(HWND hParent, RECT rect) {
    ...
    s_hWndEdit = CreateWindow("EDIT", "EditWindow", WS_CHILD | WS_TABSTOP | ES_LEFT | ES_WANTRETURN, rect.left,
                              rect.top, rect.right - rect.left, rect.bottom - rect.top, hParent, NULL, NULL, NULL);
...
LRESULT APIENTRY CN3UIEdit::EditWndProc(HWND hWnd, WORD Message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
        ...
        if (s_pFocusedEdit) {
            CN3UIEdit::UpdateCaretPosFromEditCtrl(false);
        }
        return 0;
        //break;
...
void CN3UIEdit::SetCaretPos(UINT nPos) {
    ...
    SIZE size = {0, 0};
    if (!szBuff.empty() && m_pBuffOutRef) {
        m_pBuffOutRef->GetTextExtent(szBuff, m_nCaretPos, &size);
    }
...
void CN3UIEdit::SetString(const std::string & szString) {
    ...
    if (szString.size() > m_iMaxStrLen) {
        ...
        if (IsHangulMiddleByte(szString.c_str(), m_iMaxStrLen)) {
            szNewBuff = szString.substr(
                0, m_iMaxStrLen - 1); // -1은 한글이므로 하나 덜 카피하기 위해 +1은 맨 마지막에 NULL 넣기 위해
            if (UISTYLE_EDIT_PASSWORD & m_dwStyle) {
...
bool CN3UIEdit::Load(HANDLE hFile) {
    if (false == CN3UIStatic::Load(hFile)) {
        return false;
    }
...
bool CN3UIEdit::Save(HANDLE hFile) {
    if (false == CN3UIStatic::Save(hFile)) {
        return false;
    }
...
fs::path CN3UIEdit::GetSndFName_Typing() const {
    if (m_pSnd_Typing) {
...
void CN3UIEdit::UpdateTextFromEditCtrl() {
    if (NULL == s_pFocusedEdit || NULL == s_hWndEdit) {
        return;
    }
