...
BOOL CStreamSoundObj::InitWriteBuffer() {
    ...
    hResult =
        m_pDSBuff->Lock(Offset, m_BlockSize * 2, &pSoundBlock1, &byteSoundBlock1, &pSoundBlock2, &byteSoundBlock2, 0);
    if (FAILED(hResult)) {
        return FALSE;
    }
...
BOOL CStreamSoundObj::WriteBuffer() {
    ...
    hResult = m_pDSBuff->Lock(Offset, m_BlockSize, &pSoundBlock1, &byteSoundBlock1, &pSoundBlock2, &byteSoundBlock2, 0);
    if (FAILED(hResult)) {
        return FALSE;
    }
