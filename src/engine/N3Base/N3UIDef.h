...
const DWORD UI_MOUSEPROC_CHILDDONESOMETHING =
    0x00000002; // 자식이 먼가 일을 했다.(이 플래그가 설정되어 있으면 항상 UI_MOUSEPROC_DONESOMETHING도 설정되어있다.)
...
struct __FLOAT_RECT {
    ...
    BOOL PtInRect3D(float x, float y) const {
        return ((x >= left && x <= right && y >= bottom && y <= top) ? TRUE : FALSE);
    }
};
