...
CN3CPartSkins * CN3CPart::SkinsSet(const fs::path & fsFile) {
    s_MngSkins.Delete(&m_pSkinsRef);
    m_pSkinsRef = s_MngSkins.Get(fsFile, TRUE);
...
bool CN3CPart::Load(HANDLE hFile) {
    ...
    if (nL > 0) {
        ...
        ReadFile(hFile, szFile.data(), nL, &dwRWC, NULL);
        s_MngSkins.Delete(&m_pSkinsRef);
        m_pSkinsRef = s_MngSkins.Get(szFile);
    }
...
bool CN3CPart::Save(HANDLE hFile) {
    ...
    if (m_pTexRef) {
        szFile = m_pTexRef->FilePathWin().string();
        nL = szFile.length();
    } else {
        nL = 0;
    }
    ...
    if (nL > 0) {
        WriteFile(hFile, szFile.c_str(), nL, &dwRWC, NULL);
    }

    if (m_pSkinsRef) {
        szFile = m_pSkinsRef->FilePathWin().string();
        nL = szFile.length();
    } else {
        nL = 0;
    }
...
void CN3CPart::Render(int nLOD) {
    ...
    if (NULL == m_pSkinsRef || m_pSkinsRef->m_Skins[nLOD].VertexCount() <= 0) {
        return;
    }
...
CN3Texture * CN3CPart::TexOverlapSet(const fs::path & fsFile) {
    if (m_pTexOverlapRef && n3std::iequals(m_pTexOverlapRef->FilePath(), fsFile)) {
        return m_pTexOverlapRef;
    }

    s_MngTex.Delete(&m_pTexOverlapRef);
    m_pTexOverlapRef = s_MngTex.Get(fsFile, true, s_Options.iTexLOD_Chr);
    return m_pTexOverlapRef;
}

void CN3CPart::TexOverlapSet(CN3Texture * pTex) {
    if (pTex == m_pTexOverlapRef) {
        return;
    }
...
void CN3CPlugBase::PMeshSet(const fs::path & fsFile) {
    m_PMeshInst.Create(fsFile);
}
...
void CN3CPlugBase::Render(const __Matrix44 & mtxParent, const __Matrix44 & mtxJoint) {
    ...
    if (m_PMeshInst.GetNumVertices() <= 0) {
        return;
    }
...
CN3Texture * CN3CPlugBase::TexOverlapSet(const fs::path & fsFile) {
    // if(m_pTexOverlapRef && m_pTexOverlapRef->FilePath() == fsFile) return m_pTexOverlapRef;
    if (m_pTexOverlapRef && n3std::iequals(m_pTexOverlapRef->FilePath(), fsFile)) {
        return m_pTexOverlapRef;
    }
...
e_PlugType CN3CPlugBase::GetPlugTypeByFileName(const fs::path & fsFile) {
    if (fsFile.empty()) {
        return PLUGTYPE_UNDEFINED;
    }
    // berserk
    // 일단 확장자로 구분한다. 별로 좋은 방법 같지는 않지만.. N3CPlug, N3CPlug_Cloak
    std::string szFile = fsFile.string();
    if (szFile.ends_with("ug")) { // PLUGTYPE_NORMAL
        return PLUGTYPE_NORMAL;
    } else if (szFile.ends_with("ak")) {
...
bool CN3CPlug::Load(HANDLE hFile) {
    ...
    if (iUseVMesh) {
        ...
        // TODO: FIXME: this can overflow the longer the game runs
        // Note that they added this because s_MngPMesh doesn't allow adding files without
        // names, since it uses them in std::map container as keys.
        static int iSN = 0;
        pPMesh->FilePathSet(std::format("Temp_Plug_{:d}.n3pmesh", iSN++));
        s_MngPMesh.Add(pPMesh);
        m_PMeshInstFX.Create(pPMesh); // FX 에 쓸 PMesh Instance
    }

    m_fsFXMainFile.clear();
    m_fsFXTailFile.clear();
    InitFX(m_fsFXMainFile, m_fsFXTailFile, 0xffffffff);

    return true;
}

void CN3CPlug::InitFX(const fs::path & fsMainFile, const fs::path & fsTailFile, D3DCOLOR TraceCR) {
    if (fsMainFile.empty()) {
        if (m_pFXMainBundle) {
            delete m_pFXMainBundle;
        }
        m_pFXMainBundle = NULL;
        m_fsFXMainFile = fsMainFile;
    } else if (!m_pFXMainBundle || fsMainFile != m_pFXMainBundle->FilePath()) {
        m_fsFXMainFile = fsMainFile;
        if (m_pFXMainBundle) {
            delete m_pFXMainBundle;
        }
        m_pFXMainBundle = new CN3FXBundle;
        if (!m_pFXMainBundle->LoadFromFile(m_fsFXMainFile)) {
            delete m_pFXMainBundle;
            m_pFXMainBundle = NULL;
            m_fsFXMainFile.clear();
    ...
    } else if (!m_pFXTailBundle[0] || fsTailFile != m_pFXTailBundle[0]->FilePath()) {
        m_fsFXTailFile = fsTailFile;
        if (m_pFXTailBundle[0]) {
            delete m_pFXTailBundle[0];
        }
        m_pFXTailBundle[0] = new CN3FXBundle;
        if (!m_pFXTailBundle[0]->LoadFromFile(m_fsFXTailFile)) {
            delete m_pFXTailBundle[0];
            ...
            if (pMesh) {
                vInterval = m_PMeshInstFX.GetMesh()->Max() - m_PMeshInstFX.GetMesh()->Min();
            }
...
bool CN3CPlug::Save(HANDLE hFile) {
    ...
    if (iUseVMesh) {
        m_PMeshInstFX.GetMesh()->Save(hFile);
    }
...
#ifdef _N3TOOL
void CN3CPlug::ImportPMesh(const fs::path & fsFile) {
    if (fsFile.empty()) {
        return;
    }
    m_PMeshInstFX.Release();
    m_PMeshInstFX.Create(fsFile);
}
#endif
...
void CN3CPlug::RenderFX(const __Matrix44 & mtxParent, const __Matrix44 & mtxJoint) {
    if (!m_pFXMainBundle || !m_pFXPart) {
        return;
    }
    if (!m_PMeshInstFX.GetVertices()) {
        return;
    }
    __VertexT1 * pvAxis = m_PMeshInstFX.GetVertices();
    //if(m_pFXMainBundle->FilePath() != m_fsFXMainFile) InitFX(m_fsFXMainFile, m_fsFXTailFile);
...
bool CN3Chr::Load(HANDLE hFile) {
    ...
    for (int i = 0; i < iPC; i++) {
        ...
        if (nL > 0) {
            szFile.assign(nL, '\0');
            ReadFile(hFile, szFile.data(), nL, &dwRWC, NULL);
            m_Plugs[i]->LoadFromFile(szFile);

            // CN3CPlugBase * pPlug = NULL;
            // e_PlugType     eType = CN3CPlugBase::GetPlugTypeByFileName(szFile);
            // if (eType == PLUGTYPE_NORMAL) { // PLUGTYPE_NORMAL
            //     pPlug = (CN3CPlugBase *)new CN3CPlug();
            //     if (false == pPlug->LoadFromFile(szFile)) {
            //         delete pPlug;
            //         continue;
            //     }
    ...
    ReadFile(hFile, m_nJointPartStarts, sizeof(m_nJointPartStarts), &dwRWC,
             NULL); // 조인트의 일부분이 따로 에니메이션 되야 한다면.. 조인트 인덱스 시작 번호
    ReadFile(hFile, m_nJointPartEnds, sizeof(m_nJointPartEnds), &dwRWC,
...
bool CN3Chr::Save(HANDLE hFile) {
    ...
    // 관절 파일 이름 써주기..
    if (m_pRootJointRef) {
        szFile = m_pRootJointRef->FilePathWin().string();
        nL = szFile.length();
    }
    WriteFile(hFile, &nL, 4, &dwRWC, NULL);
    if (nL > 0) {
    ...
    for (int i = 0; i < iPC; i++) {
        szFile = m_Parts[i]->FilePathWin().string();
        if (szFile.empty()) {
            m_Parts[i]->FilePathSet(std::format("{:s}_Default{:d}.n3cpart", m_szName, i));
            szFile = m_Parts[i]->FilePathWin().string();
        }
    ...
    WriteFile(hFile, &iPC, 4, &dwRWC, NULL);
    for (int i = 0; i < iPC; i++) {
        szFile = m_Plugs[i]->FilePathWin().string();
        if (szFile.empty()) {
            m_Plugs[i]->FilePathSet(std::format("{:s}_Default{:d}.n3cplug", m_szName, i));
            szFile = m_Plugs[i]->FilePathWin().string();
            i++;
        }
    ...
    nL = 0;
    if (m_pAniCtrlRef) {
        szFile = m_pAniCtrlRef->FilePathWin().string();
        nL = szFile.length();
    }
    WriteFile(hFile, &nL, 4, &dwRWC, NULL);
    if (nL > 0) {
    ...
    WriteFile(hFile, m_nJointPartStarts, sizeof(m_nJointPartStarts), &dwRWC,
              NULL); // 조인트의 일부분이 따로 에니메이션 되야 한다면.. 조인트 인덱스 시작 번호
    WriteFile(hFile, m_nJointPartEnds, sizeof(m_nJointPartEnds), &dwRWC,
              NULL); // 조인트의 일부분이 따로 에니메이션 되야 한다면.. 조인트 인덱스 끝 번호
    ...
    nL = 0;
    if (m_pFXPlug) {
        szFile = m_pFXPlug->FilePathWin().string();
        nL = szFile.length();
    }
...
void CN3Chr::Tick(float fFrm) {
    ...
    for (int i = 0; i < MAX_CHR_LOD; i++) {
        if (fLOD < CHR_LOD_CALCULATION_VALUES
                       [0][i]) // 일단 가장 큰값을 기준으로 하고 LOD 값을 정한 다음..... Render 에서 변경한다....
        {
            m_nLOD = i;
            break;
        }
    ...
    if (!m_Plugs.empty()) {
        this->TickPlugs(fLOD); // 붙은 무기 Tick
    }
...
void CN3Chr::TickAnimationFrame() {
    if (NULL == m_FrmCtrl.pAniData) {
        return; // 에니메이션 데이터 가 없다!!
    }

    // 루핑중이고 지연시간이 있으면..
    if ((m_FrmCtrl.pAniData->iBlendFlags & 1) && m_FrmCtrl.iAniLoop > 0 && m_FrmCtrl.bProcessingDelayNow &&
        m_FrmCtrl.fBlendTime == 0) {
        ...
        if (m_FrmCtrl.fFrmCur > m_FrmCtrl.pAniData->fFrmEnd) // 에니메이션이 한번 순환하면..
        {
            if (m_FrmCtrl.fFreezeTime > 0) // 멈춰야 된다면..
            {
                ...
                } else if (m_FrmCtrl.pAniData->iBlendFlags & 1) // 루핑 지연시간이 있으면..
                {
                    ...
                    if (fFrmDiff > 0) {
                        m_FrmCtrl.fFrmCur -= m_FrmCtrl.pAniData->fFrmEnd - m_FrmCtrl.pAniData->fFrmStart;
                    } else {
                        m_FrmCtrl.fFrmCur = m_FrmCtrl.pAniData->fFrmStart;
                    }
                }
    ...
    if (m_FrmCtrlUpper.pAniData) // 상하체를 분리해야 하면.. // 따로 처리해야 한다면..
    {
        if (0.0f != m_FrmCtrlUpper.fBlendTime) // Blending 할 에니메이션이 있으면..
        {
            ...
            if (m_FrmCtrlUpper.fFrmCur > m_FrmCtrlUpper.pAniData->fFrmEnd) // 에니메이션이 한번 순환하면..
            {
                ...
                {
                    m_FrmCtrlUpper.fFrmCur =
                        m_FrmCtrlUpper.pAniData->fFrmEnd; // 에니메이션 데이터의 마지막 프레임으로 하고..
    ...
    if (NULL == m_FrmCtrlUpper.pAniData) // 상하체 통째로 처리..
    {
        ...
        for (int i = 0; i < iJC; i++) {
            if (m_FrmCtrl.fBlendTime > 0) {
                ...
                m_JointRefs[i]->ReCalcMatrixBlended(m_FrmCtrl.fFrmCur, m_FrmCtrl.fBlendFrm,
                                                    fBlendFactor); // Joint Animation Blending...
            } else {
        ...
        for (int i = 0; i < MAX_CHR_ANI_PART; i++) {
            ...
            if (0 == i) {
                if (pFrmCtrls[i]->fBlendTime > 0) // Motion Blending 이 필요하면...
                {
                    ...
                    m_pRootJointRef->ReCalcMatrixBlended(pFrmCtrls[i]->fFrmCur, pFrmCtrls[i]->fBlendFrm,
                                                         fBlendFactor); // Joint Animation Blending...// 하체 일경우
                } else {
            ...
            if (m_nJointPartStarts[i] > 0) {
                ...
                for (int j = m_nJointPartStarts[i]; j <= m_nJointPartEnds[i]; j++) {
                    if (pFrmCtrls[i]->fBlendTime > 0) {
                        m_JointRefs[j]->ReCalcMatrixBlended(pFrmCtrls[i]->fFrmCur, pFrmCtrls[i]->fBlendFrm,
                                                            fBlendFactor); // Joint Animation Blending...
                    } else {
...
void CN3Chr::TickPlugs(float fLOD) {
    ...
    for (int i = 0; i < iPC; i++) {
        ...
        if (pPlug->m_ePlugType == PLUGTYPE_NORMAL) {
            ...
            if (!(pAniData->fFrmPlugTraceEnd - pAniData->fFrmPlugTraceStart)) {
                continue; // 궤적 프레임의 시작과 끝이 차이가 없으면..
            }

            if (pPlug->m_nTraceStep > 0 && fFrmCur >= pAniData->fFrmPlugTraceStart &&
...
void CN3Chr::Render() {
    ...
    if (s_iLODDelta > 0) {
        ...
        for (int i = 0; i < MAX_CHR_LOD; i++) {
            if (fLOD <
                CHR_LOD_CALCULATION_VALUES
                    [s_iLODDelta][i]) // 일단 가장 큰값을 기준으로 하고 LOD 값을 정한 다음..... Render 에서 변경한다....
            {
                m_nLOD = i;
                break;
            }
    ...
    for (int i = 0; i < iPC; i++) {
        ...
        // 검기 그리기...
        if (pPlug->m_ePlugType == PLUGTYPE_NORMAL) {
            if (pPlug->m_nTraceStep <= 1) {
                continue;
            }
            if (false == pPlug->m_bRenderTrace) {
                continue;
            }
...
void CN3Chr::JointSet(const fs::path & fsFile) {
    bool bNeedInit = false;
    if (NULL == m_pRootJointRef) {
        bNeedInit = true;
    } else if (m_pRootJointRef && m_pRootJointRef->FilePath() != fsFile) {
        bNeedInit = true; // 파일 이름이 달라야 지우고 새로 한다..
    }

    if (bNeedInit) {
        s_MngJoint.Delete(&m_pRootJointRef);
        m_pRootJointRef = s_MngJoint.Get(fsFile);
        this->Init(); // 초기화...
    }
}
...
CN3CPart * CN3Chr::PartSet(int iIndex, const fs::path & fsFile) {
    if (iIndex < 0 || iIndex >= m_Parts.size()) {
        return NULL;
    }
    if (m_Parts[iIndex]->FilePath() == fsFile) {
    ...
    if (fsFile.empty()) {
        ...
        m_Parts[iIndex]->LoadFromFile(fsFile);
    }
...
void CN3Chr::PlugDelete(int iIndex) {
    if (iIndex < 0 || iIndex >= m_Plugs.size()) {
        return;
    }
...
CN3CPlug * CN3Chr::PlugSet(int iIndex, const fs::path & fsFile) {
    if (iIndex < 0 || iIndex >= m_Plugs.size()) {
        return NULL;
    }

    if (m_Plugs[iIndex]->FilePath() == fsFile) {
    ...
    if (fsFile.empty()) {
        ...
        m_Plugs[iIndex]->LoadFromFile(fsFile);
    }
...
/*
void CN3Chr::CollisionSkinSet(const fs::path & fsFile)
{
    s_MngSkin.Delete(m_pSkinCollision);
    m_pSkinCollision = s_MngSkin.Get(fsFile);
}
*/

void CN3Chr::AniCtrlSet(const fs::path & fsFile) {
    s_MngAniCtrl.Delete(&m_pAniCtrlRef);
    m_pAniCtrlRef = s_MngAniCtrl.Get(fsFile);
                      ...
                      bool  bStopUpperAnimation) // 상체 에니메이션이 있으면.. 멈추도록
{
    ...
    if (FLT_MIN != fBlendTime) {
        m_FrmCtrl.fBlendTime = fBlendTime; // 모션 Blending 값 설정..
    } else {
        m_FrmCtrl.fBlendTime = m_FrmCtrl.pAniData->fTimeBlend;
    }
    if (m_FrmCtrl.fBlendTime > 0) {
        m_FrmCtrl.fBlendFrm = fFrmPrev; // Blending 할 에니메이션 Frame..
    } else {
        m_FrmCtrl.fBlendFrm = m_FrmCtrl.fFrmCur; // Blending 할 에니메이션 은 없다!!
    }
...
void CN3Chr::AniUpperSet(int iAni, float fFreezeTime) {
    ...
    if (iAni == m_FrmCtrlUpper.iAni) {
        return; // 전의 것과 같으면 넘어간다..
    }
...
void CN3Chr::AniDefaultSet() {
    ...
    pAniCtrlDefault->FilePathSet(fs::path("Chr") / "Default.n3anim");
...
void CN3Chr::FindMinMax() {
    ...
    // 너무 작으면 늘려준다..
    if (m_vMax.x - m_vMin.x < 0.5f) {
        m_vMax.x += 0.25f;
        m_vMin.x -= 0.25f;
    }
    if (m_vMax.y - m_vMin.y < 0.5f) {
        m_vMax.y += 0.25f;
        m_vMin.y -= 0.25f;
    }
    if (m_vMax.z - m_vMin.z < 0.5f) {
        m_vMax.z += 0.25f;
        m_vMin.z -= 0.25f;
    }
...
int CN3Chr::CheckCollisionPrecisely(int ixScreen, int iyScreen, __Vector3 * pvPick) {
    __Vector3 vPos, vDir;
    ::_Convert2D_To_3DCoordinate(ixScreen, iyScreen, CN3Base::s_CameraData.mtxView, CN3Base::s_CameraData.mtxProjection,
...
CN3FXPlug * CN3Chr::FXPlugSet(const fs::path & fsFile) {
    if (m_pFXPlug) {
        m_pFXPlug->Release();
    } else {
        m_pFXPlug = new CN3FXPlug();
    }
    if (!m_pFXPlug->LoadFromFile(fsFile)) {
        return NULL;
    }
    return m_pFXPlug;
}
...
CN3FXPlug * CN3Chr::FXPlugCreate() {
    if (m_pFXPlug) {
        ...
        fs::path fsChrDir("Chr");
        if (m_szName.size() > 0) {
            m_pFXPlug->FilePathSet(fsChrDir / (m_szName + ".n3fxplug"));
        } else {
            m_pFXPlug->FilePathSet(fsChrDir / "Default.n3fxplug"); // 그냥 default이름을 붙인다.
        }
    }
    return m_pFXPlug;
}
