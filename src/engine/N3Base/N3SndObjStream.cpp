...
BOOL CN3SndObjStream::InitWriteBuffer() {
    ...
    hResult =
        m_lpDSBuff->Lock(Offset, m_BlockSize * 2, &pSoundBlock1, &byteSoundBlock1, &pSoundBlock2, &byteSoundBlock2, 0);
    if (FAILED(hResult)) {
        return FALSE;
    }
...
BOOL CN3SndObjStream::WriteBuffer() {
    ...
    hResult =
        m_lpDSBuff->Lock(Offset, m_BlockSize, &pSoundBlock1, &byteSoundBlock1, &pSoundBlock2, &byteSoundBlock2, 0);
    if (FAILED(hResult)) {
        return FALSE;
    }
