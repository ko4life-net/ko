...
#endif // end of #ifndef _3DSERVER
{
  public:
    ...
    struct __CellMain // 기본 셀 데이터
    {
        ...
        void Save(HANDLE hFile) {
            ...
            if (nShapeCount > 0) {
                WriteFile(hFile, pwShapeIndices, nShapeCount * 2, &dwRWC, NULL);
            }
            for (int z = 0; z < CELL_MAIN_DEVIDE; z++) {
                for (int x = 0; x < CELL_MAIN_DEVIDE; x++) {
  ...
  public:
    ...
    float GetHeightNearstPos(const __Vector3 & vPos, float fDist,
                             __Vector3 * pvNormal = NULL); // 가장 가까운 높이을 돌려준다. 없으면 -FLT_MAX 을 돌려준다.
    float GetHeight(float fX, float fZ,
                    __Vector3 * pvNormal = NULL); // 현재 지점에서 제일 높은 값을 돌려준다. 없으면 -FLT_MAX 을 돌려준다.
    int   SubCellPathThru(const __Vector3 & vFrom, const __Vector3 & vAt, int iMaxSubCell,
