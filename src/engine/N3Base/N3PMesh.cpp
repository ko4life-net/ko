...
CN3PMesh::~CN3PMesh() {
#ifdef _USE_VERTEXBUFFER
    if (m_pVB && m_pVB->Release() == 0) {
        m_pVB = NULL;
    }
    if (m_pIB && m_pIB->Release() == 0) {
        m_pIB = NULL;
    }
#else
    delete[] m_pVertices;
...
#ifdef _DEBUG
        if (bFixed) {
            ::MessageBox(s_hWndBase, "Fix incorrect Progressive Mesh", m_szName.c_str(), MB_OK);
        }
#endif
    }
...
bool CN3PMesh::Save(HANDLE hFile) {
...
#else
    if (m_iMaxNumVertices > 0) {
        WriteFile(hFile, m_pVertices, m_iMaxNumVertices * sizeof(__VertexT1), &dwNum, NULL);
    }
    if (m_iMaxNumIndices > 0) {
        WriteFile(hFile, m_pIndices, m_iMaxNumIndices * sizeof(WORD), &dwNum, NULL);
    }
#endif
    ...
    if (m_iNumCollapses > 0) {
        for (int i = 0; i < m_iNumCollapses; i++) {
            if (m_pCollapses[i].iIndexChanges < 0) {
                m_pCollapses[i].iIndexChanges = 0; // save..
            }
        }
        WriteFile(hFile, m_pCollapses, m_iNumCollapses * sizeof(__EdgeCollapse), &dwNum, NULL);
    }
    if (m_iTotalIndexChanges > 0) {
        WriteFile(hFile, m_pAllIndexChanges, m_iTotalIndexChanges * sizeof(m_pAllIndexChanges[0]), &dwNum, NULL);
    }

    WriteFile(hFile, &m_iLODCtrlValueCount, sizeof(m_iLODCtrlValueCount), &dwNum, NULL);
    if (m_iLODCtrlValueCount > 0) {
        WriteFile(hFile, m_pLODCtrlValues, m_iLODCtrlValueCount * sizeof(__LODCtrlValue), &dwNum, NULL);
    }
...
void CN3PMesh::FindMinMax() {
    ...
    for (int i = 0; i < m_iMaxNumVertices; i++) {
        if (pVertices[i].x < m_vMin.x) {
            m_vMin.x = pVertices[i].x;
        }
        if (pVertices[i].y < m_vMin.y) {
            m_vMin.y = pVertices[i].y;
        }
        if (pVertices[i].z < m_vMin.z) {
            m_vMin.z = pVertices[i].z;
        }
        if (pVertices[i].x > m_vMax.x) {
            m_vMax.x = pVertices[i].x;
        }
        if (pVertices[i].y > m_vMax.y) {
            m_vMax.y = pVertices[i].y;
        }
        if (pVertices[i].z > m_vMax.z) {
            m_vMax.z = pVertices[i].z;
        }
    }
    m_pVB->Unlock();
#else
    ...
    // Calculate the radius with the maximum and minimum values.
...
HRESULT CN3PMesh::Create(int iNumVertices, int iNumIndices) {
    ...
    if (m_iMaxNumVertices > 0) {
        hr = m_lpD3DDev->CreateVertexBuffer(m_iMaxNumVertices * sizeof(__VertexT1), D3DUSAGE_DYNAMIC, FVF_VNT1,
    ...
    if (m_iMaxNumIndices > 0) {
        hr = m_lpD3DDev->CreateIndexBuffer(m_iMaxNumIndices * sizeof(WORD), D3DUSAGE_DYNAMIC, D3DFMT_INDEX16,
...
HRESULT CN3PMesh::GenerateSecondUV() {
    ...
    if (m_iMaxNumVertices > 0) {
        hr = m_lpD3DDev->CreateVertexBuffer(m_iMaxNumVertices * sizeof(__VertexT2), D3DUSAGE_DYNAMIC, FVF_VNT2,
...
void CN3PMesh::ReGenerateSmoothNormal() {
    ...
    PMI.SetLODByNumVertices(m_iMaxNumVertices); // Set it to the maximum point...
