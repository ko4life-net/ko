...
bool CN3SndObj::StaticInit(HWND hWnd, DWORD dwCoopLevel, DWORD dwPrimaryChannels, DWORD dwPrimaryFreq,
...
bool CN3SndObj::Create(const std::string & szFN, e_SndType eType) {
    ...
    if (SNDTYPE_2D != eType && SNDTYPE_3D != eType) {
        return false;
    }
    if (m_lpDSBuff) {
        this->Init();
    ...
    if (SNDTYPE_2D == eType) // 2D sound source
    {
        dsbd.dwFlags = DSBCAPS_CTRLVOLUME; // | DSBCAPS_STATIC;
    } else if (SNDTYPE_3D == eType)        // 3D sound source..
    {
...
void CN3SndObj::SetVolume(int Vol) {
    ...
    if (m_lpDS3DBuff) {
        return; // When it is 3D Sound, the sound cannot be adjusted..!!!
    }
    ...
    long  dwVol = (long)(log10(fVol) * 3000); // Decibel-related sound control..
...
//
void CN3SndObj::Tick() {
    if (NULL == m_lpDSBuff || m_ePlayState == SNDSTATE_STOP) {
        return;
    }
    ...
    if (m_ePlayState == SNDSTATE_FADEIN) {
        if (m_fTmpSecPerFrm >= m_fFadeInTime) {
            ...
            if (m_fFadeInTime > 0.0f) {
                vol = ((m_fTmpSecPerFrm / m_fFadeInTime) * (float)m_iMaxVolume);
            }
            SetVolume(vol);
        }
    }
...
void CN3SndObj::Play(const D3DVECTOR * pvPos, float delay, float fFadeInTime, bool bImmediately) {
    ...
    if (m_lpDS3DBuff) // When it is 3D sound, there is no need for FadeIn etc.. Return because the volume does not eat..
    {
        m_ePlayState = SNDSTATE_PLAY;
        if (m_lpDSBuff) {
            if (m_bIsLoop) {
...
void CN3SndObj::SetConeOutSizeVolume(LONG vol) {
    if (m_lpDS3DBuff) {
        m_lpDS3DBuff->SetConeOutsideVolume(vol, DS3D_IMMEDIATE);
    }
}
...
void CN3SndObj::SetConeOrientation(D3DVECTOR * pDir) {
    if (m_lpDS3DBuff) {
        m_lpDS3DBuff->SetConeOrientation(pDir->x, pDir->y, pDir->z, DS3D_IMMEDIATE);
    }
}
...
void CN3SndObj::SetListenerPos(const D3DVECTOR * pVPos, bool IsDeferred) {
    if (NULL == s_lpDSListener || NULL == pVPos) {
        return;
    }
...
void CN3SndObj::SetListenerOrientation(const D3DVECTOR * pVAt, const D3DVECTOR * pVUp, bool IsDeferred) {
    if (NULL == s_lpDSListener || NULL == pVAt || NULL == pVUp) {
        return;
    }
    DWORD dwParam = (IsDeferred) ? DS3D_DEFERRED : DS3D_IMMEDIATE;
