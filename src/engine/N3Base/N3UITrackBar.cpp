...
void CN3UITrackBar::SetRegion(const RECT & Rect) {
    ...
    if (iThumbWidth <= 0 && iThumbHeight <= 0) { // thumb 이미지는 설정되어 있지 않다면 임으로 적당하게 설정
        RECT rc;
        if (UISTYLE_TRACKBAR_VERTICAL == m_dwStyle) { // 세로
            rc.left = Rect.left;
            rc.top = Rect.top + iBkHeight * 0.3f;
            rc.right = Rect.right;
            rc.bottom = rc.top + iBkHeight * 0.3f;
        } else { // 가로
            rc.left = Rect.left + iBkWidth * 0.3f;
            rc.top = Rect.top;
            rc.right = rc.left + iBkWidth * 0.3f;
