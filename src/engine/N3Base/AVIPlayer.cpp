...
void CAVIPlayer::Release() {
    if (m_pGraphBuilder) {
        m_pGraphBuilder->Release(), m_pGraphBuilder = NULL;
    }
    if (m_pMediaControl) {
        m_pMediaControl->Release(), m_pMediaControl = NULL;
    }
    if (m_pVideoWindow) {
        m_pVideoWindow->Release(), m_pVideoWindow = NULL;
    }
    if (m_pMediaEvent) {
        m_pMediaEvent->Release(), m_pMediaEvent = NULL;
    }
...
bool CAVIPlayer::PlayCutScene(LPTSTR pszMovie, HINSTANCE hInstance) {
    ...
    while (1) {
        ...
        if (E_ABORT != m_pMediaEvent->GetEvent(&lEventCode, (LONG_PTR *)&lParam1, (LONG_PTR *)&lParam2, 0)) {
...
BOOL CAVIPlayer::CreateHiddenWindow(HINSTANCE hInstance, LPTSTR szFile) {
    ...
    m_hWndMain = CreateWindowEx(0, "AVI", szTitle,
                                0, // not visible
