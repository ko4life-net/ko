...
bool CN3FXPartBillBoard::ParseScript(char * szCommand, char * szBuff0, char * szBuff1, char * szBuff2, char * szBuff3) {
    ...
    //    보드 갯수.
    if (n3std::iequals(szCommand, "<billboard_count>")) {
        m_iNum = atoi(szBuff0);
        if (m_iNum > 0) {
            CreateVB();
        }
    ...
    //    보드 크기.
    if (n3std::iequals(szCommand, "<billboard_size>")) {
    ...
    if (n3std::iequals(szCommand, "<distance>")) {
        m_fRadius = atof(szBuff0);
        return true;
    }

    if (n3std::iequals(szCommand, "<Rotate_only_y>")) {
        if (n3std::iequals(szBuff0, "true")) {
            m_bRoateOnlyY = true;
        } else if (n3std::iequals(szBuff0, "false")) {
            m_bRoateOnlyY = false;
    ...
    if (n3std::iequals(szCommand, "<billboard_scale_velocity>")) {
    ...
    if (n3std::iequals(szCommand, "<billboard_scale_acceleration>")) {
        m_fScaleAccelX = atof(szBuff0);
        m_fScaleAccelY = atof(szBuff1);
        return true;
    }

    if (n3std::iequals(szCommand, "<billboard_Rotation>")) {
        m_fRotBillBoardX = atof(szBuff0);
        m_fRotBillBoardY = atof(szBuff1);
        m_fRotBillBoardZ = atof(szBuff2);
        m_mtxRot.Identity();
        m_mtxRot.Rotation(D3DXToRadian(m_fRotBillBoardX), D3DXToRadian(m_fRotBillBoardY),
...
bool CN3FXPartBillBoard::Load(HANDLE hFile) {
    ...
    if (m_iVersion >= 3) {
        ReadFile(hFile, &m_bRoateOnlyY, sizeof(bool), &dwRWC, NULL);
    }
    ...
    if (m_iVersion >= 6) { // Todo
        bool m_bRotationRate;
        ReadFile(hFile, &m_bRotationRate, sizeof(bool), &dwRWC, NULL); // is this Rotation Rate bool?
    }
...
bool CN3FXPartBillBoard::Save(HANDLE hFile) {
    ...
    if (m_iVersion >= 3) {
        WriteFile(hFile, &m_bRoateOnlyY, sizeof(bool), &dwRWC, NULL);
    }
...
void CN3FXPartBillBoard::Render() {
    ...
    if (!m_bRoateOnlyY) {
        ...
        if (vRadiusPos.z == 0.0f && vRadiusPos.x >= 0) {
            ...
            ang = -atan2(vRadiusPos.x, -vRadiusPos.z);
        }
    ...
    if (m_bAlpha) // Alpha 사용
    {
        ...
        for (int i = 0; i < m_iNum; i++) {
            HRESULT hr =
                CN3Base::s_lpD3DDev->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2, &(m_pVB[i * 4]), sizeof(__VertexXyzColorT1));
        }
