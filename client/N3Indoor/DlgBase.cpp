...
void CDlgBase::UpdateInfo()
{
	...
	if(pBase->Type() & OBJ_TRANSFORM)
	{
		...
		if(pItem) pItem->VectorSet(pSI->Pos());
		pItem = m_LPTransform.GetPropItem("Scale");
		if(pItem) pItem->VectorSet(pSI->Scale());
	...
	if(pBase->Type() & OBJ_SHAPE)
	{
		...
		pItem = m_LPShape.GetPropItem("소속");			if(pItem) pItem->m_curValue.Format("%d", pSI->m_iBelong);
		pItem = m_LPShape.GetPropItem("Event ID");		if(pItem) pItem->m_curValue.Format("%d", pSI->m_iEventID);
		pItem = m_LPShape.GetPropItem("Event Type");	if(pItem) pItem->m_curValue.Format("%d", pSI->m_iEventType);
		pItem = m_LPShape.GetPropItem("NPC ID");		if(pItem) pItem->m_curValue.Format("%d", pSI->m_iNPC_ID);
		pItem = m_LPShape.GetPropItem("NPC Status");		if(pItem) pItem->m_curValue.Format("%d", pSI->m_iNPC_Status); // NPC 로 쓰는 오브젝트일 경우 NPC Type
...
BOOL CDlgBase::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	CMainFrame* pFrm =  NULL;
	pFrm = (CMainFrame* )AfxGetMainWnd();
	if (!pFrm)	return CDialog::OnNotify(wParam, lParam, pResult);
	...
	if(pBase != NULL)
	{
		...
		if((void*)wParam == &m_LPShape && (pBase->Type() & OBJ_SHAPE))
		{
			...
			if(pItem->m_propName == "소속" && pItem->m_curValue.GetLength() > 0)			pSI->m_iBelong = atoi(pItem->m_curValue);
			else if(pItem->m_propName == "Event ID" && pItem->m_curValue.GetLength() > 0)	pSI->m_iEventID = atoi(pItem->m_curValue);
			else if(pItem->m_propName == "Event Type" && pItem->m_curValue.GetLength() > 0) pSI->m_iEventType = atoi(pItem->m_curValue);
			else if(pItem->m_propName == "NPC ID" && pItem->m_curValue.GetLength() > 0)		pSI->m_iNPC_ID = atoi(pItem->m_curValue);
			else if(pItem->m_propName == "NPC Status" && pItem->m_curValue.GetLength() > 0) pSI->m_iNPC_Status = atoi(pItem->m_curValue); // NPC 로 쓰는 오브젝트일 경우 NPC Type
